			...
			if empty "$FORM_submit"; then
				...
				empty "$FORM_generate_wep_128" && empty "$FORM_generate_wep_40" &&
				{
					...
					case "$FORM_mode" in
						...
						mesh)
							append validate_forms "string|FORM_mesh_id_$vcfg|@TR<<MESH ID>>||$FORM_mesh_id" "$N";; 
						*)
...
EOF
			...
			javascript_forms="
				...
				if (isset('mode_$vcfg','mesh'))
				{
					...
					set_visible('mesh_id_form_$vcfg', v);

					v = !isset('mode_$vcfg','mesh');
					set_visible('ssid_form_$vcfg', v);

					if (isset('encryption_$vcfg','psk') || isset('encryption_$vcfg','psk2') || isset('encryption_$vcfg','psk-mixed/tkip+aes')) 
					{ 
