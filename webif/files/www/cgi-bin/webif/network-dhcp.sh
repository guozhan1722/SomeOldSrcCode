...
config="lan"
pre_ip="$(ifconfig br-lan | grep inet | awk -F'[: ]+' '{print $4}' | awk -F '.' '{print $1"."$2"."$3"."}')"
	...
	if [ "$FORM_submit" = "" ]; then
		...
		do
			...
			count=$(($count + 1))
                ...
                ip_cnt=$(expr $limit + $start)
	else
		config_get interface $config interface
		eval start="\$FORM_start_$config"
#		eval limit="\$FORM_limit_$config"
		eval ip_cnt="\$FORM_limit_$config"
		limit=$(expr $ip_cnt - $start)
