...
if equal $FORM_rdflash "1" ; then
	...
	echo "<input type='hidden' name='rdflash' value='1' /><input type='hidden' name='file' value=\"$FORM_configfile\" />"	
			...
			if [ ! -e "$tmp/config.name" ] || [ ! -e "$tmp/config.boardtype" ]; then
				echo "<tr><td colspan=\"2\">@TR<<confman_invalid_file#Invalid file>>: VIP421.config!</td></tr>" 
			else
...
cat <<EOF
<tr><td width="70%">@TR<<Reset to Default Configuration>>?&nbsp;&nbsp;&nbsp;</td>
<td><input class="flatbtn" type="submit" name="reset" value="@TR<<Reset>>" /></td> </tr> 
EOF
...
cat<<EOF
<tr>
<td width="70%">@TR<<Saved VIP421.config file>>:&nbsp;&nbsp;&nbsp;<input type="file" class="flatbtn" name="configfile" /></td>
