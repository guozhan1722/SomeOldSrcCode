...
for rule in $rule_cfgs; do
	if [ "$FORM_submit" = "" -o "$add_rule_cfg" = "$rule" ]; then
		...
		eval FORM_target="\$FORM_target_$rule"
                eval mac="\$(echo $FORM_src_ip |cut -c3)"

                if [ "$mac" = ":" ]; then
       		validate <<EOF
mac|FORM_src_ip|@TR<<Source IP Address>>||$FORM_src_ip
...
EOF
   		equal "$?" 0 && {
			...
			uci_set firewall "$rule" src_mac "$FORM_src_ip"
...
footer ?>
<!--
###WEBIF:name:Network:800:Firewall
-->
