...
#if 0
#ident	"ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mData.c#8 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mData.c#8 $"
#endif
...
static void rxDataCompleteSniff
(
 A_UINT32 devNum,
 A_UINT32 waitTime,
 A_UINT32 timeout,
...
static void txEnableCompression
(
 A_UINT32 devNum
);
...
)
{
	...
	if(flags & TX_CLEAN) {
		...
		if(pLibDev->tx[queueIndex].compBufferAddr) {
			memFree(devNum, pLibDev->tx[queueIndex].compBufferAddr);
			pLibDev->tx[queueIndex].compBufferAddr = 0;
		}
	}
	if(flags & RX_CLEAN) {
...
/**************************************************************************
* txEnableCompression - Assuming txDataSetup has already been called,
*	process descriptors for enabling compression
*
*/
void txEnableCompression
(
 A_UINT32 devNum
)
{
	...
	//allocate the scratch buffer that the mac will need
	//needs to be 2 max packet sizes + 128 and aligned on 512 byte boundary
	pLibDev->tx[queueIndex].compBufferAddr = memAlloc(devNum, COMP_SCRATCH_BUFFER_SIZE);
	alignedCompBuffer = pLibDev->tx[queueIndex].compBufferAddr;

//printf("SNOOP: buffer before alignment = %x\n", pLibDev->tx[queueIndex].compBufferAddr);
	if(alignedCompBuffer & (512 - 1)) {
		//buffer needs to be 512 byte aligned
		alignedCompBuffer += 512 - (alignedCompBuffer & (512 - 1));
	}
//printf("SNOOP: buffer after alignment = %x\n", pLibDev->tx[queueIndex].compBufferAddr);

	//configure the qcu registers for compression, write buffer address and size
	//for correct qcu
	REGW(devNum, 0xb00, (80+(2*queueIndex)));
//printf("SNOOP: wrote %x to 0xb00 (queueIndex = %d)\n", (80+(2*queueIndex)), queueIndex);
	REGW(devNum, 0xb04, alignedCompBuffer);
//printf("SNOOP: wrote %x to 0xb04\n", pLibDev->tx[queueIndex].compBufferAddr);
	REGW(devNum, 0xb08, 12);

	//enable the QCU for compression
	regData = REGR(devNum, 0x9c0 + (queueIndex << 2));
	regData |= (1 << 12);
//printf("SNOOP: wrote %x to %x\n", regData, 0x9c0 + (queueIndex << 2));
	REGW(devNum, 0x9c0 + (queueIndex << 2), regData);
}
...
)
{
	...
	if(pLibDev->libCfgParams.enableCompression) {
		txEnableCompression(devNum);
	}
...
memDisplay(devNum, pLibDev->tx[0].descAddress, 8);
printf("SNOOP::txDataComplete::Frame contents at %x pointed location is \n", pLibDev->tx[0].descAddress);
...
)
{
	...
	{
		localDescPtr->bufferPhysPtr = bufferAddress;

		//update the link pointer
		if (i == pLibDev->rx.numDesc - 1) { //ie its the last descriptor
			//link it to itself
			localDescPtr->nextPhysPtr = descAddress;
...
//		}
		...
		if(pLibDev->rx.overlappingDesc) {
			...
			buffAddrIncrement = (sizeof(MDK_PACKET_HEADER) + sizeof(WLAN_DATA_MAC_HEADER3) + 0x22  + (enablePPM ? PPM_DATA_SIZE : 0));
		} else {
...
/**************************************************************************
* rxEnableDecompression - Enable dempression of packets from
*	specified node
*
*/
MANLIB_API void rxEnableDecompression
(
 A_UINT32 devNum,
 A_UCHAR *mac,						//address of station sending compressed frames
 A_UINT32 keyCacheIndex				//index into key cache to create entry
)
{
//	LIB_DEV_INFO *pLibDev = gLibInfo.pLibDevArray[devNum];
	WLAN_MACADDR macAddress;
	A_UINT32 wrdata, rddata, addr0, addr1, half, word;

	memcpy(macAddress.octets, mac, WLAN_MAC_ADDR_SIZE);

    REGW(devNum, 0x8800+(keyCacheIndex*0x20), 0x55555555);
    REGW(devNum, 0x8804+(keyCacheIndex*0x20), 0x5555);
    REGW(devNum, 0x8808+(keyCacheIndex*0x20), 0xaaaaaaaa);
    REGW(devNum, 0x880c+(keyCacheIndex*0x20), 0xaaaa);
    REGW(devNum, 0x8810+(keyCacheIndex*0x20), 0xcccccccc);
    REGW(devNum, 0x8814+(keyCacheIndex*0x20), 0x5);


#ifndef ARCH_BIG_ENDIAN
	half = macAddress.st.half;
	word = macAddress.st.word;
#else
	half = (A_UINT32)swap_s(macAddress.st.half);
	word = swap_l(macAddress.st.word);
#endif
    addr0 = ((half & 0x1) << 31) | ((word >> 1) & 0x7fffffff);
//    addr0 = ((macAddress.st.half & 0x1) << 31) | ((macAddress.st.word >> 1) & 0x7fffffff);
    REGW(devNum, 0x8818+(keyCacheIndex*0x20), addr0);

    addr1 = (half >> 1) & 0x7fff;
    wrdata = (1 << 15) | addr1;
    REGW(devNum, 0x881c+(keyCacheIndex*0x20), wrdata);

    REGW(devNum, 0x0400, keyCacheIndex);              //  # Key Index for Decomp Mask
    REGW(devNum, 0x0404, 0x1);             //  # Enable Decomp for Index
    //REGW(devNum, 0x420, 0x5);               // Decompression global enable and broadcast
    REGW(devNum, 0x0420, 0x1);             //  # Decompression global enable

    rddata = REGR(devNum, 0x8004);
    wrdata = rddata | (1 << 31);       // # Enable key search for Broadcast
    REGW(devNum, 0x8004, wrdata);
}
...
)
{
  ...
  *error=1;
  ...
  if (pFile!=NULL)
  {
    ...
    fprintf (pFile, "%02x-%02x-%02x-%02x-%02x-%02x",macAddr[5],macAddr[4],macAddr[3],macAddr[2],macAddr[1],macAddr[0]);
    fclose (pFile);
    printf("MAC ADRRESS has been stored in %s\n",file_name);
    *error=0;
    return;
  }
  printf("Cannot store MAC ADRRESS in the file %s\n",file_name);
...
memDisplay(devNum, pLibDev->rx.descAddress, 8);
printf("reg8000=%x:reg8004=%x:reg8008=%x:reg800c=%x:reg8014=%x\n", REGR(devNum, 0x8000), REGR(devNum, 0x8004), REGR(devNum, 0x8008), REGR(devNum, 0x800c), REGR(devNum, 0x8014));
...
*/
			...
			if (!(remoteStats & LEAVE_DESC_STATUS)) {
			    //zero out status
			    pLibDev->devMap.OSmemWrite(devNum, statsInfo.descToRead + FIRST_STATUS_WORD,
						(A_UCHAR *)tempBuff, 8);
			}

//pLibDev->devMap.OSmemWrite(devNum, statsInfo.descToRead + SECOND_STATUS_WORD,
//			(A_UCHAR *)&tempBuff, 4);
...
//					(A_UCHAR *)&statsInfo.bufferAddr, sizeof(statsInfo.bufferAddr));
			...
			if ( !(statsInfo.status1 & DESC_MORE) ) {
				...
				if(skipStats && (numStatsToSkip != 0)) {
					...
					// Processing the packet and extracting the SSID information
					//==========================================================//
					A_UINT16			 frameControlValue;
					WLAN_MACADDR		 frameMacAddr = {{0}};
					...
					if(statsInfo.status2 & DESC_FRM_RCV_OK) // take only the valid frames
						...
						if(memValue==0x80){ // process only the beacons
							...
							if(ssid_length==0){ // hidden SSID
								rssi=(A_INT8)((statsInfo.status1 >> pLibDev->bitsToRxSigStrength) & SIG_STRENGTH_MASK);
								...
								SSID[k++]=(memValue>>16)&0x000000ff;
								SSID[k++]=(memValue>>24)&0x000000ff;
								if(ssid_length>2)
									for(iIndex=10; iIndex<(11+((ssid_length-3)/4)); iIndex++){
											...
											for(i=0;((i<4)&&(k<ssid_length));i++,k++){
													SSID[k]=(memValue>>i*8)&0x000000ff;
											}
									}
								SSID[k]=0;
								//compare the SSIDs
						...
						}// if(statsInfo.status2 & DESC_FRM_RCV_OK) , if(memValue==0x80){
			...
			else {//if ( !(statsInfo.status1 & DESC_MORE) )
			if(i != timeout) {
				curTime = milliTime();
				if (curTime > (startTime+timeout)) {
					i = timeout;
...
/**************************************************************************
* rxDataSniff - Start and complete reception for sniffer type mode
*
*/
MANLIB_API void rxDataSniff
(
 A_UINT32 devNum,
 A_UINT32 waitTime,
 A_UINT32 timeout,
...
)
{
	LIB_DEV_INFO *pLibDev = gLibInfo.pLibDevArray[devNum];
	pLibDev->mdkErrno = 0;
	pLibDev->rx.rxMode = RX_PROMISCUOUS_MODE;
	rxDataStart(devNum);
	if (pLibDev->mdkErrno == 0) {
		rxDataCompleteSniff(devNum, waitTime, timeout, enableCompare, dataPattern, dataPatternLength);
	}
}

/**************************************************************************
* rxDataCompleteSniff - Complete the reception of the sniff packets
*
*/
void rxDataCompleteSniff
(
 A_UINT32 devNum,
 A_UINT32 waitTime,
 A_UINT32 timeout,
...
//	junkErrCounter = 0;
	...
	do {
		...
		if (statsInfo.status2 & DESC_DONE) {
			...
			if (numDesc == pLibDev->rx.numExpectedPackets - 1) {
				//we are done
				rxComplete = TRUE;
			}
...
printf("SNOOP::Process packet:bP=%d:cFR=%d:iBS=%d\n", statsInfo.badPacket, statsInfo.controlFrameReceived, statsInfo.illegalBuffSize);
#endif
			//only process packets if things are good, for now just check for full packet
			if ( !(statsInfo.status1 & DESC_MORE) ) {
#ifdef DEBUG_MEMORY
printf("SNOOP::Processing packet\n");
#endif
				...
				if ((rxStatsInfo.status2 & DESC_FRM_RCV_OK)
				 && (rxStatsInfo.mdkPktType == MDK_LAST_PKT)){
					...
					if(skipStats && (numStatsToSkip != 0)) {
						...
						rxStatsInfo.descNumber = numDesc;
...
#ifdef DEBUG_MEMORY
printf("SNOOP::Desc contents are\n");
memDisplay(devNum, DescAddress, 8);
#endif
...
)
{
	...
	A_UINT16			 frameControlValue;
	LIB_DEV_INFO		*pLibDev = gLibInfo.pLibDevArray[devNum];
	WLAN_MACADDR		 frameSrcMacAddr = {{0}};
	WLAN_MACADDR		 frameDestMacAddr = {{0}};
	WLAN_MACADDR		 frameBssMacAddr = {{0}};
	WLAN_MACADDR		 broadcastMacAddr = {{0xff,0xff,0xff,0xff,0xff,0xff}};
...
printf("SNOOP::Address 2 offset = %x\n", ADDRESS2_START);
	...
	for (i = 0; i < WLAN_MAC_ADDR_SIZE; i++)
	{
		memcpy( &(frameSrcMacAddr.octets[i]), &(pStatsInfo->frame_info[0]) + ADDRESS2_START + i, sizeof(frameSrcMacAddr.octets[i]));
		memcpy( &(frameDestMacAddr.octets[i]), &(pStatsInfo->frame_info[0]) + ADDRESS1_START + i, sizeof(frameDestMacAddr.octets[i]));
		memcpy( &(frameBssMacAddr.octets[i]), &(pStatsInfo->frame_info[0]) + ADDRESS3_START + i, sizeof(frameBssMacAddr.octets[i]));
...
*/
 ...
 if(pLibDev->rx.rxMode == RX_FIXED_NUMBER) {
        ...
        if(A_MACADDR_COMP(&pLibDev->bssAddr, &frameBssMacAddr)) {
			pStatsInfo->badPacket = TRUE;
			pStatsInfo->gotHeaderInfo = TRUE;
			return;
		}
		...
		if (A_MACADDR_COMP(&(pStatsInfo->addrPacket), &frameSrcMacAddr)) {
		...
		if((pStatsInfo->mdkPktType >= MDK_NORMAL_PKT) && (pStatsInfo->mdkPktType <= MDK_TXRX_STATS_PKT)) {
			A_MACADDR_COPY(&frameSrcMacAddr, &pStatsInfo->addrPacket);
...
//			  mError(devNum, EIO, "Device Number %d:mdkExtractAddrAndSequence: A non-MDK packet was received before any MDK packets - ignoring\n", devNum);
    ...
    //New test - verify mac addresses read from the packet are as expected based on what we know
	if(A_MACADDR_COMP(&pLibDev->bssAddr, &frameBssMacAddr)) {
		pStatsInfo->addressMismatch = TRUE;
	}
	if(A_MACADDR_COMP(&broadcastMacAddr, &frameDestMacAddr)) {
        //its not broadcast, so look for match on dest address to our address
        if(A_MACADDR_COMP(&pLibDev->macAddr, &frameDestMacAddr)) {
    		pStatsInfo->addressMismatch = TRUE;
        }
	}
...
)
{
	A_BOOL			isDuplicate;
	LIB_DEV_INFO *pLibDev = gLibInfo.pLibDevArray[devNum];
	A_UINT32 descTempTime, descTotalTime;
	float newRxTime = 0;
	A_UINT32 bitsReceived;
	A_BOOL dontCountThisPacket = FALSE;


	/* first check for good packets */
	if(pStatsInfo->descNumber == 0) {
//printf("Snoop: first descriptor = %x\n", pStatsInfo->status2);
		pStatsInfo->descPreviousTime = (pStatsInfo->status2 >> 16) & 0x7fff;
		descTotalTime = 0;
		if(pStatsInfo->status2 & DESC_FRM_RCV_OK) {
			dontCountThisPacket = TRUE; //need to remove the 1st packet from new throughput calculation
		}

		//update the rate start time
		//pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].startTime = descStartTime;
		pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].totalTime = descTotalTime;
	}
	else {
		descTotalTime = pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].totalTime;
		descTempTime = (pStatsInfo->status2 >> 16) & 0x7fff;
		if (descTempTime < pStatsInfo->descPreviousTime) {
			descTotalTime += (0x8000 - pStatsInfo->descPreviousTime + descTempTime);
		}
		else {
			descTotalTime += (descTempTime - pStatsInfo->descPreviousTime);
		}
		pStatsInfo->descPreviousTime = descTempTime;
//		descEndTime = pStatsInfo->descMidTime;
		pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].totalTime = descTotalTime;
		if(pStatsInfo->lastDescRate != pStatsInfo->descRate) {
			//this is the next rate so update the time
		//	descStartTime = pStatsInfo->descMidTime;
			pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].totalTime = 0;

			//update the throughput on the last rate
			if(pStatsInfo->status2 & DESC_FRM_RCV_OK) {
				//need to remove the 1st packet from new throughput calculation
				dontCountThisPacket = TRUE;
			}

		}
	}

	pStatsInfo->lastDescRate = pStatsInfo->descRate;
	...
	if (pStatsInfo->status2 & DESC_FRM_RCV_OK)
	{
		...
		if (!isDuplicate)
		{
			...
			if(!dontCountThisPacket) {
				pStatsInfo->totalTPBytes += ((pStatsInfo->status1 & 0xfff) -
					sizeof(WLAN_DATA_MAC_HEADER3) - FCS_FIELD);
				if(pLibDev->wepEnable) {
					pStatsInfo->totalTPBytes -= (WEP_IV_FIELD + 2);
				}

			}

			//calculate the rx throughput
//printf("SNOOP: pT = %d, tT = %d ", pStatsInfo->descPreviousTime, descTotalTime);
//printf("sts1 = %x, sts2 = %x\n", pStatsInfo->status1, pStatsInfo->status2);
			bitsReceived = pStatsInfo->totalTPBytes * 8;
			newRxTime = (float)((descTotalTime))/1000;  //puts the time in millisecs
			pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].newThroughput =
				(A_UINT32)((float)bitsReceived / newRxTime);
//printf("SNOOP: tb = %d, bR = %d, Time = %5.2f tput = %5.1f\n", pStatsInfo->totalBytes, bitsReceived,
//		    newRxTime, (float)pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].newThroughput/1000);
		}
	...
	else
	{
		...
		else if (pStatsInfo->status1 & 0x6000)
		{
			pLibDev->rx.rxStats[pStatsInfo->qcuIndex][0].decompressionErrors++;
			pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].decompressionErrors++;
		}
	}
...
/*  Siva
		pLibDev->devMap.OSmemRead(devNum, pStatsInfo->bufferAddr - PPM_DATA_SIZE, (A_UCHAR *)&pktData, sizeof(pktData));
*/
	#ifdef ARCH_BIG_ENDIAN
...
//	pktBodySize -=4;
	...
	while (pktBodySize) {
		...
		if(mismatchBits || pStatsInfo->addressMismatch) {
//printf("SNOOP: mismatched bits at %d location, data = %x, compare = %x\n", pktBodySize,
//	   pktData, compareWord);
			...
			if (pStatsInfo->status2 & DESC_FRM_RCV_OK) {
                ...
                if(pStatsInfo->addressMismatch) {
                    //addresses were not as expected, this should be flagged as an error compare
				    pLibDev->rx.rxStats[pStatsInfo->qcuIndex][0].bitErrorCompares ++;
				    pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].bitErrorCompares ++;
                }
//				printf("snoop: error at %d/%d : mismatch bits = %d [0x%x==0x%x]\n", pktBodySize, initSize, countBits(mismatchBits), compareWord, pktData);
                ...
                if(pStatsInfo->addressMismatch) {
                    //addresses were not as expected, this should be flagged as an error compare
				    pLibDev->rx.rxStats[pStatsInfo->qcuIndex][0].bitMiscompares ++;
				    pLibDev->rx.rxStats[pStatsInfo->qcuIndex][pStatsInfo->descRate].bitMiscompares ++;
                }
			}
		}
		pTempPtr ++;
...
//		statsInfo->totalBytes = (statsInfo->status1 & 0xfff);
		...
		if (statsInfo->status2 & DESC_DONE) {
			...
			if (pLibDev->rx.enablePPM) {
			  frame_info_len += PPM_DATA_SIZE;
			  statsInfo->ppm_data_len = PPM_DATA_SIZE;
			}
	...
	//printf("SNOOP::fillRxDescAndFrame:enablePPM=%d:frame_info_len=%d\n", pLibDev->rx.enablePPM, frame_info_len);
			if (pLibDev->rx.enablePPM) {
			  memcpy( &(statsInfo->ppm_data[0]), &(tmpFrameInfo[0]), PPM_DATA_SIZE);
			  memcpy( &(statsInfo->frame_info[0]), &(tmpFrameInfo[0])+PPM_DATA_SIZE, (frame_info_len - PPM_DATA_SIZE));
			}
			else {
