...
#ifdef NO_GPIB
#define GPIB_BAD_CALL() do { \
        fprintf(stderr, "%s:%d GPIB is disabled\n", __func__, __LINE__); \
} while(0)
#define gpibWrite(args...) GPIB_BAD_CALL()
#define gpibQuery(args...) ({ GPIB_BAD_CALL(); ""; })
#define ibrd(args...) ({ GPIB_BAD_CALL(); -1; })
#define ibdev(args...) ({ GPIB_BAD_CALL(); -1; })
#else
...
MANLIB_API double tempMeasTemp(const unsigned long ud) {
    ...
    //if ( rsp[pos] == 'N' ) return 0;
        ...
        point = pow((double)10, (double)(-1 * (rsp[pos + 2] & 0x3)) );
...
//	printf("\nSNOOP: measured channel is %4.6lf\n", meas_freq/1000000);
...
double spa_calc_obwl(double *trace, const double tot) {
    ...
    for (m = 0; m < 200; m++) {
        ...
        if (sum >= tot) {
            ...
            return m + mInst_interp(tot,sum_last,sum);
        }
    }
...
double spa_calc_obwu(double *trace, const double tot) {
    ...
    for (k = 0; k < n; k++) {
        ...
        if (sum >= tot) {
            ...
            return k + mInst_interp(tot,sum_last,sum);
        }
    }
...
double mInst_interp(const double y, const double y1, const double y2) {
...
MANLIB_API char *spaMeasACP(const int ud,
    ...
    int n, rsp;
    double mainc, upper20, lower20, upper40, lower40;
    ...
    gpibWrite(ud, ":ACP:CSP 20E6;");                      // spacing
    gpibWrite(ud, ":BAND:RES 3E5;VID 3E5;");              // resolution bandwidth
    mainc = atof(gpibQuery(ud, ":READ:ACP:MAIN?", 55L));   // measure the main channel power
    ...
    n = sprintf(emissions, "%07.2f, %07.2f, %07.2f, %07.2f, %07.2f", mainc, upper20, lower20, upper40, lower40);
    emissions[n] = '\0';
...
int spa_plot_mask(char *gp_script, char *gp_data_file, char *gp_output_file, char *gp_label) {
    ...
    fprintf(fp, "%s", qq("set output '%s'\n", gp_output_file));
    fprintf(fp, "%s", qq("set label '%s'\n", gp_label));
    fprintf(fp, "%s", qq("plot '%s' using 1:2 w l, '%s' using 1:3 w l\n", gp_data_file, gp_data_file));
