...
//    dk_quiet((A_UINT16)(debugMode ? 0 : 1));
    dk_quiet((A_UINT16)(debugMode ? 1 : 1));
    ...
    )
{
    ...
    if (globDrvInfo.pDevInfoArray[devIndex]) {
      ...
      pdevInfo->hDevice = -1;
#endif
      pdevInfo->cd_endianness = ART_ENDIAN_UNKNOWN;
...
)
{
	...
	if (A_OK == checkRegSpace(pdevInfo, address))
	{
		A_UINT32 real = address - pdevInfo->pdkInfo->aregPhyAddr[pdevInfo->pdkInfo->bar_select];
#ifdef SIM
		...
		retval = (*(volatile int *)pMem);

		/* Byte-swap PCI clock-domain registers */
		if (0x4000 <= real && real < 0x5000 &&
		    pdevInfo->cd_endianness == ART_ENDIAN_BIG)
			retval = swap_l(retval);
