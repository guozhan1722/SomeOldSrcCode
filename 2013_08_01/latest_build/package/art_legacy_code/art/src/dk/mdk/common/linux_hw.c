...
A_STATUS set_adf_freq(A_UINT32 devNum, A_UINT64 *Adfreq) {
    MDK_WLAN_DEV_INFO    *pdevInfo = globDrvInfo.pDevInfoArray[devNum];

    printf("linux_hw:set_adf_freq: Adfreq=0x%llx\n", *Adfreq);
    if (ioctl(pdevInfo->hDevice, DK_IOCTL_ADF_SET_FREQ, Adfreq) < 0) {
        uiPrintf("Error: set_adf_freq ioctl failed !\n");
        return(A_ERROR);
    }
}

A_STATUS get_adf_freq(A_UINT32 devNum, A_UINT64 *Adfreq) {
    MDK_WLAN_DEV_INFO    *pdevInfo = globDrvInfo.pDevInfoArray[devNum];
    
    printf("linux_hw:get_adf_freq: Adfreq=0x%llx\n", *Adfreq);
    if (ioctl(pdevInfo->hDevice, DK_IOCTL_ADF_GET_FREQ, Adfreq) < 0) {
        uiPrintf("Error: get_adf_freq ioctl failed !\n");
        return(A_ERROR);
    }
}
...
A_STATUS get_device_client_info(MDK_WLAN_DEV_INFO *pdevInfo, PDRV_VERSION_INFO pDrvVer, PCLI_INFO cliInfo) {
   ...
   if (pdevInfo->pdkInfo->devIndex > (WLAN_MAX_DEV-1)) {
    ...
    uiPrintf("Daniel: device_fn=%d, devIndex=%d\n", pdevInfo->pdkInfo->device_fn, pdevInfo->pdkInfo->devIndex);
	...
	if (ioctl(hDevice,DK_IOCTL_GET_CLIENT_INFO,&linux_cliInfo) < 0) {
		...
		q_uiPrintf("rpa=%x:rr=%x:nB=%d:mpa=%x:ms=%x:irq=%x:\n", linux_cliInfo.reg_phy_addr, linux_cliInfo.reg_range, linux_cliInfo.numBars, linux_cliInfo.mem_phy_addr, linux_cliInfo.mem_size, linux_cliInfo.irq);
    if (pDrvVer->minorVersion >= 2) {
      for(iIndex=0;iIndex<linux_cliInfo.numBars;iIndex++) {
         ...
         cliInfo->aregVirAddr[iIndex] = (A_UINT32) vir_addr;
         ...
         cliInfo->aregVirAddr[0] = (A_UINT32) vir_addr;
    ...
    cliInfo->memVirAddr = (A_UINT32) mem_vir_addr;
