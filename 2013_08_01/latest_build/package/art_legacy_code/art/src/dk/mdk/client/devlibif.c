...
void m_setRegisterPatch(int devNum, int patch_no)
{
	setRegisterPatch(devNum, patch_no);
}

void m_setSpurFrequency(int devNum, A_UINT32 *_5g, A_UINT32 *_2p4g)
{
	setSpurFrequency(devNum, _5g, _2p4g);
}
...
{
	MDK_WLAN_DEV_INFO *pdevInfo = globDrvInfo.pDevInfoArray[getDevIndex(devNum)];
	pdevInfo->cd_endianness = endian;
#ifdef LINUX
	if (ioctl(pdevInfo->hDevice, DK_IOCTL_CD_ENDIANNESS, &endian) < 0) {
		uiPrintf("Error: cd_endianness ioctl failed\n");
	}
#endif
}
...
A_INT32 initializeMLIB(MDK_WLAN_DEV_INFO *pdevInfo)
{
	...
	devMap.SetCDEndian = SetCDEndian;
	devMap.GetCDEndian = GetCDEndian;
