...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5211/mCfg211.c#8 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5211/mCfg211.c#8 $"
#endif
...
#if !defined(SPIRIT_AP) && !defined(FREEDOM_AP)
#define F2_PCICFG 0x4010
	/* Detect PCI Clock Domain Endianness by forcing a sleep, waiting until
	 * the radio sleeps, and seeing which sleep bit goes high. */
	if (ART_GET_CD_ENDIAN(devNum) == ART_ENDIAN_UNKNOWN) {
		for(i = 0; i < timeout; i++ ) {
			unsigned rd;
			REGW(devNum, F2_SCR, 0x003d3d00); /* endian-agnostic force-sleep */
			rd = REGR(devNum, F2_PCICFG);
			if ((rd & F2_PCICFG_SPWR_DN) || (rd & swap_l(F2_PCICFG_SPWR_DN))) {
				ART_SET_CD_ENDIAN(devNum, (rd & F2_PCICFG_SPWR_DN) ?
					ART_ENDIAN_LITTLE : ART_ENDIAN_BIG);
				break;
			}
			mSleep(1);
		}
		if (i == timeout)
			mError(devNum, EIO, "hwResetAr5211: Device did not sleep.\n");
	}
    ...
    if (resetMask & BUS_RESET) {
		if(((pLibDev->macRev < 0x90) || (pLibDev->macRev > 0x9f)) && (pLibDev->macRev != 0) && ((pLibDev->macRev < 0xe0) || (pLibDev->macRev > 0xef)) && ((pLibDev->macRev < 0xa0) || (pLibDev->macRev > 0xa3))){
			//ie its not condor                                                               
			if (!isNala(pLibDev->swDevID))
			{
//   				printf("SNOOP: doing a PCI reset, macRev = %x: BAD FOR CONDOR/SWAN!!!!!!!!!!!!!!!1\n", pLibDev->macRev);
			}
			reg = reg | F2_RC_PCI; 
		}
		if((pLibDev->macRev >= 0xa0) && (pLibDev->macRev <= 0xa3)) {
			//double check for being eagle
			if(value & 0x18) { //check for autosizing in 0x4010
	   			if (!isNala(pLibDev->swDevID))
				{
//					printf("SNOOP: doing a PCI reset, macRev = %x: BAD FOR CONDOR/SWAN!!!!!!!!!!!!!!!1\n", pLibDev->macRev);
				}
...
//	mSleep(10);
	...
	/* Auto-detect PCI register byte-swapping */
	i = REGR(devNum, F2_SREV) & F2_SREV_ID_M;
	REGW(devNum, F2_CFG, 0);
	REGW(devNum, F2_Q0_TXDP, 0x0055aa00);
	REGW(devNum, F2_CFG, F2_CFG_SWRG);
	if ((REGR(devNum, F2_Q0_TXDP) & 0x00ffff00) != 0x0055aa00) {
		/* PCI HC already swaps... don't swap again */
		REGW(devNum, F2_CFG, 0);
	} else {
		REGW(devNum, F2_CFG, swap_l(F2_CFG_SWRG));
	}
#else
