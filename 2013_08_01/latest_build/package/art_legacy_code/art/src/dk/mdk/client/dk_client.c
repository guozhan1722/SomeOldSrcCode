...
void connectThread (void *pparam)
{
    ...
    //uiPrintf("connectThread starting up!\n");
	semLock(sem);
#ifndef MALLOC_ABSENT
...
)
{
	...
	if( pCmd->cmdLen != sizeof(pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD)+sizeof(pCmd->cmdID) + sizeof(pCmd->devNum) )
	{
	/* check that the size is appropriate before we access */
		...
		extra_cmd = false;
			...
			if (extra_cmd)
			{
	     		pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.REGPATCH.patch_no = ltob_l(pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.REGPATCH.patch_no);
				spur = (A_UINT32 *)&pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.SPUR;
				count = sizeof(pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.SPUR)/sizeof(A_UINT32);
				while (count--)
				{
					*spur = ltob_l(*spur);
					spur++;
	...
	memcpy(bss->pData,pCmd->CMD_U.RESET_DEVICE_CMD.bss,bss->Length);
		...
		if (extra_cmd)
		{
			m_setRegisterPatch(pCmd->devNum, pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.REGPATCH.patch_no);
			m_setSpurFrequency(pCmd->devNum, pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.SPUR._5g, pCmd->CMD_U.RESET_DEVICE_EXTRA_CMD.extra.SPUR._2p4g);
		}
...
A_UINT32 processRxDataSniffCmd
(
	struct dkThreadInfo *dkThread
)
{
...
)
{
	...
	/* check that the size is appropriate before we access */
     	if( pCmd->cmdLen != sizeof(pCmd->CMD_U.FORCE_SINGLE_PCDAC_TABLE_CMD)+sizeof(pCmd->cmdID) +sizeof(pCmd->devNum)) 
        {
...
/**************************************************************************
* processEnableDecompCmd - Process M_RX_ENABLE_DECOMP_CMD_ID command
*
* RETURNS: 0 processed OK, an Error code if it is not
*/
A_UINT32 processEnableDecompCmd
(
	struct dkThreadInfo *dkThread
)
{	
    ...
    /* check that the size is appropriate before we access */
    if( pCmd->cmdLen != sizeof(pCmd->CMD_U.ENABLE_DECOMP_CMD)+sizeof(pCmd->cmdID)+sizeof(pCmd->devNum) )
        {
        uiPrintf("Error, command Len is not equal to size of ENABLE_DECOMP_CMD\n");
        return(COMMS_ERR_BAD_LENGTH);
        }
...
#ifdef ENDIAN_SWAP
{
	...
	pCmd->CMD_U.ENABLE_DECOMP_CMD.keyCacheIndex = ltob_l(pCmd->CMD_U.ENABLE_DECOMP_CMD.keyCacheIndex);		
}
#endif

    
    /* call the function */
    rxEnableDecompression (pCmd->devNum, 
					pCmd->CMD_U.ENABLE_DECOMP_CMD.macAddress, 
					pCmd->CMD_U.ENABLE_DECOMP_CMD.keyCacheIndex);
	
#else
	uiPrintf("EnableDecompressionCmd command not implemented\n");
#endif //__ATH_DJGPPDOS__
    return(0);
}
...
//	pCmd->CMD_U.SET_LIB_CONFIG_CMD.libParams.applyCtlLimit  = ltob_l(pCmd->CMD_U.SET_LIB_CONFIG_CMD.libParams.applyCtlLimit);
	...
	configureLibParams(pCmd->devNum, &(pCmd->CMD_U.SET_LIB_CONFIG_CMD.libParams));
...
/**************************************************************************
* processSetAdfreqCmd - Process SET_ADF4350_FREQ_CMD command
*
* RETURNS: A_INT32 maxlinPwr*2
*/
A_INT32 processSetAdfreqCmd
(
	struct dkThreadInfo *dkThread
)
{
    ...
    /* check that the size is appropriate before we access */
    if( pCmd->cmdLen != sizeof(pCmd->CMD_U.ADFREQ_CMD)+sizeof(pCmd->cmdID) +sizeof(pCmd->devNum))
    {
        uiPrintf("Error, command Len is not equal to size of SET_ADF4350_FREQ_CMD\n");
        return(COMMS_ERR_BAD_LENGTH);
    }
    uiPrintf("processSetAdfreqCmd: Adfreq=0x%llx.\n", pCmd->CMD_U.ADFREQ_CMD.Adfreq);
    set_adf_freq(pCmd->devNum, &(pCmd->CMD_U.ADFREQ_CMD.Adfreq)) ;
    return 0;   
}

/**************************************************************************
* processSetAdfreqCmd - Process GET_ADF4350_FREQ_CMD command
*
* RETURNS: A_INT32 maxlinPwr*2
*/
A_INT32 processGetAdfreqCmd
(
	struct dkThreadInfo *dkThread
)
{
    ...
    if( pCmd->cmdLen != sizeof(pCmd->cmdID) +sizeof(pCmd->devNum))
    {
        uiPrintf("Error, command Len is not equal to size of GET_ADF4350_FREQ_CMD\n");
        return(COMMS_ERR_BAD_LENGTH);
    }
    get_adf_freq(pCmd->devNum, &(pCmd->CMD_U.ADFREQ_CMD.Adfreq));
    uiPrintf("processGetAdfreqCmd: Adfreq=0x%llx.\n", &(pCmd->CMD_U.ADFREQ_CMD.Adfreq));
    return 0;
}
...
)
{
...
#if !defined(THIN_CLIENT_BUILD)
	case M_RX_ENABLE_DECOMP_CMD_ID:
		status = processEnableDecompCmd(dkThread);
		sizeReplyBuf = 0;
		break;
#endif
	...
	case RX_DATA_SNIFF_CMD_ID:
		status = processRxDataSniffCmd(dkThread);
		sizeReplyBuf = 0;
		break;
    ...
    case SET_ADF4350_FREQ_CMD_ID:
        status = processSetAdfreqCmd(dkThread);
        sizeReplyBuf = 0;
        break;

    case GET_ADF4350_FREQ_CMD_ID:
        status = processGetAdfreqCmd(dkThread);
        sizeReplyBuf = 8;
        break;
