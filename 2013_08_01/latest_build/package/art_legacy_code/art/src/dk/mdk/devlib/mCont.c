...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mCont.c#1 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mCont.c#1 $"
#endif
...
//		resetDevice(devNum, pLibDev->macAddr.octets, pLibDev->bssAddr.octets, pLibDev->freqForResetDevice, pLibDev->turbo);
	...
	if(type == CONT_SINGLE_CARRIER) {
		...
		/* Set up ant. ctl. to be TX on all paths - this way, we don't
		 * have to switch antennas, and the full output comes out the
		 * desired port. */
#define setup_switch(ant) do { \
	value = getFieldForMode(devNum, "bb_switch_table_t" ant, pLibDev->mode, pLibDev->turbo); \
	writeField(devNum, "bb_switch_table_r" ant, value); \
	writeField(devNum, "bb_switch_table_r" ant "x1", value); \
	writeField(devNum, "bb_switch_table_r" ant "x2", value); \
	writeField(devNum, "bb_switch_table_r" ant "x12", value); \
} while (0)
		setup_switch("1");
		setup_switch("2");
#endif
