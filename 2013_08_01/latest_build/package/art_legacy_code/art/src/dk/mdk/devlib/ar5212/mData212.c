...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5212/mData212.c#3 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5212/mData212.c#3 $"
#endif
...
)
{
	...
	A_BOOL      xrRate = 0;
	A_UINT32    i;
	A_UINT32    compressionFlags = DESC_COMPRESSION_DISABLE;
	...
	if(pLibDev->libCfgParams.enableCompression) {
		compressionFlags = 0x0;  //compression enable always
	}

	localVeniceDescPtr->hwControl[1] = pktSize | (broadcast ? (1 << BITS_TO_VENICE_NOACK) : 0) |
									( compressionFlags << BITS_TO_COMPRESSION); //Disable compression for now
...
)
{
	...
	*/
	...
	localVeniceDescPtr->hwControl[1] = pktSize | 
								   (0x1 << 24) |     //noack. No longer waits for an ack. Fixes a problem where
								                     // contTx never ends, but might cause tx underruns on slow pci's.
