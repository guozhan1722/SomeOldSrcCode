...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mEeprom.c#8 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/mEeprom.c#8 $"
#endif
...
//printf("Snoop: regOffset = %x, regValue = %x\n", regOffset, temp32);
	...
	//config files.
//printf("gb0 = %d, gb1 = %d, gb2 = %d, gb3 = %d\n", pGainBoundaries[0],
//	   pGainBoundaries[1], pGainBoundaries[2], pGainBoundaries[3]);
...
)
{
	...
	if(((pHeaderInfo->majorVersion == 5) && (pHeaderInfo->minorVersion >= 3)) ||
		...
		pHeaderInfo->enableJapanOddU1 = (A_UINT16)((tempValue >> 10) & 0x1);
	...
	if(((pHeaderInfo->majorVersion == 4) && (pHeaderInfo->minorVersion >= 4)) ||
		...
		if((pHeaderInfo->majorVersion == 5) && (pHeaderInfo->minorVersion >= 4)) {
			pHeaderInfo->artBuildNumber = (A_UINT16)(tempValue & 0xff);
				pHeaderInfo->earFileIdentifier = 0;
		}
		else {
			//look for build number shift with max out problem
			if((tempValue >> 8) & 0x1) { //build num is in ear location
				pHeaderInfo->artBuildNumber = (A_UINT16)(tempValue & 0xff);
				pHeaderInfo->earFileIdentifier = 0;
			} else {
			...
			if((tempValue >> 9) & 0x1) { //build num should be displayed as hex
				//set highest bit of build number to indicate display as hex
				pHeaderInfo->artBuildNumber |= (0x1 << 15); 
			}
		}
...
*/
	...
	if (!isGriffin(pLibDev->swDevID)) { // exclude griffin
		if((mode != MODE_11A)&&
			...
			if(!isSwan(pLibDev->swDevID))  //swan doesn't have b_db/b_ob
			{
	...
	else if (!isGriffin(pLibDev->swDevID) && !isEagle(pLibDev->swDevID)){
...
//		setAntennaTbl5211(devNum, pModeInfo->antennaControl);
	...
	if(pHeaderInfo->majorVersion >= 4) {
		...
		if((((pLibDev->swDevID & 0xff) == 0x14) || ((pLibDev->swDevID & 0xff) >= 0x16)) &&
			(!isGriffin(pLibDev->swDevID)) && !isEagle(pLibDev->swDevID)){
...
)
{
    ...
    if (pLibDev->devMap.remoteLib && (pLibDev->devMap.r_eepromReadBlock != NULL)) {
	  ...
	  for  (j=0;j<length;j++)	{
//..Siva..		eepromValue[j] = ar5kInitData[i].pMacAPI->eepromRead(devNum, startOffset + j);
...
static A_UINT16        xpdGainValues[MAX_NUM_PDGAINS_PER_CHANNEL];
...
)
{
	...
	if (pLibDev->eePromLoad) {
		if((pLibDev->p16kEepHeader->majorVersion >= 4) && (pLibDev->p16kEepHeader->eepMap == 1)){
			switch(mode) {
			case MODE_11A:
				if (!pLibDev->p16kEepHeader->Amode && !pLibDev->p16kEepHeader->enableJapanMode11aNew) {
					return;
				}
			...
			get_xpd_gain_and_pcdacs_for_powers(devNum, (A_UINT16)pLibDev->freqForResetDevice, pLibDev->pGen3RawData[mode], 
								numPdGain, pModeInfo->xgain, &minPower, &maxPower, &midPower, xpdGainValues, powerValues);
	...
	if((pLibDev->eePromLoad && pLibDev->eepData.infoValid) &&
		(pLibDev->p16kEepHeader->majorVersion >= 5) && (pLibDev->p16kEepHeader->eepMap == 2)) {
			...
			get_gain_boundaries_and_pdadcs_for_powers(devNum, (A_UINT16)pLibDev->freqForResetDevice, pLibDev->pGen5RawData[mode], 
								(A_UINT16)pdGainOverlap_t2, &(minCalPowerGen5_t2), gainBoundaries, xpdGainValues, pdadcValues);
...
//			if(isGriffin(pLibDev->swDevID)) {
			if(pLibDev->p16kEepHeader->eepMap >= 2) {
				...
				if (numPdGainsUsed >= 1) {
					//writeField(devNum, "bb_pd_gain_setting1", xpdGainValues[0]);
					regValue = (regValue & BB_PD_GAIN_SETTING1_CLEAR) | (xpdGainValues[0] << BB_PD_GAIN_SETTING1_SHIFT);
				}
				if (numPdGainsUsed >= 2) {
					//writeField(devNum, "bb_pd_gain_setting2", xpdGainValues[1]);
					regValue = (regValue & BB_PD_GAIN_SETTING2_CLEAR) | (xpdGainValues[1] << BB_PD_GAIN_SETTING2_SHIFT);
				}
				if (numPdGainsUsed >= 3) {
					//writeField(devNum, "bb_pd_gain_setting2", xpdGainValues[1]);
					regValue = (regValue & BB_PD_GAIN_SETTING3_CLEAR) | (xpdGainValues[2] << BB_PD_GAIN_SETTING3_SHIFT);
				}
			...
			if(minCalPowerGen5_t2 != 0) {
				...
				for(i = 0; i < NUM_RATES; i++) {
					pRatesPower[i] = (A_INT16)(pRatesPower[i] + pwr_index_offset);
					if (pRatesPower[i] > 63) pRatesPower[i] = 63;
                    //if (pRatesPower[i] == 0) pRatesPower[i] = 1;
				} 
			} else {
				pLibDev->pwrIndexOffset = 0;
			}		
	...
	if ( ((pLibDev->swDevID & 0xff) >= 0x0013) &&  // do it for all post-venice boards
		 (!isGriffin(pLibDev->swDevID) && !isEagle(pLibDev->swDevID)) &&         // except for griffin & eagle
		 (pLibDev->mode == MODE_11G) && (pLibDev->eePromLoad)) {
			...
			forcePowerTxMaxVenice(devNum, &(pRatesPower[0]), &(powerValues[0]), cck_ofdm_correction);
			return;
	}
...
/***************************************************************************
*  forcePowerTxMaxVenice  -  corrects for the gain-delta between ofdm and cck 
...
//	pdGainOverlap_t2 = (A_UINT16)getFieldForMode(devNum, "bb_pd_gain_overlap", mode, pLibDev->turbo);
	...
	retEAR[startOffset++] = ((force_pd_gain & 0x1) << 15) |
							...
							((en_pd_cal & 0x1) << 8) |
							((0 & 0x3) << 6) |
		                    ((xpdGainValues[1] & 0x3) << 4) | 
							((xpdGainValues[0] & 0x3) << 2) | 
