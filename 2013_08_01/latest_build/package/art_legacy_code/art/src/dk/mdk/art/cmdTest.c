...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devmld/cmdTest.c#5 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devmld/cmdTest.c#5 $"
#endif
...
static A_UINT32 RSSITest(A_UINT32 devNum, A_UINT32 testChannel, A_UINT32 testMode, A_UINT32 turbo, A_UINT16 antenna);
static A_UINT32 GetMacAdress(A_UINT32 devNum, A_CHAR * file_name);
	...
	for (i = 0; i < configSetup.numListChannels; i++) {
		...
		for(k = 0; k < numGoldAntenna; k++) {
			...
			for(j = 0; j < numAntenna; j++) {
				...
				if(configSetup.cmdLineTestMask & SSID_TEST_MASK) {
					uiPrintf("\n\n===========================================================================\n");
					uiPrintf("Performing SSID test on DUT Antenna %c, golden Antenna %s at Channel %d in mode %s %s\n",
						(configSetup.antenna==(USE_DESC_ANT|DESC_ANT_A)) ? 'A' : 'B',
						goldAntString,
						configSetup.testChannelList[i].channel, modeStr[configSetup.testChannelList[i].mode],
						(configSetup.testChannelList[i].turbo == TURBO_ENABLE) ? "turbo" : "" );
					globalTestMask |= RSSITest(devNum, configSetup.testChannelList[i].channel, 
						configSetup.testChannelList[i].mode, configSetup.testChannelList[i].turbo, antennaSelected);
					Sleep(1000);
				
				}
                if(configSetup.cmdLineTestMask & GET_MAC_ADDR_MASK) {
					uiPrintf("\n\n===========================================================================\n");
					uiPrintf("Writing MAC address into the file %s\n",configSetup.SSID);
					globalTestMask |= GetMacAdress(devNum, configSetup.macAddrFileName);
					Sleep(1000);
...
)
{
	...
	for(i = 0; i < testSetup.numIterations; i++) {
		...
		art_setAntenna(devNum, configSetup.antenna);
		rxDataSetupFixedNumber(devNum, testSetup.numPackets, testSetup.packetSize, enablePPM);
		checkBeaconRSSI(devNum, testSetup.beaconTimeout, 0, 0, pattern, 2,configSetup.SSID,rssiThreshold,&result);
		...
		if (!result) {
			 reasonFailMask = MASK_SSID_ANTA << antennaMaskOffset;
		     uiPrintf("\nSSID Test Failed. Error code %d \n", ERR_SSID_ANTA + antennaMaskOffset);
		}
	    else {
		     uiPrintf("\nSSID Test Passed, return code 0\n");
		}
		//art_rxGetStats(devNum, 0, 0, &rStats);
...
//Get Mac adress: Stores the mac address into a file 
 A_UINT32 GetMacAdress
(
 A_UINT32 devNum,
 A_CHAR * file_name
 )
{
     A_INT32 error;
	 SaveMacAddress(devNum,file_name,&error);
     return error;
}
...
//	uiPrintf("Good Packets        Throughput        RSSI    Result  Rate\n");
	...
	Sleep(3000);	// delay between packet sends
	if(testMode == MODE_11B){
...
{
	if(!strcmp(configSetup.rangeLogFile, "")) {
...
A_BOOL parseTestFile(void) 
{
    ...
    while(fgets(lineBuf, 120, fStream) != NULL) {
		...
		if(*pLine == '\0') {
            ...
            if(!sscanf(pLine, "%d", &testSetup.numIterations)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.numPackets)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.packetSize)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.packetSizeTP)) {
...
//			}
            ...
            if(!sscanf(pLine, "%d", &testSetup.numPacketsTP)) {
...
//			}
            ...
            if(!sscanf(pLine, "%d", &testSetup.numPacketsTP_CCK)) {
...
//			}
            ...
            if(!sscanf(pLine, "%d", &testSetup.sideChannel5G)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.sideChannel2G)) {
            ...
            if(!sscanf(pLine, "%x", &testSetup.eepromCompareSingleLocation)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.perPassThreshold)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.perTPThreshold11b)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.perTPThreshold)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.ppmMin)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.ppmMax)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11a_antA)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11b_antA)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11a_antB)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11b_antB)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11g_antA)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.rssiThreshold11g_antB)) {
			...
			if(!(configSetup.cmdLineTestMask & GOLDEN_TEST_MASK)) {
				...
				if(!sscanf(pLine, "%hd", &configSetup.powerOutput)) {
					uiPrintf("Unable to read the PER_THRESHOLD from %s\n", TESTSETUP_FILE);
					return 0;
				}
            ...
            if(!sscanf(pLine, "%d", &testSetup.beaconTimeout)) {
            ...
            if(!sscanf(pLine, "%d", &testSetup.maxCRCAllowed)) {
