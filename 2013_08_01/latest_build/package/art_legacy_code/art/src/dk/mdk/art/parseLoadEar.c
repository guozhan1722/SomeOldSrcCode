			...
			case REGISTER_WRITES_TYPE: // register data
				switch(temp_reg_cfg->reg_hdr.field0.pack0.type) {
					case WRITE_TYPE0:
						...
						token = (char *) strtok(NULL, delimiters);
						ASSERT(token != NULL);
						sscanf(token, "%hx", &temp_reg_write_type0->msw);

						if (temp_reg_write_type0->field0.pack.tag == 0 || temp_reg_write_type0->field0.pack.tag == 3) {
						    token = (char *) strtok(NULL, delimiters);
						    ASSERT(token != NULL);
						    sscanf(token, "%hx", &temp_reg_write_type0->lsw);
						}
						break;
					case WRITE_TYPE1:
						...
						for(iIndex=0; iIndex<=temp_reg_write_type1->field0.pack.num; iIndex++) {
						   ...
						   token = (char *) strtok(NULL, delimiters);
						   ASSERT(token != NULL);
						   sscanf(token, "%hx", &temp_type1_value);
...
//FJC						printf("header value = %x\n", temp_reg_write_type3->field0.value);
						...
						token = (char *) strtok(NULL, delimiters);
						ASSERT(token != NULL);
						sscanf(token, "%hx", &temp_reg_write_type3->address);

						token = (char *) strtok(NULL, delimiters);
						ASSERT(token != NULL);
						sscanf(token, "%hx", &temp_reg_write_type3->data_msw);

						if (temp_reg_write_type3->field0.pack.num_bits > 16) {
						    token = (char *) strtok(NULL, delimiters);
						    ASSERT(token != NULL);
						    sscanf(token, "%hx", &temp_reg_write_type3->data_lsw);
						}
			...
			case VERSION_MASK_TYPE: // version mask
				token = (char *) strtok(NULL, delimiters);
				sscanf(token, "%hx", &version_mask);
				version_mask |= (1<<15);
				version_mask_given = 1;
				break;
			case VERSION_ID_TYPE: // Versiod id
				token = (char *) strtok(NULL, delimiters);
				sscanf(token, "%hx", &(*ear_cfg)->version_id);
				break;
		}
	}
