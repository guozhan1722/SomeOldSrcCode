...
static A_UINT32 LO_SEN_TEST_RATE_11G_CCK[] = {0xb1};
...
#else
static A_UINT32		timestart, globalteststarttime;
#endif
...
void programMACIDFromLabel(A_UINT32 devNum)
{
	...
	if (configSetup.remote && art_dut_is_ap(CalSetup.subsystemID))
	{
...
A_UINT16 parseTargets(void)
{
    ...
    while(fgets(lineBuf, 120, fStream) != NULL) {
        ...
        while(parsingTP && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			if (!sscanf(pLine, "%hd", &(pTargetsSet->pTargetChannel[ii].channelValue))) {
        ...
        while(parsingTP_11g && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			if (!sscanf(pLine, "%hd", &(pTargetsSet_2p4[MODE_11g]->pTargetChannel[ii].channelValue))) {
				uiPrintf("Could not read the test frequency %d from token %s\n", ii, pLine);
			}
		...
		while(parsingTP_11b && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			if (!sscanf(pLine, "%hd", &(pTargetsSet_2p4[MODE_11b]->pTargetChannel[ii].channelValue))) {
				uiPrintf("Could not read the test frequency %d from token %s\n", ii, pLine);
			}
        ...
        while( parsingTG && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			if (parsingLine1 > 0) {
				...
				{
					if (!sscanf(pLine, "%hd", &(pTestGroupSet->pTestGroup[jj].BandEdge[ii++]))) {
			...
			if (parsingLine3 > 0)
			{
				...
				while ( (pLine != NULL) && (ii < pTestGroupSet->pTestGroup[jj].numBandEdgesDefined))
				{
					if (!sscanf(pLine, "%hd", &(pTestGroupSet->pTestGroup[jj].inBandFlag[ii++]))) {
						uiPrintf("Could not read the inband flag %d for CTL %d from token %s\n", ii-1, jj, pLine);
					}
					pLine = strtok(NULL, delimiters);
				}	

				if (!( (pLine == NULL) || (pLine[0] == '#') ) && sscanf(pLine, "%hd", &(parsingLine3)) )
				{
...
*/	
	...
	if(CalSetup.testRxThroughput[mode])
	{
		...
		dutThroughputRxTest(devNum, mode, 0, throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
		if (REWIND_TEST) {
			return;
		}
...
void goldenTest_2p4(A_UINT32 devNum, A_UINT32 mode)
{
	...
	if(CalSetup.testThroughput[mode])
	{
		...
		goldenThroughputTest(devNum, mode, 0, throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
...
*/
	...
	if(CalSetup.testRxThroughput[mode])
	{
		...
		goldenThroughputRxTest(devNum, mode, 0, throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
	}
	if(kbhit())
		return;;
	...
	for(index=0; index<ChannelCount; index++) {
        ...
        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
            ...
            art_writeField(devNum, "rf_mixvga_ovr", 0);
                ...
                target = 0;
                art_forceSinglePowerTxMax(devNum, CalSetup.minimumPower[mode]);
                rate = 0;
	        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
...
//uiPrintf("SNOOP: using atten value = %5.2f\n", attenVal); 
            ...
            if(zeroPowerTest) {
                Sleep(CalSetup.targetPowerZeroDelay);
            }
			reset = 0; //1;
...
//					if (!testPERLocal && !pTestSet[mode]->pTestChannel[index].TEST_PER)
					...
					{
                        if(do_11g_CCK) {
                            senRates[kk] = LO_SEN_TEST_RATE_11G_CCK[0];    
                        } else {
...
A_BOOL prepare_for_next_card (A_UINT32 *p_devNum)
{
	...
	if(configSetup.enableLabelScheme) {
		...
		{
	devNumInst[1] = art_setupDevice(configSetup.instance);
	if(devNumInst[1] < 0) {
	...
	devNumInst[2] = devNumInst[1];
	uiPrintf("Attached to the Device for instance = %d\n", configSetup.instance);

	devNumArr[MODE_11a] = devNumArr[MODE_11g] = devNumArr[MODE_11b] = devNumInst[1];
		} else
		{
	if(CalSetup.modeMaskForRadio[0] != 0) {
...
//			testThisChannel = pTestSet[mode]->pTestChannel[index].TEST_TURBO_THROUGHPUT;
		...
		art_forceSinglePowerTxMax(devNum, goldenTXPowerLocal);
...
static A_BOOL dutTestContinuity(A_UINT32 devNum)
{
	...
	configureGPIOOutput(devNum, CalSetup.continuityOutputGpio);
	configureGPIOInput(devNum, CalSetup.continuityInputGpio);

	//write 1 to output GPIO ...
	driveGPIO(devNum, CalSetup.continuityOutputGpio, 1);
	
	//...and verify input GPIO sees it
	gpioResult = readGPIO(devNum, CalSetup.continuityInputGpio);
	...
	driveGPIO(devNum, CalSetup.continuityOutputGpio, 0);
...
void dutMeasCurrActive(A_UINT32 devNum, A_UINT32 mode)
{
	...
	A_UINT32  sleep_interval = 50;
	A_UINT32  antenna = DESC_ANT_A | USE_DESC_ANT;
	...
	if(CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[mode] ) // tx
	{
		...
		for (i = 0; i < numChannels; i++)
		{
			...
			{	
				...
				art_txContBegin(devNum, CONT_FRAMED_DATA, REPEATING_5A , yldLogRate, antenna);
				Sleep(sleep_interval);
				failedThisTest = TRUE;
				iter = 0;
				while (failedThisTest && (iter < CalSetup.maxRetestIters))
...
void goldenMeasCurrActive(A_UINT32 devNum, A_UINT32 mode, A_UINT32 numPackets, A_UINT32 frameLen)
{
	...
	for (i=0; i < numChannels; i++)
	{
		...
		{
			art_resetDevice(devNum, rxStation, bssID, channelTimes10[i], turbo);
			art_txDataSetup(devNum, rate, txStation, num_tx_desc, tx_length, pattern, 2, retry, antenna, broadcast);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST)
			{
				return;
			...
			art_txContBegin(devNum, CONT_FRAMED_DATA, REPEATING_5A, rate, antenna);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST)
			{
				return;
