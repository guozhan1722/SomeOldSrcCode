...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5211/mData211.c#5 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/ar5211/mData211.c#5 $"
#endif
...
)
{
	...
	localDescPtr->hwControl[0] = ( (rateValue << BITS_TO_TX_XMIT_RATE) | 0x01000000 |  //clear dest mask
...
)
{
	...
	else if (pLibDev->rx.rxMode == RX_PROMISCUOUS_MODE) {
		REGW(devNum, F2_RX_FILTER, F2_RX_PROM);
	}
	else {
...
)
{
	...
	//0x8048 bit 22 = mc_force_channel_idle_high, set for mac deaf mode
	//REGW(devNum, 0x8048, REGR(devNum, 0x8048 | 0x40000))
...
)
{
	...
	//REGW(devNum, 0x8048, REGR(devNum, 0x8048) & 0xffbfffff);
}
