...
#if 0
#ident  "ACI $Id: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/athreg.c#1 $, $Header: //depot/sw/branches/ART_V53/sw/src/dk/mdk/devlib/athreg.c#1 $"
#endif
		...
		if (modeSpecific) {
			...
			for(i = FIELD_NAME; (i <= BASE_11G_VALUE) && !fileError; i++) {
				...
				case FIELD_NAME:
					...
					memset(&modeFieldInfoIn, 0, sizeof(MODE_INFO));
					if(strlen(token) > MAX_NAME_LENGTH) {
						printf("field name on line %d is more than %d characters\n", 
										currentLineNumber, MAX_NAME_LENGTH);
						fileError = 1;
						break;
					}
...
)
{
	...
	while( x < pRfRegBank->numRegFields ) {
        ...
        numRegBits = 0;
        while ((x < pRfRegBank->numRegFields ) &&
				(pRfFields->rfRegNumber == currentRfReg )) {
            baseByteValue = 0;
            turboByteValue = 0;
            load = 0;
