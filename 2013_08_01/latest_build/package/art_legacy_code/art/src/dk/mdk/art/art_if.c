...
#ifndef __ATH_DJGPPDOS__
		GlobalCmd.CMD_U.RESET_DEVICE_EXTRA_CMD.extra.REGPATCH.patch_no = CalSetup.registerSpecificPatch;
...
#endif
		if (!artSendCmd(&GlobalCmd,
						sizeof(GlobalCmd.CMD_U.RESET_DEVICE_EXTRA_CMD)+sizeof(GlobalCmd.cmdID),
						NULL))
		{
		if (!artSendCmd(&GlobalCmd,
...
A_UINT32 art_SetAdf4350(A_UINT32  devNum, A_UINT64  Adfreq)
{
    // create cmd to send to client
	GlobalCmd.cmdID = SET_ADF4350_FREQ_CMD_ID;
	GlobalCmd.devNum = (A_INT8) devNum;
	GlobalCmd.CMD_U.ADFREQ_CMD.Adfreq	= FbytesSwap(Adfreq);
...
)
{
	...
	else 
	{
		...
		GlobalCmd.CMD_U.ENABLE_DECOMP_CMD.keyCacheIndex = keyCacheIndex;
    ...
    )					// NOTE: if command does not expect a return struct
{
    ...
    if (CalSetup.subsystemID == 0xa034 &&
        (pCmdStruct->cmdID == M_EEPROM_READ_CMD_ID ||
         pCmdStruct->cmdID == M_EEPROM_READ_BLOCK_CMD_ID))
        cmdSize -= 4;
