...
#pragma alloc_text(PAGE, AthUsb_DriverUnload)
#endif
#endif
    ...
    )
{
    ...
    registryPath = &Globals.AthUsb_RegistryPath;
    ...
    if (!registryPath->Buffer) {
        ...
        AthUsb_DbgPrint(1, ("Failed to allocate memory for registryPath\n"));
    ...
    //
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = AthUsb_DispatchDevCtrl;
    DriverObject->MajorFunction[IRP_MJ_POWER]          = AthUsb_DispatchPower;
    DriverObject->MajorFunction[IRP_MJ_PNP]            = AthUsb_DispatchPnP;
    DriverObject->MajorFunction[IRP_MJ_CREATE]         = AthUsb_DispatchCreate;
    DriverObject->MajorFunction[IRP_MJ_CLOSE]          = AthUsb_DispatchClose;
    DriverObject->MajorFunction[IRP_MJ_CLEANUP]        = AthUsb_DispatchClean;
    DriverObject->MajorFunction[IRP_MJ_READ]           =
    DriverObject->MajorFunction[IRP_MJ_WRITE]          = AthUsb_DispatchReadWrite;
    DriverObject->MajorFunction[IRP_MJ_SYSTEM_CONTROL] = AthUsb_DispatchSysCtrl;
    DriverObject->DriverUnload                         = AthUsb_DriverUnload;
    ...
    )
{
    ...
    AthUsb_DbgPrint(3, ("AthUsb_DriverUnload - begins\n"));
    ...
    AthUsb_DbgPrint(3, ("AthUsb_DriverUnload - ends\n"));
    ...
    )
{
    ...
    AthUsb_DbgPrint(3, ("AthUsb_AddDevice - begins\n"));
    ...
    if(!NT_SUCCESS(ntStatus)) {
        ...
        //                
        AthUsb_DbgPrint(1, ("Failed to create device object\n"));
        return ntStatus;
    }
    ...
    //
    ntStatus = AthUsb_WmiRegistration(deviceExtension);

    if(!NT_SUCCESS(ntStatus)) {

        AthUsb_DbgPrint(1, ("AthUsb_WmiRegistration failed with %X\n", ntStatus));
    ...
    CheckOSVersion(deviceExtension);    
    ...
    if(WinXpOrBetter == deviceExtension->WdmVersion) {
        ...
        AthUsb_GetRegistryDword(ATHUSB_REGISTRY_PARAMETERS_PATH,
                                 L"AthUsbEnable",
    ...
    //Initialize streaming DPC
    KeInitializeDpc(&deviceExtension->StreamDpc,StreamDpcProc,deviceObject);
    KeInitializeTimer(&deviceExtension->StreamTimer);
    KeInitializeEvent(&deviceExtension->StreamDpcEvent,NotificationEvent,TRUE);    
    ...
    AthUsb_DbgPrint(3, ("AthUsb_AddDevice - ends\n"));
