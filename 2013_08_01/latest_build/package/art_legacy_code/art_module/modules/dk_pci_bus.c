...
)
{
...
#ifdef DK_DEBUG
	printk(KERN_INFO "DK::Pci probe \n");
#endif
	error = pci_enable_device(dev); 
	if (error != 0) {
			printk(KERN_INFO "DK:: pci_enable_device failed \n");
			return error;
	}
   ...
   pci_write_config_dword(dev,0x10,0xffff);
	...
	printk(KERN_INFO "Class code = %d:start search index=%d\n", ret_val, sIndex);
...
{
#ifdef DK_DEBUG
	printk(KERN_INFO "DK::Pci remove \n");
#endif
	remove_client(dev);
#if LINUX_VERSION_CODE > 132098
	pci_disable_device(dev);
...
{
#ifdef DK_DEBUG
		printk(KERN_INFO "DK::Pci suspend \n");
#endif
#if LINUX_VERSION_CODE > 132098
		return 0;
#else
...
{
#ifdef DK_DEBUG
		printk(KERN_INFO "DK::Pci resume \n");
#endif
#if LINUX_VERSION_CODE > 132098
		return 0;
#else
...
#endif
    {ATHEROS_VENDOR_ID, PCI_ANY_ID, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"GRIFFIN_MAC"},
//	{ATHEROS_VENDOR_ID, 0xff16, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"GRIFFIN_MAC"},
	...
	{ATHEROS_VENDOR_ID, 0x002c, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"KITE1_1_PCIE"},
	{ATHEROS_VENDOR_ID, 0xff1d, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"OWL"},
#endif
	{ATHEROS_VENDOR_ID, 0xff1b, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"EAGLE"},
	{ATHEROS_VENDOR_ID, 0x001b, PCI_ANY_ID, PCI_ANY_ID, 0, 0, (unsigned long)"EAGLE"},
#if 0
...
#if 0 /* was removed in 2.6.23 since nothing ever called it */
...
) 
{
	int status;
#ifdef DK_DEBUG
	printk(KERN_INFO "DK::Bus module init  \n");	
#endif // DK_DEBUG
#ifdef MODULE
#ifdef DK_DEBUG
	printk(KERN_INFO "DK::MODULE\n");	
#endif // DK_DEBUG
...
#endif
	printk(KERN_INFO "DK::bus_module_init:status=%d from dkpci_driver\n", status);
#ifdef DK_UART
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,22)
	status |= pci_module_init(&dkuart_pci_driver);
#else
	status |= pci_register_driver(&dkuart_pci_driver);
#endif
	printk(KERN_INFO "DK::bus_module_init:status=%d from dkuart_pci_driver\n", status);
#endif
	return status;
#else
...
{
#ifdef DK_DEBUG
	printk(KERN_INFO "DK::Bus module exit  \n");	
#endif // DK_DEBUG
