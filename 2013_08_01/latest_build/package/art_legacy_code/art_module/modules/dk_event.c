...
#ifdef DK_DEBUG
	printk("DK::Copy Event:Event Ptr = %x \n",(UINT32)pNewEvent);
#endif
...
)
{
		...
		save_flags(pQueue->flags);
		cli();
#else
		local_irq_save(pQueue->flags);
#endif
}
...
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,27)
		restore_flags(pQueue->flags);
#else
		local_irq_restore(pQueue->flags);
#endif
}
