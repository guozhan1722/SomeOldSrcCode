...
static unsigned long long adfreq=0;
...
#ifdef DK_DEBUG
		printk(KERN_INFO "DK:: dk_open:minor=%d\n", minor);
#endif
		/* 
...
)
{
		int cli_id;
#ifdef DK_DEBUG
		printk(KERN_INFO "DK:: dk_release \n");
#endif
...
#ifdef DK_DEBUG
		printk(KERN_INFO "DK::dk_ioctl \n");
#endif

		cli_id = (int) file->private_data;
		p_client = get_client(cli_id);
		if (p_client == NULL) {
				printk(KERN_INFO "DK:: Invalid client \n");
				return -1;
		}
		switch (cmd) {
			case DK_IOCTL_GET_VERSION:
#ifdef DK_DEBUG
				printk(KERN_INFO "DK:: DK_IOCTL_GET_VERISION \n");
#endif
...
#ifdef DK_DEBUG
				printk(KERN_INFO "DK:: DK_IOCTL_GET_CLIENT_INFO \n");
#endif
				if (get_cli_info(cli_id,&ci) < 0) {
					printk(KERN_INFO "DK:: get_cli_info failed, cli_id : %d \n", cli_id);
					ret = -1;
				} else {
...
#ifdef DK_DEBUG
				printk(KERN_INFO "DK::Cfg read @ offset %x \n",co.offset);
#endif
...
#ifdef DK_DEBUG
                                printk(KERN_INFO "DK::Rtc reg read @ offset %x \n",co.offset);
#endif
//#ifdef AP83
...
//#endif
...
#ifdef DK_DEBUG
				printk(KERN_INFO "DK::Reading Chio ID @ offset %x \n",co.offset);
#endif
...
#ifdef DK_DEBUG
				printk(KERN_INFO "DK::Cfg write @ offset %x : %x \n",co.offset,co.value);
#endif
#if defined(OWL_PB42) || defined(PYTHON_EMU)
...
#ifdef DK_DEBUG
                                printk(KERN_INFO "DK::rtc write @ offset %x : %x \n",co.offset,co.value);
#endif
#ifdef AP83
...
#ifdef DK_DEBUG
				printk(KERN_INFO "DK::Create event \n");
#endif
						...
						switch (p_event->type ) {
							case ISR_INTERRUPT:
								...
								if (pushEvent(p_event, &p_client->isr_event_q,
									...
									printk(KERN_INFO "DK::Push Event Failed \n");
									kfree(p_event);
								} 
								break;
							default:
								printk(KERN_INFO "DK::Event Type %d not supported \n",p_event->type);
								kfree(p_event);
								break;
						}
					} 
			...
			case DK_IOCTL_GET_NEXT_EVENT:
#ifdef DK_DEBUG
				printk(KERN_INFO "DK::Get next event \n");
#endif
				ret = 0;
				eo.valid = 0;
				if (p_client->trigered_event_q.queueSize) {
					...
					#ifdef DK_DEBUG 
						printk(KERN_INFO "DK:: Pop event %x \n",(UINT32)p_event);
					#endif 
						kfree(p_event);
					} 
				}
...
#ifdef DK_DEBUG
                    printk(KERN_INFO "DK::Get DK_IOCTL_FLASH_READ \n");
#endif
                    if (copy_from_user((void *)&fop,(void *)arg,sizeof(fop))) {
                           printk(KERN_INFO "DK :: Copy_from_user failed\n");
                           return -EFAULT;
                    }
//printk("DK FLASH_READ:: fop.fcl is 0x%x, fop.offset is 0x%x, fop.len is 0x%x\n",fop.fcl, fop.offset, fop.len);
            ...
            case DK_IOCTL_FLASH_WRITE:
#ifdef DK_DEBUG
                 printk(KERN_INFO "DK::Get DK_IOCTL_FLASH_WRITE\n ");
#endif
                 ...
                 if(NULL==write_buf){
                     printk(KERN_INFO "DK::FWR  Unable to allocate memory Failed\n");
                     return -EFAULT;
                 }
...
#ifdef DK_DEBUG
                 printk(KERN_INFO "DK::FWR:addr=%x:\n",fop.offset);
#endif
                 ret=dk_flash_write(fopwr.fcl,fopwr.offset,fopwr.len,write_buf);
                 if(!ret){
                     kfree(write_buf);
                     printk(KERN_INFO "DK:: dk_flash_write Failed\n");
                     return -EFAULT;
                 }
                 kfree(write_buf);
#if 0
...
#endif
*/
        case DK_IOCTL_CD_ENDIANNESS:
            if (get_user(ret, (UINT32 *)arg)) {
                printk(KERN_INFO "DK:: get_user failed\n");
                return -EFAULT;
            }
            cli_set_cd_endianness(cli_id, ret);
            ret = 0;
            break;
        case DK_IOCTL_ADF_SET_FREQ:
            if (copy_from_user((void *)&adfreq,(void *)arg,sizeof(adfreq))) {
                      printk(KERN_INFO "DK::DK_IOCTL_ADF_SET_FREQ get_user failed\n");
                      return -EFAULT;
            }
            f = adfreq;
            printk(KERN_INFO "DK::DK_IOCTL_ADF_SET_FREQ=0x%llx\n", adfreq);	
            ret = ADF4350_SetParams(adfreq);			
			break;
        case DK_IOCTL_ADF_GET_FREQ:
            if (copy_to_user((void *)&adfreq,(void *)arg,sizeof(adfreq))) {
                printk(KERN_INFO "DK::DK_IOCTL_ADF_GET_FREQ get_user failed\n");
                return -EFAULT;
            }      
            printk(KERN_INFO "DK:: DK_IOCTL_ADF_GET_FREQ=0x%llx\n", adfreq);				
            ret = 0;
			break;
        default:
				printk(KERN_INFO "DK::Unreconginzed ioctl command %d \n",cmd);
				break;
		}
		return ret;
}
...
INT32  dk_dev_init(void) {
		...
		printk(KERN_INFO "dk_dev_init::status after register_chrdev(dk) = %d\n", status);

#ifdef DK_UART
        status |= register_chrdev(DK_UART_MAJOR_NUMBER, "dk_uart", &dk_fops);
		printk(KERN_INFO "dk_dev_init::status after register_chrdev(dk_uart) = %d\n", status);
#endif
		return status;
}
