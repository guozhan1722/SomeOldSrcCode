...
static INT32 __init dk_module_init(void) 
{
        ...
        printk(KERN_INFO "Writing value 0x6 to Merlin PCI command register\n");
        writel(0x6,addr); // enabling ddr and dma of Merlin 
		
#endif
		error = dk_dev_init();
		if (error < 0) {
			printk(KERN_INFO "DK::Cannot register device \n");
			return error;
		}
		init_client();
#ifdef AP83
                if (init_wmac_device()){ // enabling the wmac ; setting the handle for applications
                         printk(KERN_INFO "Error in initializing wmac \n");
                         return error;
                }
		return 0;
#endif
		...
		if (error < 0) {
			...
			cleanup_client();
			dk_dev_exit();
			printk(KERN_INFO "DK::Cannot locate device. Reset the machine \n");
			return error;
		}
#endif
		return 0;
...
{
#ifdef DK_DEBUG
		printk(KERN_INFO "DK::Module exit \n");
#endif // DK_DEBUG
#if defined(OWL_PB42) || defined(PYTHON_EMU)
		bus_module_exit();
#endif
