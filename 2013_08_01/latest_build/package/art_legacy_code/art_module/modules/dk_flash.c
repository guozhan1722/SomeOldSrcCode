...
sysFlashConfigInit_x16(FlashWPTR ptr)
{
    ...
    cfi.size = 0;
    cfi.base = (UINT8 *)(ioremap((long)ptr, 0x10000));
...
printk("cfi.blockSize is 0x%x\n", cfi.blockSize);
    ...
    basePtr = (UINT8 *)(ioremap((long)base, cfi.ebSize[off]));
    i = NFLC - 1;

printk("base is 0x%p\n", base);
printk("basePtr is 0x%p\n", basePtr);
printk("off is 0x%x\n", off);
...
unsigned int dk_flash_write (int flc, unsigned int offset, size_t len,const UINT8 * buf) {
  return sysFlashConfigWrite(flc, offset, (UINT8*)buf, len);
}
