...
#define ATHEROS_AP53_FULL         0xa053
#define ATHEROS_AP53_LITE         0xa054
#define ATHEROS_XB63              0x3067
#define ATHEROS_HB63              0x3064
...
#define XPD_LO			3//Low Gain was 2
#define XPD_HI			1//Low Gain was 1
...
#define CURR_CNSMPN_TX		0
#define CURR_CNSMPN_RX		1
...
#define PCI_EXPRESS_CONFIG_PTR_SERDES  0x40
...
typedef struct CurrConsmpnSttngs_t {
	A_BOOL		idleTestEnable;
	A_BOOL		txTestEnable[3];   // a/b/g
	A_BOOL		rxTestEnable[3];
	//double		supplyVoltage;
	double		idleHi;
	double		idleLo;
	double		hi[2][3];   // tx or rx:a/b/g
	double		lo[2][3];
} STRUCT_CURR_CNSMPN_STTNGS;
...
typedef struct calSetup_t {
    ...
    A_BOOL   calPower;
	char	 restoreFilename[50];
	A_BOOL	 restoreEeprom;
	double   offsetCalPower;
	A_BOOL	 useFastCal;   // power meter or golden unit
	A_BOOL   reprogramTargetPwr;
	A_BOOL   testOBW;
    A_BOOL   testSpecMask;
	A_BOOL   testChannelAccuracy;
    A_BOOL   testTXPER;
    A_BOOL   testRXSEN;
	A_BOOL	 testTURBO;
	...
	double   targetPowerToleranceUpperAntB;
	double   targetPowerToleranceLowerAntB;
	double   targetPowerToleranceUpperZero;
	double   targetPowerToleranceLowerZero;
	A_UINT16 targetPowerAntBRate;

    A_INT32  goldenPPM;
	A_BOOL   goldenPPMDisable;
	...
	A_UINT32 dmmModel;
	A_UINT32 dmmGPIBaddr;
    double   attenDutPM;
    double   attenDutPMAux;
    double   attenDutSA;
    double   attenDutGolden;

    double   attenDutPM_2p4[2];
    double   attenDutPMAux_2p4[2];
...
// Addition for Condor tests
	STRUCT_CURR_CNSMPN_STTNGS	CURR_CNSMPN_STTNGS;
	A_UINT32 testContinuity;
	A_UINT32 continuityOutputGpio;
	A_UINT32 continuityInputGpio;
...
// 2.4 GHz parameters 
	...
	A_BOOL   testChannelAccuracy_2p4[2];
	...
	A_BOOL    maskFailPointsUseMatrix;
...
//	A_UINT32 cal_single_xpd[3];
	...
	A_UINT32    numRetriesThruput;
	A_BOOL      staCardOnAP;
	A_BOOL		LOsBypass;
	A_BOOL      doCCKin11g;
    A_BOOL      fixZeroPowerDuringCal;
    A_UINT32    targetPowerZeroDelay;
	A_UINT32    enableSerdesProgramming;
	A_BOOL      enableASPMLatency;
	A_BOOL      enableWA;
	A_UINT32    minimumPower[3];
	A_UINT32    aspmSupport;
	A_UINT32    zeroAttenuator;
    A_UINT32	spur_frequency_5g[EEPROM_SPUR_11A_SIZE];	//eeprom 0x138 - 0x13c
    A_UINT32	spur_frequency_2p4g[EEPROM_SPUR_11G_SIZE];	//eeprom 0x13d - 0x14f
} CAL_SETUP; 
...
typedef struct testChannelMasks {
	...
	A_BOOL			TEST_THROUGHPUT_RX;
	double			throughputRxThreshold;
	A_BOOL			TEST_TURBO_THROUGHPUT;
	double			throughputTurboThreshold;
	A_BOOL          TEST_CHANNEL_ACCURACY;
	A_BOOL			TEST_AUX_ANTENNA;
	A_BOOL			TEST_CURR_CNSMPN_TX;
	A_BOOL			TEST_CURR_CNSMPN_RX;
	A_UINT32		mask_fail_threshold; 
    A_BOOL          TEST_ZERO_POWER;
} TEST_CHANNEL_MASKS ;
