...
extern A_UINT32	optGainLadderIndex[3];
extern A_BOOL	NEED_GAIN_OPT_FOR_MODE[];
extern A_UINT32	optHighGainLadderIndex[3];
extern A_BOOL	NEED_HIGHGAIN_OPT_FOR_MODE[];
...
//		if (1) {
	...
	for (ii=0; ii<pRawDataset_gen3[mode]->numChannels; ii++)
	{
        ...
        //apply fixed gain here, in case being changed for other pdgain
...
//		if (CalSetup.cal_mult_xpd_gain_mask[mode] > 0) {
		if ( xgain_list[1] != xgain_list[0] ) {
            ...
            //apply high gain fixed gain if needed
            if(NEED_HIGHGAIN_OPT_FOR_MODE[mode]) {
...
void golden_iq_cal(A_UINT32 devNum, A_UINT32 mode, A_UINT32 channel) {
	...
	art_resetDevice(devNum, txStation, NullID, channel, 0);
	art_forceSinglePowerTxMax(devNum, (A_UINT16)((10-2*CalSetup.offsetCalPower)<= 0 ? 0: (10-2*CalSetup.offsetCalPower)));//Siavash
	...
	while (!verifyAckStr("Done with iq_cal for mode") && (iter++ < (maxIter + 1))) {
		...
		if (!TransmitOn) {
			art_txContBegin(devNum, CONT_FRAMED_DATA, RANDOM_PATTERN, IQ_CAL_RATE, DESC_ANT_A | USE_DESC_ANT | DESC_ANT_B);//Siavash added B
			TransmitOn = TRUE;
		}
...
//	double pwr_meas_q = 0;
	...
	A_UINT32 rddata;
	//for summing the power on the  11 samples per iteration
	A_UINT32 total_pwr_meas_i = 0 ;
	A_UINT32 total_pwr_meas_q = 0;
	A_INT32 total_iq_corr_meas = 0;
...
//			uiPrintf("[%d] rddata = %d\n", iterRX, rddata);
		...
		// sum them up it prevents ADC noise in estimation
		total_pwr_meas_i   = total_pwr_meas_i + pwr_meas_i;
		total_pwr_meas_q   = total_pwr_meas_q + pwr_meas_q;

	    if (iq_corr_meas > 0x80000000) {
		   iq_corr_meas = (0xffffffff - iq_corr_meas) + 1;
		   total_iq_corr_meas = total_iq_corr_meas - iq_corr_meas;
		} else {
			total_iq_corr_meas = total_iq_corr_meas + iq_corr_meas;
		}
...
//	uiPrintf("SNOOP: interm i_coeff = %d, q_coeff = %d\n", i_coeff, q_coeff);
	...
	if (CalSetup.customerDebug > 0) {
		uiPrintf("pwr_I = %u, pwr_Q = %u, IQ_Corr = %u\n", pwr_meas_i, pwr_meas_q, iq_corr_meas);
	}
