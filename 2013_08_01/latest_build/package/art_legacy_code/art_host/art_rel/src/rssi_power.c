...
//		measure_all_channels_gen5(devNum, CalSetup.customerDebug, mode) ;
		//Remarked teh measure _all ... by Siavash new function is MH_Cal_pm_gen5
		//measure_all_channels_generalized_gen5(devNum, CalSetup.customerDebug, mode) ;
		MH_cal_pm_gen5(devNum, CalSetup.customerDebug, mode) ;
						...
						A_UINT32 debug)
{
	...
	for (ii=0; ii<numAvg; ii++)
	{
		...
		{
			art_forceSinglePowerTxMax(devNum, pcdacs[jj]);// Siavash ??? 
...
//	A_UINT32 complete_timeout_short = (mode == MODE_11b) ? 50: 20; // for one pcdac
	...
	for (ii=0; ii<numAvg; ii++)
	{
		...
		for (pcdIter=0; pcdIter < numPcd; pcdIter++)
		{
			...
			{
				// arm power meter
				usleep(1000*E4416_SLEEP_INTERVAL);
