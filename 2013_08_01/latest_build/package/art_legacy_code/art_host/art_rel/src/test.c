...
//								    6   9  12  18  24  36  48  54  1L   2L   2S   5.5L  5.5S 11L  11S XR0.25  XR0.5  XR1   XR2   XR3
const A_UCHAR rateSensitivities[]={91, 90, 89, 88, 84, 81, 75, 74, 95,  93,  93,  92,   92,  87,  87, 104,    103,   101,  98,   97};
...
q_uiPrintf("findEepFile: mark 1\n");	
	tempDevNum = art_setupDevice(configSetup.instance);
q_uiPrintf("findEepFile: mark 2\n");	
	if(tempDevNum < 0) {
...
q_uiPrintf("findEepFile: mark 3\n");
    ...
    q_uiPrintf("findEepFile: mark 5, swDeviceID=0x%x, hwDeviceID=0x%x\n", swDeviceID, hwDeviceID);
...
//#ifdef CONDOR_HACK
			...
			swDeviceID = devStruct.swDevID;
			hwDeviceID = devStruct.hwDevID;
            q_uiPrintf("715: swDeviceID=0x%x, hwDeviceID=0x%x\n", swDeviceID, hwDeviceID);
...
//#endif
	...
	swDeviceID = devStruct.swDevID;
	hwDeviceID = devStruct.hwDevID;
    uiPrintf("815: swDeviceID=0x%x, hwDeviceID=0x%x\n", swDeviceID, hwDeviceID);
...
//END SNOOP
	//************************************************************
	// putting VISA session right before the test menu 
	// Default RM session is always open till the end of the test
	//************************************************************
...
#endif	// __ATH_DJGPPDOS__
        printf("|   a - (A)DF 4350 frequency test          |\n");
        if(configSetup.logging) {
			...
			if (supportMultiModes(&supportedModes))
 			{
				...
				if (configSetup.useTargetPower == TRUE) {
					...
					uiPrintf("1248: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
				}
				updateConfigFromRegValues(devNum);
...
A_BOOL parseCmdLine(A_INT32 argc, A_CHAR *argv[])
{
    ...
    // default setting
    {
        ...
        configSetup.cmdLineSubsystemID = 0x2062;
        strncpy(configSetup.machName, "127.0.0.1", 256);
    }
//	if (argc > 4)
//	{
...
//	}
	...
	//uiPrintf("parseCmdLine: argc=%d.\n",argc );
	//uiPrintf("parseCmdLine: argv[0]=%s.\n",argv[0]);
	//uiPrintf("parseCmdLine: argv[1]=%s.\n",argv[1]);
	//uiPrintf("parseCmdLine: argv[2]=%s.\n",argv[2]);
	for( i = argc; i > 1; i--) {
	  if (strncmp(argv[i-1],"id=",3) == 0)
	  {
	    if(!sscanf(&argv[i-1][3], "%x", &configSetup.cmdLineSubsystemID)) {
        ...
        configSetup.remote = 1;
        configSetup.primaryAP = 1;
        uiPrintf("configSetup.cmdLineSubsystemID = 0x%x.\n",configSetup.cmdLineSubsystemID);
        //        strncpy(configSetup.machName, "127.0.0.1", 256);
	  }
#ifndef __ATH_DJGPPDOS__
		...
		else if (strncmp(argv[i-1],"remote=",7) == 0)
		{
			configSetup.remote = 1;
			configSetup.primaryAP = 1;
			strncpy(configSetup.machName, &(argv[i-1][7]), 256);
...
//daniel            (char *)temp=strtok(&argv[i - 1][6], "\"");
	    {
	      char *pos = strtok(&argv[i - 1][6], "\"");
	      temp = (A_CHAR*)pos;
	    }
			if(temp!=NULL){
...
*/
    ...
    while(!done) {
		...
		if (configSetup.useTargetPower == TRUE)
		{
//			uiPrintf("2586: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
			configSetup.powerOutput = art_getMaxPowerForRate(devNum, (A_UINT16)configSetup.channel, DataRate[configSetup.dataRateIndex]);
//			uiPrintf("2588: powerOutput = %4.1f, ", (float)configSetup.powerOutput);			
			//while(1);
		}
...
#endif
		{
          q_uiPrintf("2742: configSetup.powerOutput =  %4.1f\n", ((float)configSetup.powerOutput)/2);
...
//						pcdac_pwr = CCK_11G_IDX;
				...
				//uiPrintf("2750: printContMenu mark, dBm_pwr=%d\n", dBm_pwr);
//				uiPrintf("SNOOP: dbm_pwr = %d, pcdac_pwr = %d\n", dBm_pwr, pcdac_pwr);
                ...
                q_uiPrintf("2766: configSetup.powerOutput =  %4.1f\n", ((float)configSetup.powerOutput)/2);
				tweakGainF(pCurrGainLadder, devNum, 1);
				//Sleep(200);		
				usleep(200000);
				readGainFInContTx(pCurrGainLadder, devNum, (configSetup.powerControlMethod == DB_PWR_CTL) ? dBm_pwr : pcdac_pwr);	
                q_uiPrintf("2772: configSetup.powerOutput =  %4.1f\n", ((float)configSetup.powerOutput)/2);
                ...
                q_uiPrintf("2785: configSetup.powerOutput =  %4.1f\n", ((float)configSetup.powerOutput)/2);
				if(countm++==9)
                {
                    countm = 0;
                    printContMenu(devNum);
...
*/
        ...
        q_uiPrintf("2820: inputKey = %d\n", inputKey);
        q_uiPrintf("2821: configSetup.powerOutput =  %4.1f\n", ((float)configSetup.powerOutput)/2);
...
//					if (configSetup.mode == MODE_11A) {
			...
			case 'O':
				if((macRev != 0x40) || (configSetup.contMode != CONT_TX100) || (configSetup.mode == MODE_11B)) {
					...
					{
						configSetup.powerOutput = art_getMaxPowerForRate(devNum, (A_UINT16)configSetup.channel, DataRate[configSetup.dataRateIndex]);
						uiPrintf("2840: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
					}
				}
				break;
			case 'k':
			case 'K':
				if((macRev != 0x40) || (configSetup.contMode != CONT_TX100) || (configSetup.mode == MODE_11B)){
					...
					{
						configSetup.powerOutput = art_getMaxPowerForRate(devNum, (A_UINT16)configSetup.channel, DataRate[configSetup.dataRateIndex]);
						uiPrintf("2853: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
					}
				}
				break;
			...
			case 'E':
				...
				if (CalSetup.useInstruments)
				{
				devSA_local = spaInit(CalSetup.saGPIBaddr, CalSetup.saModel);// &CalSetup.globalSession
...
//		devSA_local = spaInit(CalSetup.saGPIBaddr, 4);// &CalSetup.globalSession
...
*/
	...
	// setup attenuator here for first time useAttenuator is always true at begining
	if (useAttenuator){
		if(CalSetup.useInstruments) {
				if ( CalSetup.attModel != NO_ATT){ 
					devATT = attInit(CalSetup.attGPIBaddr, CalSetup.attModel);
					//targetSensitivity = 54 MBPS
					attSet(devATT, (A_INT32)(configSetup.powerOutput/2 - CalSetup.attenDutGolden + rateSensitivities[7] ));//configSetup.dataRateIndexTP
					attClose();
	...
	if (CalSetup.useInstruments){
		printf("|   ` - Toggle Attenuator=(Sensitivity at 54 Mbps ) - Backoff	|\n");			
	}
	...
	printMode();
	uiPrintf("\n");
	//uiPrintf("3351 link test mark, exitLoop=%d\n", exitLoop);
    while(exitLoop == FALSE) {
#ifdef __ATH_DJGPPDOS__
		if (_bios_keybrd(_KEYBRD_READY)) {
#else
		//uiPrintf("3356 link test mark\n");
		if(kbhit()) {     
#endif		
           	inputKey = getchar();
		//uiPrintf("3360 link test mark, inputKey = %x\n", inputKey);
			if(!processCommonOptions(devNum, inputKey)) {
				switch(inputKey) {
				case '9':  // toggle dynamic gain adj. on or off
//				  if((swDeviceID & 0xff) >= 0x14) {
							...
							if(CalSetup.useInstruments) {
								if ( CalSetup.attModel != NO_ATT){ 
									...
									if (configSetup.useTargetPower)
										attSet(devATT, (A_INT32)(95));
									else
										attSet(devATT, (A_INT32)(configSetup.powerOutput/2 - CalSetup.attenDutGolden + rateSensitivities[7] ));
									usleep(1000*50);
									attClose();
								}
							}
...
//				time1 = milliTime();
			...
			usleep(1000*1500);	// delay 1500 ms between packet sends
            break;
        case 2:			// receive mode
...
//		printf("|   k - Decrease Buffer Size by 100 (K dec by 1000)             |\n");
		...
		for(i = (A_UINT16)startRateIndex; i < (A_UINT16)maxRateIndex+1; i++) {
			if((configSetup.enableXR) &&  
				((configSetup.mode == MODE_11G) || (configSetup.mode == MODE_11A))){
				if((i > 7) && (i < 15)) {
					continue;
...
void throughputMenu(A_UINT32 devNum)
{
	...
	A_UINT32 pcdac_pwr = OFDM_CCK_DEF_IDX;
	A_INT32  dBm_pwr;
	A_UINT32  max_num_pkts;//, value, rddata;
	A_INT32 devATT =0;
...
*/
		...
		if (useAttenuator){
			if(CalSetup.useInstruments) {
					if ( CalSetup.attModel != NO_ATT){ 
						devATT = attInit(CalSetup.attGPIBaddr, CalSetup.attModel);
						//targetSensitivity =
						attSet(devATT, (A_INT32)(configSetup.powerOutput/2 - CalSetup.attenDutGolden + rateSensitivities[configSetup.dataRateIndexTP] ));
						attClose();
	...
	printf("|   h - Increase db by 1 (q - increase b-db)                    |\n");
	if (CalSetup.useInstruments){
		printf("|   ` - Toggle Attenuator = (STA Sensitivity at Rate) - Backoff |\n");			
	}
	...
	if (configSetup.useTargetPower == TRUE)
	{
		...
		uiPrintf("4280: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
	}
    while(exitLoop == FALSE) {
#ifdef __ATH_DJGPPDOS__
		if (_bios_keybrd(_KEYBRD_READY)) {
...
//				  if((swDeviceID & 0xff) >= 0x14) {
				...
				case 'O':
					...
					{
						configSetup.powerOutput = art_getMaxPowerForRate(devNum, (A_UINT16)configSetup.channel, DataRate[configSetup.dataRateIndexTP]);
						uiPrintf("4334: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
					}
					break;
				case 'k':
				case 'K':
					...
					{
						configSetup.powerOutput = art_getMaxPowerForRate(devNum, (A_UINT16)configSetup.channel, DataRate[configSetup.dataRateIndexTP]);
						uiPrintf("4347: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
					}
					break;
				case 's':
				case 'S':
...
//			setRegistersFromConfig(devNum);
			...
			printConfigSettings(devNum);
            uiPrintf("4481 Throughput Test mark\n");
...
//				if ((configSetup.mode == MODE_11A) || (configSetup.mode == MODE_11O)){
	...
	case 'f':
		if (configSetup.validCalData)
		{
			...
			if(configSetup.powerOutput > POWER_OUT_MAX) {
				configSetup.powerOutput = (A_UINT16)CalSetup.minimumPower[calModeFor[configSetup.mode]];
			}
#endif
	...
	case 'F':
		if (configSetup.validCalData)
		{
			...
			if(configSetup.powerOutput > POWER_OUT_MAX) {
				configSetup.powerOutput = (A_UINT16)CalSetup.minimumPower[calModeFor[configSetup.mode]];
				uiPrintf("4797: powerOutput = %4.1f, ", (float)configSetup.powerOutput);
			}
#endif
			configSetup.powerControlMethod = DB_PWR_CTL;
			configSetup.useTargetPower = FALSE;					
	...
	case 'c':
		if (configSetup.validCalData)
		{
            //uiPrintf("Before configSetup.powerOutput = %4.1f\n", ((float)configSetup.powerOutput)/2);
			if(configSetup.powerOutput == USE_REG_FILE) {
				configSetup.powerOutput = INITIAL_POWER_OUT;
			}
			else {
#ifndef __ATH_DJGPPDOS__
				if(configSetup.powerOutput == CalSetup.minimumPower[calModeFor[configSetup.mode]]) {
            ...
            //uiPrintf("After configSetup.powerOutput = %4.1f\n", ((float)configSetup.powerOutput)/2);
            //sleep(1);
		} else
		{
				invalidEepromMessage(3000);
...
//daniel replaced by:	
	...
	//uiPrintf("addToCfgTable: mark1, temp=%s, configSetup.eepFileDir=%s, pFilename=%s.\n", temp, configSetup.eepFileDir, pFilename); 
...
//		configSetup.powerOvr = art_regRead(devNum, TPCRG1_REG) & BB_FORCE_DAC_GAIN_SET;
	...
	if(!isEagle(swDeviceID) && !isGriffin(swDeviceID)) {
        ...
        uiPrintf("5855: configSetup.gainI = %x\n", configSetup.gainI);
...
)
{
	...
	if(configSetup.powerControlMethod == DB_PWR_CTL)
	{
		...
		if((configSetup.powerOutput != USE_REG_FILE) &&	(!configSetup.useTargetPower)){
			for(i = 0; i < NUM_RATES; i++) {
				if (configSetup.powerOutput >= configSetup.offsetCalPower2x)
											powerVals[i] = configSetup.powerOutput-configSetup.offsetCalPower2x;
				else
											powerVals[i] = 0;
			}
...
//++JC++
	...
	if(configSetup.powerOvr) {
		if(isEagle(swDeviceID) || isGriffin(swDeviceID)) {
			ForceSinglePCDACTableGriffin(devNum, (A_UINT16)configSetup.gainI, GAIN_OVERRIDE);			
		}
		else {
		...
		} else
		{
			if (configSetup.powerOutput == USE_REG_FILE) {
				...
				if(ctlPower == configSetup.powerOutput) {
					...
					if (configSetup.debugInfo) {
						uiPrintf("Target Power = %4.1f[%4.1f], ", ((float)configSetup.powerOutput+configSetup.offsetCalPower2x)/2, art_getMaxLinPower(devNum));
					} else {
						uiPrintf("Target Power = %4.1f, ", ((float)configSetup.powerOutput+configSetup.offsetCalPower2x)/2);
						//uiPrintf("powerOutput = %4.1f, ", ((float)configSetup.powerOutput)/2);
						//uiPrintf("offsetCalPower2x = %4.1f, ", ((float)configSetup.offsetCalPower2x)/2);
					}
				}
			} else {
				if (configSetup.debugInfo) {
...
static unsigned long long GetFreq(char *string)
{
    ...
    if(c) {
        *c = 0;
        ...
        temp1 = atoll(string);
        scale = decide_scale(string);
        temp1 *= scale;           
    }
    return temp1;
}

static void CollectInput(char *getc, int len)
{
    int i;
    for(i=0; i<len-1; i++) {
        *(getc + i) = getchar();
...
void Display_Adf4350_menu(void)
{
	uiPrintf("\n");
    uiPrintf("========================================================\n");
    uiPrintf("| ADF4350 test Utility                                 |\n");
    uiPrintf("|   D - Set up frequency for A(D)F chip 1 and transmit |\n");
    uiPrintf("|   S - (S)top transmitting on A(D)F chip 1            |\n");
    uiPrintf("|   F - Set up frequency for AD(F) chip 2 and transmit |\n");
    uiPrintf("|   P - Sto(p) transmitting on A(D)F chip 2            |\n");
    uiPrintf("|   T - Set up frequency for A(t)heros chip            |\n");
    uiPrintf("|   C - (C)ontinue transmitting on Atheros chip        |\n");
    uiPrintf("|   Q - (Q)uit and returen to upper layer menu         |\n");
	uiPrintf("| ESC - exit                                           |\n");
    uiPrintf("| -- ALL VALUES ARE TREATED AS DECIMAL HERE --         |\n");
	uiPrintf("========================================================\n");
}

A_BOOL Adf4350Menu(A_UINT32 devNum)
{
    A_BOOL	exitLoop = FALSE;
    unsigned long long Adfreq = 0;
    A_UINT32 err = 0;
    char getc[256];
    A_UINT32    temp;
 
    clearScreen(); 
    Display_Adf4350_menu();
    while(exitLoop == FALSE) {
        if(kbhit()) {
			switch(toupper(getchar())) {
            case 'D':
                uiPrintf("Enter new frequency for ADF Chip 1 in Hz: ");
                CollectInput(getc, sizeof(getc));
                Adfreq = GetFreq(getc);
                if((Adfreq<137500000)||(Adfreq>4400000000LL)) {
                    uiPrintf("ERROR: %lld is out of range\n", Adfreq);
                    break;
                } else {
                    uiPrintf("Setting ADF435 frequency to %lld Hz\n", Adfreq);
                    err = art_SetAdf4350(devNum, 1, Adfreq);
                    if(err) uiPrintf("Setting ADF435 frequency error code(0x%x)\n", err); 
                }
                 Display_Adf4350_menu();
                break;
            case 'S':
                Adfreq = 0;
                err = art_SetAdf4350(devNum, 1, Adfreq);
                if(err) uiPrintf("Stop ADF435 chip 1 error code(0x%x)\n", err); 
                clearScreen(); 
                Display_Adf4350_menu();
                break;
            case 'F':
                uiPrintf("Enter new frequency for ADF Chip 2 in Hz: ");
                CollectInput(getc, sizeof(getc));
                Adfreq = GetFreq(getc);
                if((Adfreq<137500000)||(Adfreq>4400000000LL)) {
                    uiPrintf("ERROR: %lld is out of range\n", Adfreq);
                    ...
                    uiPrintf("Setting ADF435 frequency to %lld Hz\n", Adfreq);
                    err = art_SetAdf4350(devNum, 2, Adfreq);
                    if(err) uiPrintf("Setting ADF435 frequency error code(0x%x)\n", err); 
                }
                Display_Adf4350_menu();
                break;
            case 'P':
                Adfreq = 0;
                err = art_SetAdf4350(devNum, 2, Adfreq);
                if(err) uiPrintf("Stop ADF435 chip 2 error code(0x%x)\n", err); 
                clearScreen(); 
                Display_Adf4350_menu();
                break;
            case 'T':
                uiPrintf("Must enter new channel value in MHz: ");
                CollectInput(getc, sizeof(getc));
                temp = 10*atol(getc);  
...
A_BOOL EEPROM_Routine(A_UINT32 devNum)
{
	...
	while(exitLoop == FALSE) {
		...
		if(kbhit()) {
			...
			case 'D':	//Display			    
				uiPrintf("Display eeprom\n");
				if(sizeWarning)	uiPrintf(" Only the  %d Location are shown \n",eepromSize);
				...
				for (i=0;i<length;i++) {
                    ...
                    {
                        uiPrintf("\n0x%04x: ", addr+i);
                        uiPrintf("0x%04x ", eepromValue[i]);
                    }
                    else uiPrintf("0x%04x ", eepromValue[i]);
			    }
			    uiPrintf("\n");
...
//
	switch(configSetup.contMode) {
		...
		case CONT_TX99:
            ...
            if (!configSetup.turbo) uiPrintf(", Normal rate");
			else if (configSetup.turbo==TURBO_ENABLE) uiPrintf(", Turbo rate");
			else if (configSetup.turbo==HALF_SPEED_MODE) uiPrintf(", Half rate");	
			else if (configSetup.turbo==QUARTER_SPEED_MODE) uiPrintf(", Quarter rate");
			break;
