...
)
{
	...
	if(pHeaderInfo->majorVersion >= 4) {
		...
		(pHeaderInfo->majorVersion >= 5)){
			...
			if(pHeaderInfo->artBuildNumber & (0x1 << 15)) {
				//flag is set to display build number as hex
				uiPrintf("|  ART Build Number       %3x  |\n", (pHeaderInfo->artBuildNumber & 0x7f));
			} else {
		...
		if(((pHeaderInfo->majorVersion  == 5) && (pHeaderInfo->minorVersion >= 3))
			...
			uiPrintf(" |  enable Jap odd Uni 1   %3d  ", pHeaderInfo->enableJapanOddU1);
	...
	for(j = 1; j <= 5; j++) {
        ...
        if(strlen(binaryString)<=4) {
            for(i = 0; i < (4 - strlen(binaryString)); i++) {
                uiPrintf("0");
            }
        }       
...
)
{
    ...
    ATbytesSwap(pRdEdgePwrInfo, (sizeof(MDK_RD_EDGES_POWER)*NUM_16K_EDGES*maxNumCtl)/2);
	...
	while ((pTestGroups[i] != 0) && (i < maxNumCtl)) {
		...
		if(!CalSetup.Bmode && (mode == MODE_11G) && (ctlMode == MODE_11B)) {
			//want to display this ctl rather than skip it. 
		}
		else if(mode != ctlMode) {
			//skip mode
			i++;
...
)
{
    ...
    ATbytesSwap(pHeaderPtr, sizeof(MDK_EEP_HEADER_INFO)/2);
	...
	if((majorRevision ==3) ||
        ...
        ATbytesSwap(pTempPtr, sizeof(MDK_PCDACS_ALL_MODES)/2);
        ...
        ATbytesSwap(pTempPtr, sizeof(EEPROM_FULL_DATA_STRUCT_GEN3)/2);
        ...
        ATbytesSwap(pTempPtr, sizeof(EEPROM_FULL_DATA_STRUCT_GEN5)/2);
    ...
    ATbytesSwap(pTempPtr, sizeof(MDK_TRGT_POWER_ALL_MODES)/2);
	printTargetPowerInfo_16K((MDK_TRGT_POWER_ALL_MODES *)pTempPtr, mode);
#ifndef __ATH_DJGPPDOS__
	if(!CalSetup.Bmode && (mode == MODE_11G)) {
		art_GetEepromStruct(devNum, EEP_TRGT_POWER_16K, &pTempPtr);
        ATbytesSwap(pTempPtr, sizeof(MDK_TRGT_POWER_ALL_MODES)/2);
		printTargetPowerInfo_16K((MDK_TRGT_POWER_ALL_MODES *)pTempPtr, MODE_11B);
	}
#endif
	uiPrintf("\nPress any key to continue\n");
	while(!kbhit());
