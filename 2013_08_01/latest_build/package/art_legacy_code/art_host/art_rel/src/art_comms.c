...
)
{
    ...
    if ( osSockRead(pOSSock, (A_UINT8 *)(&connectlen), sizeof(connectlen)) ) {
		connectlen = TbytesSwap(connectlen);
	...
	if(status == A_OK) {
        ...
        PipeBuffer_p->CMD_U.GENERIC_CMD.intVar1 = FbytesSwap(PipeBuffer_p->CMD_U.GENERIC_CMD.intVar1);
		PipeBuffer_p->CMD_U.GENERIC_CMD.intVar2 = FbytesSwap(PipeBuffer_p->CMD_U.GENERIC_CMD.intVar2);
		PipeBuffer_p->CMD_U.GENERIC_CMD.intVar3 = FbytesSwap(PipeBuffer_p->CMD_U.GENERIC_CMD.intVar3);
    ...
    ReplyCmd_p->replyCmdLen = FbytesSwap(ReplyCmd_p->replyCmdLen);
	ReplyCmd_p->replyCmdId = FbytesSwap(ReplyCmd_p->replyCmdId);
	ReplyCmd_p->status = FbytesSwap(ReplyCmd_p->status);
