...
//A_UINT32 VERIFY_DATA_PACKET_LEN   = 3900;
A_UINT32 VERIFY_DATA_PACKET_LEN   = 2300;
...
#define NUM_THROUGHPUT_ADDRS 3

static A_UCHAR throughputStationIDs[NUM_THROUGHPUT_ADDRS][6] = {
...
static A_CHAR *PER_11B_RATE_STR[] = {"1Mbps", "5.5Mbps(S)", "11Mbps(L)", "11Mbps(S)"};
...
static A_UINT32 NUM_DATA_CHECK_FRAME = 100;
...
//daniel static A_UINT32 LO_SEN_TEST_RATE_11G_CCK[] = {0xb1};
#define LO_SEN_TEST_RATE_11G_CCK 0xb1
...
A_UINT32	optGainLadderIndex[3];
A_BOOL	NEED_GAIN_OPT_FOR_MODE[] = {FALSE, FALSE, FALSE};
A_UINT32	optHighGainLadderIndex[3];
A_BOOL	NEED_HIGHGAIN_OPT_FOR_MODE[] = {FALSE, FALSE, FALSE};

static A_UINT32 loadEARState;
static A_UINT32 enableXRState;
static A_BOOL quarterChannelState;

static A_BOOL dmmAccessible = FALSE;
static A_BOOL CurrTestRxEnable = FALSE;
...
static void goldenThroughputTest(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT16 frameLen,
						  A_UINT32 numPackets);
static void dutThroughputRxTest(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT16 frameLen, 
					   A_UINT32 numPackets);
static void goldenThroughputRxTest(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT16 frameLen,
						  A_UINT32 numPackets);
static A_BOOL dutTestContinuity(A_UINT32 devNum);

extern A_UINT16 readGPIO(A_UINT32 devNum, A_UINT16 gpioNumber);
extern void driveGPIO(A_UINT32 devNum, A_UINT16 gpioNumber, A_UINT16 value);
extern void configureGPIOInput(A_UINT32 devNum, A_UINT16 gpioNumber);
extern void configureGPIOOutput(A_UINT32 devNum,	A_UINT16 gpioNumber);

#define NUM_THROUGHPUT_PACKETS		 2000
#define NUM_THROUGHPUT_PACKETS_11B   200
#define THROUGHPUT_PACKET_SIZE		 1500
#define THROUGHPUT_TURBO_PACKET_SIZE 3992

#define MH_POWERCAL					 0
#define MH_POWER_PROFILE			 1
#define MH_POWER_READ				 2
...
void topCalibrationEntry(A_UINT32 *pdevNum_inst1, A_UINT32 *pdevNum_inst2) 
{
	...
	art_setResetParams(devNum_inst2, configSetup.pCfgFile, (A_BOOL)configSetup.eepromLoad,
								(A_BOOL)1, MODE_11A, configSetup.use_init);		
...
void dutBegin() 
{
	...
	{
		uiPrintf("DUT_CARD_SSID specified in artsetup.txt (%x) does not match the  SSID of this card: (%x). ",configSetup.dutSSID ,
				  configSetup.cfgTable.pCurrentElement->subsystemID);
...
*/
	...
	if (CalSetup.restoreEeprom) 
		restoreEeprom(devNumArr[MODE_11a], CalSetup.restoreFilename);
	while (!exitLoop) 
	{
		REWIND_TEST = FALSE;
		TestFail = FALSE;
		...
		if ( CalSetup.useFastCal || CalSetup.testTXPER || CalSetup.testRXSEN ||
			 ...
			 CalSetup.testRxThroughput[MODE_11a] || CalSetup.testRxThroughput[MODE_11g] ||
			 CalSetup.testRxThroughput[MODE_11b])
//			 ((swDeviceID & 0xFF) >= 0x14))
		{
			sendSync(devNumArr[MODE_11b], CalSetup.goldenIPAddr, CalSetup.customerDebug);
		...
		if(CalSetup.useInstruments && (!REWIND_TEST))// This should be the only place for instruments in the loop
		{
			timestart = milliTime();
			if (CalSetup.CURR_CNSMPN_STTNGS.idleTestEnable 
				|| CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11a] || CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11a]
				|| CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11b] || CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11b]
				|| CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11g] || CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11g] )
			{
				uiPrintf("\nSetting up Current Meter");
				devDMM = dmmInit(CalSetup.dmmGPIBaddr, CalSetup.dmmModel);
				if (devDMM > 0)
				{
					dmmAccessible = TRUE;
				}
				...
				{
					uiPrintf ("\nERROR: Failed to obtain handle on DMM (Current Meter)");
					dmmAccessible = FALSE;
				}
			}
			if (CalSetup.pmModel != NO_PM){
				...
				SETUP_PM_MODEL2_ONCE = FALSE;
			}

			if ( CalSetup.saModel != SPA_NONE){
				uiPrintf("\nSetting up Spectrum Analyzer");
				devSA = spaInit(CalSetup.saGPIBaddr, CalSetup.saModel); // &CalSetup.globalSession
			}
			if ( CalSetup.attModel != NO_ATT){
...
#endif
			attSet(devATT, MAX_ATTENUATOR_RANGE); //set to max
			}else{
				uiPrintf("\nNo Attenuator Instrument set");	
			}
			strcpy(testname[testnum],"instrument setup");
			testtime[testnum++] = milliTime() - timestart;
		} 
		
		if (CalSetup.do_iq_cal && (((swDeviceID & 0xFF) == 0x14)||((swDeviceID & 0xFF) >= 0x16))) //CalSetup.calPower && 
		{
			// Set attenuator
			if(CalSetup.useInstruments &&  (CalSetup.attModel != NO_ATT)) 
			{
				attenVal = (A_INT32)(5 - CalSetup.attenDutGolden - (-35));
    			attSet(devATT, attenVal);
			}else{
				attenVal = (A_INT32)(CalSetup.attenDutGolden);
				uiPrintf("\nAttenuation set to defined value between DUT and Golden");
			}
			
			usleep(1000*200);

			if ((CalSetup.Amode|| CalSetup.enableJapanMode11aNew) && (CalSetup.do_iq_cal) && (!REWIND_TEST)) ////put CalSetup.calPower
			{
				timestart = milliTime();
			...
			if ((CalSetup.Gmode) && (CalSetup.do_iq_cal)  && (!REWIND_TEST)) 
			{
				timestart = milliTime();
			...
			{
				attSet(devATT, MAX_ATTENUATOR_RANGE);
			}
	   ...
	   if ((CalSetup.cal_fixed_highgain[MODE_11a] != INVALID_FG) && (CalSetup.Amode|| CalSetup.enableJapanMode11aNew))
		 {
		   if (CalSetup.cal_fixed_highgain[MODE_11a] > (pCurrGainLadder->numStepsInLadder-1)) 
			 {
			   uiPrintf("ERROR: Invalid CAL_FIXED_HIGHGAIN specified in calsetup.txt for mode ");
			   uiPrintf("11a [%d] : Valid range 0..%d\n", CalSetup.cal_fixed_highgain[MODE_11a], 
														  (pCurrGainLadder->numStepsInLadder-1));
			   exit(0);
		   }
		   optHighGainLadderIndex[MODE_11a] = (pCurrGainLadder->numStepsInLadder -1) - CalSetup.cal_fixed_highgain[MODE_11a];
		   uiPrintf("\nusing cal fixed high gain for mode 11a from calsetup.txt ... FG%d\n", CalSetup.cal_fixed_highgain[MODE_11a]);
		   NEED_HIGHGAIN_OPT_FOR_MODE[MODE_11a] = TRUE;
	   }

	   if ((CalSetup.cal_fixed_gain[MODE_11a] != INVALID_FG) && (CalSetup.Amode|| CalSetup.enableJapanMode11aNew))
		 {
		   ...
		   NEED_GAIN_OPT_FOR_MODE[MODE_11a] = TRUE;
	   } else if (CalSetup.calPower && (CalSetup.Amode|| CalSetup.enableJapanMode11aNew) && (!REWIND_TEST)) {
			...
			if(NEED_GAIN_OPT_FOR_MODE[MODE_11a] && (CalSetup.Amode || CalSetup.enableJapanMode11aNew) && CalSetup.useInstruments) {
	   ...
	   if ((CalSetup.cal_fixed_highgain[MODE_11b] != INVALID_FG) && CalSetup.Bmode) 
		 {
		   if (CalSetup.cal_fixed_highgain[MODE_11b] >= pCurrGainLadder->numStepsInLadder) 
			 {
			   uiPrintf("ERROR: Invalid CAL_FIXED_HIGHGAIN specified in calsetup.txt for mode ");
			   uiPrintf("11b [%d] : Valid range 0..%d\n", CalSetup.cal_fixed_highgain[MODE_11b], 
														  (pCurrGainLadder->numStepsInLadder-1));
			   exit(0);
		   }
		   optHighGainLadderIndex[MODE_11b] = (pCurrGainLadder->numStepsInLadder -1) - CalSetup.cal_fixed_highgain[MODE_11b];
		   NEED_HIGHGAIN_OPT_FOR_MODE[MODE_11b] = TRUE;
		   uiPrintf("\nusing cal fixed high gain for mode 11b from calsetup.txt ... FG%d\n", CalSetup.cal_fixed_highgain[MODE_11b]);
	   } 
	   ...
	   if ((CalSetup.cal_fixed_highgain[MODE_11g] != INVALID_FG) && CalSetup.Gmode){
		   if (CalSetup.cal_fixed_highgain[MODE_11g] >= pCurrGainLadder->numStepsInLadder) {
			   uiPrintf("ERROR: Invalid CAL_FIXED_HIGHGAIN specified in calsetup.txt for mode ");
			   uiPrintf("11g [%d] : Valid range 0..%d\n", CalSetup.cal_fixed_highgain[MODE_11g], 
														  (pCurrGainLadder->numStepsInLadder-1));
			   exit(0);
		   }
		   optHighGainLadderIndex[MODE_11g] = (pCurrGainLadder->numStepsInLadder -1) - CalSetup.cal_fixed_highgain[MODE_11g];
		   NEED_HIGHGAIN_OPT_FOR_MODE[MODE_11g] = TRUE; 
		   uiPrintf("\nusing cal fixed high gain for mode 11g from calsetup.txt ... FG%d\n", CalSetup.cal_fixed_highgain[MODE_11g]);
	   }
       if ((CalSetup.cal_fixed_gain[MODE_11g] != INVALID_FG) && CalSetup.Gmode){
		...
		if((CalSetup.Amode || CalSetup.enableJapanMode11aNew) && CalSetup.useInstruments && (CalSetup.testTempMargin[MODE_11a]) && (!REWIND_TEST)) {
		...
		if(CalSetup.useInstruments) {
			if ( CalSetup.attModel != NO_ATT){
				attSet(devATT, MAX_ATTENUATOR_RANGE); //if no ATT tehn fixed ATT is set
			}
		}
		...
		if ( CalSetup.CURR_CNSMPN_STTNGS.idleTestEnable)					
		{
			if(CalSetup.useInstruments)
				...
				{
					timestart = milliTime();
					strcpy(testname[testnum], "Idle Current Consumption");
					REWIND_TEST = dutMeasCurrIdle(devDMM, CalSetup.dmmModel, devNumArr[MODE_11g]);
					testtime[testnum++] = milliTime() - timestart;
					if (REWIND_TEST) 
					{
						printResultBanner(devNumArr[MODE_11a], TRUE); //mode irrelevant, passed to satisfy parameter list.
						uiPrintf("\nFailed Idle Current Consumption test.\n");
						exitLoop = prepare_for_next_card(&(devNumArr[0]));
						continue;
					}
				}
				...
				{
					uiPrintf("Dmm (current meter) is not accessible. Skipping Idle Current Consumption test\n");	
				}
			else
				uiPrintf("useInstruments set to 0 in calsetup. Skipping  Idle Current Consumption test.\n");
		}

		if(CalSetup.testContinuity) {
			timestart = milliTime();
			strcpy(testname[testnum], "Continuity Test");
			REWIND_TEST = dutTestContinuity(devNumArr[MODE_11g]);
			testtime[testnum++] = milliTime() - timestart;
			if (REWIND_TEST) 
			{
				printResultBanner(devNumArr[MODE_11g], TRUE); //mode irrelevant, passed to satisfy parameter list.
		...
		// 11a calibration
        if((CalSetup.calPower && (CalSetup.Amode || CalSetup.enableJapanMode11aNew)) && (!REWIND_TEST)){
			timestart = milliTime();
			...
			if(CalSetup.zeroAttenuator) {
				attSet(devATT,0);
			}else{
				//remove attenuation manually
			}
		...
		if(CalSetup.calPower && CalSetup.Gmode && (!REWIND_TEST)){
			...
			if(CalSetup.zeroAttenuator) {
				if ( CalSetup.attModel != NO_ATT){
					attSet(devATT,0); // zero attenuation is set the setup is such that zero attenuation is already here 
									  // attenuation only affects DUT to GU link
				}else{
					//remove attenauation manually Siavash
				}
			}
...
//    + all other information is duplicated in both EEPBLKs
       ...
       if( configSetup.remote &&
			...
			(CalSetup.calPower && (CalSetup.Amode || CalSetup.enableJapanMode11aNew)) && (!REWIND_TEST)){
				timestart = milliTime();
...
//close the instruments here and whenever control is requested by Golden Unit
	if(CalSetup.useInstruments){
		if (CalSetup.attModel != NO_ATT){
			devATT = attClose();
		}
...
//		usleep(1000*20); // sleep 20 ms
		usleep(1000*50); // sleep 150 ms - based on feedback b.
		fillZeros = FALSE ;
		fillMaxPwr = FALSE ;
...
//daniel , use function in lib, right?
#if 0
...
void dutTest(A_UINT32 *devNum)
{
	...
	A_UINT32 mode_local;
	//A_UINT32 jumbo_frame_mode_save; removed Siavash
	...
	if (CalSetup.test32KHzSleepCrystal && CalSetup.Enable_32khz) {
		devNum_local = (CalSetup.Amode || CalSetup.enableJapanMode11aNew) ? devNumArr[MODE_11a] : devNumArr[MODE_11g];
		mode_local = (CalSetup.Amode || CalSetup.enableJapanMode11aNew) ? MODE_11a : MODE_11g;
		timestart = milliTime();
...
*/
	if (CalSetup.Amode || CalSetup.enableJapanMode11aNew) {
		...
		art_resetDevice(devNumArr[MODE_11a], txStation, bssID, 5220, 0);
		...
		if(CalSetup.testChannelAccuracy) {
			if(CalSetup.zeroAttenuator) {
				if ( CalSetup.attModel != NO_ATT){
					attSet(devATT,0);
				}
			...
			timestart = milliTime();
			strcpy(testname[testnum],"11a channel accuracy");
			setYieldParamsForTest(devNumArr[MODE_11a], "11a", "channel_accuracy", "deviation", "ppm", "", ""); 
			dutTestMeasChannelAccuracy(devNumArr[MODE_11a], MODE_11a);
			testtime[testnum++] = milliTime() - timestart;
		}
		...
		if(CalSetup.testDataIntegrity[MODE_11a])
		{
			...
			dutVerifyDataTXRX(devNumArr[MODE_11a], MODE_11a, 0, 54, VERIFY_DATA_PACKET_LEN);
			if (REWIND_TEST) {
				return;
			}
			testtime[testnum++] = milliTime() - timestart;
...
*/		
		...
		if( CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11a] 
				|| CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11a])
		{
			if (CalSetup.useInstruments > 0)
			{
				if (dmmAccessible)
				{
					if(CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11a])
					{
						sendAck(devNumArr[MODE_11a], "Start 11a Current Consumption Rx Test", 0, 0, 0, CalSetup.customerDebug);
						waitForAck(CalSetup.customerDebug);
						if (REWIND_TEST)
						{
							return;
					...
					timestart = milliTime();
					strcpy(testname[testnum],"    Current Consumption Test");
					strncpy(testname[testnum], modeName[MODE_11a], 3);
					setYieldParamsForTest(devNumArr[MODE_11a], modeName[MODE_11a], "current_consumption", "current", "amps", "power", "Watt"); 
					dutMeasCurrActive(devNumArr[MODE_11a], MODE_11a);
					testtime[testnum++] = milliTime() - timestart;
				}
				else
					uiPrintf("Dmm (current meter) is not accessible.  Skipping Current Consumption test in 11a\n");
			}
			else
				uiPrintf("useInstruments set to 0 in calsetup. Skipping Current Consumption test 11a.\n");
		}

		if(CalSetup.testRxThroughput[MODE_11a])
		{
			sendAck(devNumArr[MODE_11a], "Start 11a throughput Rx test", 0, 0, 0, CalSetup.customerDebug);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST) {
				return;
			}
			...
			timestart = milliTime();
			strcpy(testname[testnum],"11a Throughput Check");
			setYieldParamsForTest(devNumArr[MODE_11a], "11a", "rx_thruput", "thruput", "mbps", "rssi", "dB"); 
			dutThroughputRxTest(devNumArr[MODE_11a], MODE_11a, 0, 54, THROUGHPUT_PACKET_SIZE, NUM_THROUGHPUT_PACKETS);
			if (REWIND_TEST) {
				return;
			}
			testtime[testnum++] = milliTime() - timestart;
	...
	if (CalSetup.Gmode) {
		dutTest_2p4(devNumArr[MODE_11g], MODE_11g);
		if (REWIND_TEST) {
			return;
		}
	}
	...
	if ((CalSetup.Bmode)  || (CalSetup.Gmode && CalSetup.doCCKin11g)) {
		dutTest_2p4(devNumArr[MODE_11b], MODE_11b);
		if (REWIND_TEST) {
			return;
		}
	}
...
void dutTestSpecMask(A_UINT32 devNum){
    ...
    A_UCHAR  antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B;//Siavash added B
	A_UINT32 cnt1;
	A_UINT16 numChannels;
...
//#endif
	...
	for(cnt1=0; cnt1<numChannels; cnt1++) {
		...
		//			art_changeChannel(devNum, channelList[cnt1]);// if figure out how to stop tx100
				...
				usleep(1000*20);		//wait 100 ms?

				if(CalSetup.maskFailPointsUseMatrix )
					threshold = pTestSet[MODE_11a]->pTestChannel[cnt1].mask_fail_threshold;
				else
					threshold = CalSetup.maskFailLimit;
				...
				while ( failedThisTest && (iter < CalSetup.maxRetestIters)) {
					...
					failedThisTest = (fail_num > threshold) ? TRUE : FALSE;
		...
		//        if (fail_num > 20)	{
				if (fail_num > threshold)	{
					TestFail = TRUE;
					failTest[testnum] = 1;
					uiPrintf(", points failed out of 400: %4d", fail_num);
					uiPrintf("  [power=%3.1f]", pwr);
					logMeasToYield((double)(channelListTimes10[cnt1])/10, 6.0, (double)iter, (double)(threshold), (double)(fail_num), (double)(pwr), "FAIL");
				} else {
					logMeasToYield((double)(channelListTimes10[cnt1])/10, 6.0, (double)iter, (double)(threshold), (double)(fail_num), (double)(pwr), "PASS");
				}
...
void ProgramMACID(A_UINT32 devNum)
{
    ...
    if( (art_eepromRead(devNum, 0x1f) == (A_UINT32)((macAddrOUI >> 8) & 0xFFFF)) && 
        ...
        if (!configSetup.remote) 
		{
			...
			if ((existing_macID2 != existing_macID1) && !isSwan(swDeviceID))
			{
...
//			if (1) {
			...
			if (CalSetup.useFastCal && CalSetup.calPower)
			{
				...
				if (CalSetup.pmModel != NO_PM)
					gpibONL(guDevPM, 0);	// take PM offline. relinquish for dut.
			...
			if(DUT_SUPPORTS_MODE[MODE_11a] && CalSetup.testDataIntegrity[MODE_11a])
			{
				...
				goldenVerifyDataTXRX(devNumArr[MODE_11a], MODE_11a, 0, 54, VERIFY_DATA_PACKET_LEN);
			}
...
*/
			...
			if(DUT_SUPPORTS_MODE[MODE_11a] && CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11a])
			{
				configSetup.eepromLoad = 1;
				art_setResetParams(devNumArr[MODE_11a], configSetup.pCfgFile, (A_BOOL)configSetup.eepromLoad,
									(A_BOOL)1, MODE_11A, configSetup.use_init);
				...
				uiPrintf("This one comes from the calling function:\n");
				sendAck(devNumArr[MODE_11a], "start Current Consumption Rx Test mode 11a", 5220, 0, 0, CalSetup.customerDebug);
				goldenMeasCurrActive(devNum, MODE_11a, THROUGHPUT_PACKET_SIZE, NUM_THROUGHPUT_PACKETS);
			}
			
			if(DUT_SUPPORTS_MODE[MODE_11a] && CalSetup.testRxThroughput[MODE_11a])
			{
				configSetup.eepromLoad = 1;
				art_setResetParams(devNumArr[MODE_11a], configSetup.pCfgFile, (A_BOOL)configSetup.eepromLoad,
									(A_BOOL)1, MODE_11A, configSetup.use_init);		
				...
				sendAck(devNumArr[MODE_11a], "start 11a Rx throughput test", 5220, 0, 0, CalSetup.customerDebug);
		
				goldenThroughputRxTest(devNumArr[MODE_11a], MODE_11a, 0, 54, THROUGHPUT_PACKET_SIZE, NUM_THROUGHPUT_PACKETS);
			}
	        if(kbhit())
		        break;
			if (REWIND_TEST) {
...
A_UINT16 parseTargets(void)
{
    ...
    while(fgets(lineBuf, 120, fStream) != NULL) {
        ...
        while(parsingTP && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			{
				pTargetsSet->pTargetChannel[ii].Target24 = tmpRead;
			}
			pLine = strtok(NULL, delimiters);
			...
			{
				pTargetsSet->pTargetChannel[ii].Target54 = tmpRead;
			}
        ...
        while(parsingTP_11g && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			{
				pTargetsSet_2p4[MODE_11g]->pTargetChannel[ii].Target24 = tmpRead;
			}
			pLine = strtok(NULL, delimiters);
			...
			{
				pTargetsSet_2p4[MODE_11g]->pTargetChannel[ii].Target54 = tmpRead;
			}
		...
		while(parsingTP_11b && (fgets(lineBuf, 120, fStream) != NULL)) {
			...
			{
				pTargetsSet_2p4[MODE_11b]->pTargetChannel[ii].Target24 = tmpRead;
			}
			pLine = strtok(NULL, delimiters);
			...
			{
				pTargetsSet_2p4[MODE_11b]->pTargetChannel[ii].Target54 = tmpRead;
			}
...
//daniel, use the functions in lib, right?
/*
A_UINT16 fbin2freq(A_UINT16 fbin)
{
	return( (fbin>62) ? (5100 + 10*62 + 5*(fbin-62)) : 5100 + 10*fbin) ;
}
...
void program_eeprom(A_UINT32 devNum)
{
	...
	if(word == NULL)
	{
			printf(" Memory not allocated in program_eeprom() \n");
			exit(1);
	}
...
//++JC++
	if (CalSetup.eeprom_map >= CAL_FORMAT_GEN5) {
		dutCalibration_gen5(devNum, mode);
		return;
	}
//++JC++
...
{
	A_UINT16 fill_zero_pcdac = 40 ;//idiotic this is Hex values written as Decimal it means 40 hex = 64 decimal 
								   // is this seriously a released code? or os it a hack of code for kids!
	A_BOOL fillZeros ;
	A_BOOL fillMaxPwr;
	A_UINT16 numPcdacs ;
	A_INT16 i;
...
//		usleep(1000*250) ;
		...
		{	
			if (CalSetup.pmModel != NO_PM){
				...
				power = saChanPwr( devSA, channel*1e6, span, -10, rbw, vbw,chbw, chsp,CalSetup.saModel, reset)+ myAtten;
			}
		} else
		{
			power = 0;
		...
		for(rr=0; rr<numPcdacs; rr++)
		{
			...
			//(mode == MODE_11g) ? usleep(1000*50) : usleep(1000*150) ;
			(mode == MODE_11g) ? usleep(50000) : usleep(150000) ;
...
void dutTest_2p4(A_UINT32 devNum,  A_UINT32 mode)
{
	...
	if (!(CalSetup.testTXPER_2p4[mode] | CalSetup.testRXSEN_2p4[mode] | 
		  ...
		  CalSetup.testDataIntegrity[mode] | CalSetup.testThroughput[mode] | CalSetup.testRxThroughput[mode]
		  | CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[mode] | CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode]))
		return;
    ...
    if(CalSetup.testChannelAccuracy_2p4[mode])
	{
		if(CalSetup.zeroAttenuator) {
			if ( CalSetup.attModel != NO_ATT){
				attSet(devATT,0);
		...
		timestart = milliTime();
		strcpy(testname[testnum],"    channel accuracy");
		strncpy(testname[testnum],modeName[mode], 3);
		setYieldParamsForTest(devNum, modeName[mode], "channel_accuracy", "deviation", "ppm", "", ""); 
		dutTestMeasChannelAccuracy(devNum, mode);
		testtime[testnum++] = milliTime() - timestart;
	}
...
//			writeField(devNum, "rf_pwd_iclobuf2G", 6);
//		}
	...
	if(CalSetup.testDataIntegrity[mode])
	{
		...
		dutVerifyDataTXRX(devNum, mode, 0, 54, VERIFY_DATA_PACKET_LEN);
		if (REWIND_TEST) {
			return;
		}
		testtime[testnum++] = milliTime() - timestart;
	...
	if(CalSetup.testThroughput[mode])
	{
		...
		dutThroughputTest(devNum, mode, 0, (A_UINT16)throughputRate, THROUGHPUT_PACKET_SIZE, 
...
*/	
	...
	if((mode == MODE_11b) && (CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11b]
			|| CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11b]))
	{
		if (CalSetup.useInstruments > 0)
		{
			if (dmmAccessible)
			{
				if(CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode])
				{
					sendAck(devNumArr[mode], "Start 11b Current Consumption Rx Test", 0, 0, 0, CalSetup.customerDebug);
					waitForAck(CalSetup.customerDebug);
					if (REWIND_TEST)
					{
						return;
				...
				strcpy(testname[testnum],"    Current Consumption Test");
				strncpy(testname[testnum], modeName[mode], 3);
				setYieldParamsForTest(devNumArr[mode], modeName[mode], "current_consumption", "current", "amps", "power", "Watt"); 
				dutMeasCurrActive(devNumArr[mode], mode);
				testtime[testnum++] = milliTime() - timestart;
			}
			else
				uiPrintf("Dmm (current meter) is not accesible.  Skipping Current Consumption Test in mode 11b\n");
		}
		else
			uiPrintf("useInstruments set to 0 in calsetup. Skipping Current Consumption test in mode 11b.\n");
	
	}

	if((mode == MODE_11g) && (CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[MODE_11g]
				|| CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[MODE_11g]))
	{
		if (CalSetup.useInstruments > 0)
		{
			if (dmmAccessible)
			{
				if(CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode]) 
				{
					sendAck(devNumArr[mode], "Start 11g Current Consumption Rx Test", 0, 0, 0, CalSetup.customerDebug);
					waitForAck(CalSetup.customerDebug);
					if (REWIND_TEST)
					{
						return;
				...
				uiPrintf("Dmm (current meter) is not accessible. Skipping Current Consumption test in mode 11g.\n");
		}
		else
			uiPrintf("useInstruments set to 0 in calsetup. Skipping Current Consumption test in mode 11g.\n");
	}

	if(CalSetup.testRxThroughput[mode])
	{
		sendAck(devNumArr[MODE_11a], "Start 2.5G throughput Rx test", 0, 0, 0, CalSetup.customerDebug);
		waitForAck(CalSetup.customerDebug);
		if (REWIND_TEST) {
			return;
		}
		...
		timestart = milliTime();
		strcpy(testname[testnum],"    Throughput Rx Check");
		strncpy(testname[testnum],modeName[mode], 3);
		setYieldParamsForTest(devNum, modeName[mode], "rx_thruput", "thruput", "mbps", "rssi", "dB"); 
		dutThroughputRxTest(devNum, mode, 0, (A_UINT16)throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
		if (REWIND_TEST) {
			return;
		}
...
//        if (fail_num > 20)	{
		...
		if(CalSetup.maskFailPointsUseMatrix > 0)
			threshold = pTestSet[mode]->pTestChannel[cnt1].mask_fail_threshold;
		else
			threshold = CalSetup.maskFailLimit;
        ...
        if (fail_num > threshold)	{
			...
			uiPrintf(", points failed out of 400: %4d", fail_num);
			uiPrintf("  [power=%3.1f]", pwr);
			logMeasToYield((double)(channelListTimes10[cnt1])/10, yldLogRate, (double)iter, (double)(threshold), (double)(fail_num), (double)(pwr), "FAIL");
//#ifndef CUSTOMER_REL
//			pwr = pmMeasAvgPower(devPM, 0) + CalSetup.attenDutPM_2p4[mode];
//			uiPrintf("  [power=%3.1f]", pwr);
//#endif
		} else {
			logMeasToYield((double)(channelListTimes10[cnt1])/10, yldLogRate, (double)iter, (double)(threshold), (double)(fail_num), (double)(pwr), "PASS");
		}
	    //art_txContEnd(devNum);
    }
...
void dutTestMeasChannelAccuracy(A_UINT32 devNum, A_UINT32 mode){
    ...
    uiPrintf("\n\nStart Channel Accuracy Test for mode %s \n", modeName[mode]);
	...
	for(cnt1=0; cnt1<numChannels; cnt1++) {
		if (!pTestSet[mode]->pTestChannel[cnt1].TEST_CHANNEL_ACCURACY)
		{
			continue; //skip test at this channel
		}
		...
		// are domain independent. 
        art_resetDevice(devNum, txStation, bssID, channelListTimes10[cnt1], 0);	// turbo=0
		art_txContBegin(devNum, CONT_SINGLE_CARRIER, 511, 511, antenna);
		yldLogRate = 6.0;
        ...
        if(CalSetup.useInstruments) {
		    //reset = 1;
		    center = channelListTimes10[cnt1] * 1e5;
			ppm = spaMeasFreqDev(devSA, center, 0, reset);
            reset = 0;
        }
        else {
            fail_num = 0;
        ...
        uiPrintf("ppm - chan:%5.1lf = %5.2lf", (double)(channelListTimes10[cnt1])/10, ppm);
//        if (fail_num > 20)	{

		  if((ppm > ppmMaxLimit)||(ppm < ppmMinLimit)) {
//        if (fail_num > CalSetup.maskFailLimit)	{
			TestFail = TRUE;
			failTest[testnum] = 1;
			uiPrintf(" (X)\n");
			if (ppm > ppmMaxLimit) limit = ppmMaxLimit;
			else limit = ppmMinLimit;
			logMeasToYield((double)(channelListTimes10[cnt1])/10, yldLogRate, (double)iter, (double)(limit), ppm, 0, "FAIL");
		} else {
			logMeasToYield((double)(channelListTimes10[cnt1])/10, yldLogRate, (double)iter, (double)(limit), ppm, 0, "PASS");
			uiPrintf("\n");
		} 
	    //art_txContEnd(devNum);
    }
...
void goldenTest_2p4(A_UINT32 devNum, A_UINT32 mode)
{
	...
	if (!(CalSetup.testTXPER_2p4[mode] | CalSetup.testRXSEN_2p4[mode] | CalSetup.testDataIntegrity[mode]
		| CalSetup.testThroughput[mode] | CalSetup.testRxThroughput[mode] 
	    | CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode]))
		return;
	...
	if(CalSetup.testDataIntegrity[mode])
	{
		...
		goldenVerifyDataTXRX(devNum, mode, 0, 54, VERIFY_DATA_PACKET_LEN);
	}
	...
	if(CalSetup.testThroughput[mode])
	{
		...
		goldenThroughputTest(devNum, mode, 0, (A_UINT16)throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
...
*/
	...
	if(CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode])
	{
		...
		sendAck(devNumArr[mode], "start 2.5G Current Consumption Rx Test", 2412, 0, 0, CalSetup.customerDebug);
		goldenMeasCurrActive(devNum, mode, THROUGHPUT_PACKET_SIZE,
					(mode == MODE_11g)? NUM_THROUGHPUT_PACKETS:NUM_THROUGHPUT_PACKETS_11B);
	}
	...
	if(CalSetup.testRxThroughput[mode])
	{
		...
		sendAck(devNum, "start 2.5G throughput Rx test", 2412, 0, 0, CalSetup.customerDebug);
		goldenThroughputRxTest(devNum, mode, 0, (A_UINT16)throughputRate, THROUGHPUT_PACKET_SIZE, 
			(mode == MODE_11g) ? NUM_THROUGHPUT_PACKETS : NUM_THROUGHPUT_PACKETS_11B);
	}
	if(kbhit())
		return;;
...
void dutTestTargetPower(A_UINT32 devNum, A_UINT32 mode)
{
	...
	A_UINT16 index, ChannelCount, rate, ii, jj, count, antBRate;
...
//	A_UINT32 rate_mask = PER_RATEMASK;
	...
	A_UINT32 antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B; //Siavash added B
	...
	double	 power, target, attenVal, attenValMain, attenValAux;
	A_UINT32 reset = 1;
	...
	A_BOOL   auxAntennaTest = FALSE;
	A_BOOL   zeroPowerTest = FALSE;
	A_BOOL   failedThisTestAntB = FALSE;
	double targetUpperLimit = CalSetup.targetPowerToleranceUpper;
	double targetLowerLimit = CalSetup.targetPowerToleranceLower;
    ...
    if (mode == MODE_11a)
	{
		...
		attenValMain = attenVal;
		attenValAux = CalSetup.attenDutPMAux;
	} else
	{
		attenVal = CalSetup.attenDutPM_2p4[mode];
		attenValMain = attenVal;
		attenValAux = CalSetup.attenDutPMAux_2p4[mode];
	}
	...
	//look up the rate for aux antenna test
	for (jj = 0; jj < PER_RATE_COUNT; jj++) {
		if (CalSetup.targetPowerAntBRate == PER_TEST_RATE[jj]) {
			antBRate = jj;
			break;
		}
	}
	if(jj == PER_RATE_COUNT) {
		uiPrintf("Error: Illegal ANTB rate specified\n");
		return;
	}
	...
	// allocate a name for antenna B test
	strcpy(testname[testnum+1],"    target pwr ctl: Antenna B");
	strncpy(testname[testnum+1],modeName[mode], 3);
	...
	uiPrintf("\nLimits:+%3.1f/-%3.1f dB,  measured/target", CalSetup.targetPowerToleranceUpper, CalSetup.targetPowerToleranceLower);
	uiPrintf("\nAnt B Limits:+%3.1f/-%3.1f dB,  measured/target", CalSetup.targetPowerToleranceUpperAntB, CalSetup.targetPowerToleranceLowerAntB);
	if (mode == MODE_11b)
	{
//		uiPrintf("\nFREQ      1Mbps       5.5Mbps(S)   11Mbps(L)    11Mbps(S) Limits:+%3.1f/-%3.1f dB", CalSetup.targetPowerToleranceUpper, CalSetup.targetPowerToleranceLower);
		uiPrintf("\nFREQ      1Mbps       5.5Mbps(S)   11Mbps(L)    11Mbps(S)   %s antB,  0dbm(1),   0dbm(11)", PER_11B_RATE_STR[antBRate]);
	} else
	{
//		uiPrintf("\nFREQ      6Mbps       36Mbps       48Mbps       54Mbps    Limits:+%3.1f/-%3.1f dB", CalSetup.targetPowerToleranceUpper, CalSetup.targetPowerToleranceLower);
		uiPrintf("\nFREQ      6Mbps       36Mbps       48Mbps       54Mbps      %dMbps antB, 0dbm(6),   0dbm(54)", PER_TEST_RATE[antBRate]);
	}
...
//	uiPrintf("\n                                                          measured/target");
	...
	for(index=0; index<ChannelCount; index++) {
        ...
        temp = (A_UINT16)PER_RATE_COUNT+2;
		for(count=0; count<=temp; count++) {
            if(count == PER_RATE_COUNT) {
				if(!pTestSet[mode]->pTestChannel[index].TEST_AUX_ANTENNA)
				{
					uiPrintf("  ----      ");
					continue;
				}

				antenna = USE_DESC_ANT | DESC_ANT_B;
				attenVal = attenValAux;
				auxAntennaTest = TRUE;
				targetUpperLimit = CalSetup.targetPowerToleranceUpperAntB;
				targetLowerLimit = CalSetup.targetPowerToleranceLowerAntB;
				rate = antBRate;
    			target = getTargetPower(mode, pTestSet[mode]->pTestChannel[index].channelValueTimes10/10, pRatesArray[rate]);
        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
            setGainLadderForIndex(pCurrGainLadder, optGainLadderIndex[mode]);
            programNewGain(pCurrGainLadder, devNum, 0);
            art_writeField(devNum, "rf_mixvga_ovr", 0);
        }
			}
			else if (count == (PER_RATE_COUNT + 1)) {
                //test for 0dbm at rate 6mbps
				if(!(pTestSet[mode]->pTestChannel[index].TEST_ZERO_POWER & 0x1))
				{
					uiPrintf("  ----     ");
					continue;
				}

				antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B;//Siavash added B
				attenVal = attenValMain;
				zeroPowerTest = TRUE;
				targetUpperLimit = CalSetup.targetPowerToleranceUpperZero;
				targetLowerLimit = CalSetup.targetPowerToleranceLowerZero;
                target = 0;
				art_forceSinglePowerTxMax(devNum, (A_UINT16)((CalSetup.minimumPower[mode]-2*CalSetup.offsetCalPower)<=0?0:(CalSetup.minimumPower[mode]-2*CalSetup.offsetCalPower)));
                rate = 0;
	        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
                setGainLadderForIndex(pCurrGainLadder, optHighGainLadderIndex[mode]);
                programNewGain(pCurrGainLadder, devNum, 0);
                art_writeField(devNum, "rf_mixvga_ovr", 0);
            }
			}
			else if (count == (PER_RATE_COUNT + 2)) {
                //test for 54dbm at rate 54mbps
				if(!((pTestSet[mode]->pTestChannel[index].TEST_ZERO_POWER  >> 1) & 0x1))
				{
					uiPrintf("  ----     ");
					continue;
				}
				...
				targetLowerLimit = CalSetup.targetPowerToleranceLowerZero;
                target = 0;
                art_forceSinglePowerTxMax(devNum, (A_UINT16)((CalSetup.minimumPower[mode]-2*CalSetup.offsetCalPower)<=0?0:(CalSetup.minimumPower[mode]-2*CalSetup.offsetCalPower))); // is  it already twice power?
                rate = (A_UINT16)PER_RATE_COUNT - 1;
	        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
                setGainLadderForIndex(pCurrGainLadder, optHighGainLadderIndex[mode]);
                programNewGain(pCurrGainLadder, devNum, 0);
                art_writeField(devNum, "rf_mixvga_ovr", 0);
				...
				rate = count;
				attenVal = attenValMain;
				auxAntennaTest = FALSE;
                zeroPowerTest = FALSE;
				targetUpperLimit = CalSetup.targetPowerToleranceUpper;
				targetLowerLimit = CalSetup.targetPowerToleranceLower;
    			target = getTargetPower(mode, pTestSet[mode]->pTestChannel[index].channelValueTimes10/10, pRatesArray[rate]);
        if((NEED_GAIN_OPT_FOR_MODE[mode]) && !isGriffin(swDeviceID) && !isEagle(swDeviceID)) {
            setGainLadderForIndex(pCurrGainLadder, optGainLadderIndex[mode]);
            programNewGain(pCurrGainLadder, devNum, 0);
...
//uiPrintf("SNOOP: using atten value = %5.2f\n", attenVal); 
			...
			art_txContBegin(devNum, CONT_FRAMED_DATA, RANDOM_PATTERN, 
						    pRatesArray[rate], antenna);

			//usleep(1000*sleep_interval);
			usleep(1000*sleep_interval);
            if(zeroPowerTest) {
                usleep(1000*CalSetup.targetPowerZeroDelay);
            }
			reset = 0; //1;
			failedThisTest = failedThisTestAntB = TRUE;
			iter = 0;
			while ((failedThisTest|| failedThisTestAntB) && (iter < CalSetup.maxRetestIters)) {
				iter++;				
				...
				failedThisTest = failedThisTestAntB = FALSE;
				if (((power - target) > targetUpperLimit) ||
					((target - power) > targetLowerLimit) )
                {
					if(count == PER_RATE_COUNT)
					{
						failedThisTestAntB |= TRUE;
					}
					else
					{
						failedThisTest |= TRUE;
			...
			} // while failedThisTest /failedThistTestAntB
			
//if(zeroPowerTest) {
//rddata = art_regRead(devNum, 0xa264);
//printf("SNOOP: pdadc = %d, gain = %d, dacgain = %d, 0xa258 = %x\n", 
//       (rddata >> 1) & 0xFF, (rddata >> 14) & 0x3f, (rddata >> 9) & 0x1f,
//       art_regRead(devNum, 0xa258));
//}
			art_txContEnd(devNum);
			...
			if ((!done_first) && (power < (target - 2.0)))
			{
				...
				art_txContBegin(devNum, CONT_FRAMED_DATA, RANDOM_PATTERN, 
								pRatesArray[rate], DESC_ANT_A | USE_DESC_ANT |DESC_ANT_B); //Siavash added B
				usleep(1000*sleep_interval);
				power = pmMeasAvgPower(devPM, reset) + attenVal;
				power = ((A_INT16)(power*10 + 0.5))/10.0 ;
				art_txContEnd(devNum);
			...
			uiPrintf("%3.1f/%3.1f", power, target);
				...
				((target - power) > targetLowerLimit) )
			{
				...
				uiPrintf("(X) ");
				logMeasToYield((double)(channelTimes10[index])/10, (double)PER_TEST_RATE[rate], (double)CalSetup.pmModel, (double)target, (double)power, (double)maxLinPwr, "FAIL");
			} else
			{
				uiPrintf("    ");
				logMeasToYield((double)(channelTimes10[index])/10, (double)PER_TEST_RATE[rate], (double)CalSetup.pmModel, (double)target, (double)power, (double)maxLinPwr, "PASS");
			}
			
			done_first = TRUE;

			if (auxAntennaTest) break;
		}	
	}
	uiPrintf("\n");
	...
	testnum++; //to keep count correct for next test, after we inserted one for Ant B
}
...
*/		if((strnicmp("A_MODE", pLine, strlen("A_MODE")) == 0) &&
		...
		if((strnicmp("ENABLE_JAPAN_MODE_11A_NEW", pLine, strlen("ENABLE_JAPAN_MODE_11A_NEW")) == 0) &&
					((pLine[strlen("ENABLE_JAPAN_MODE_11A_NEW")] == ' ') || 
					 (pLine[strlen("ENABLE_JAPAN_MODE_11A_NEW")] == '\t') ) ){
				pLine = strchr(pLine, '=');
				pLine = strtok(pLine, delimiters);
				if(!sscanf(pLine, "%d", &tempVal)) {
					...
					if (tempVal > 0) uiPrintf(" 11a ");
					if(tempVal) {
						DUT_SUPPORTS_MODE[MODE_11a] = TRUE;
					}
				}
			}
...
void dutTestTXRX(A_UINT32 devNum, A_UINT32 mode)
{
    ...
    TX_STATS_STRUCT tStats;  //added by Dudi for RMA7265
	// txDataSetup
	A_UINT32 rate_mask = RATE_GROUP | PER_RATEMASK;
	A_UINT32 turbo = 0;
    ...
    if(CalSetup.useInstruments) {
		if ( CalSetup.attModel != NO_ATT){ 
			...
			//Make sure the combined attenuation is equal to 
			//goldenTXPowerLocal - attenDutGoldenLocal - pTestSet[mode]->pTestChannel[0].targetSensitivity 
		}
    }
	...
	if (TX_PER_STRESS_MARGIN > 0) {
		...
		if(mode != MODE_11b) {
			if(!CalSetup.goldenPPMDisable) {
				uiPrintf("\nFREQ          6Mbps      36Mbps      48Mbps      54Mbps     ppm    ():RSSI\n");
			} else {
				uiPrintf("\nFREQ          6Mbps      36Mbps      48Mbps      54Mbps            ():RSSI\n");
			}
		} else
		{
...
*/
	...
	// need to reset the params changed by SEN test for PER for each iteration
				...
				complete_timeout = ((mode == MODE_11b) ? 1000 : 500);
...
*/
						/* add tx status check for RMA7265*/
						art_txGetStats(devNum, per_test_rates_local[rate], 0, &tStats);
						if(tStats.goodPackets < (A_UINT16)(PER_TEST_FRAME*CalSetup.perPassLimit/100)){
						/*	if (CalSetup.customerDebug) */
								uiPrintf("Tx Good packets %d < %d\n", tStats.goodPackets, (A_UINT16)(PER_TEST_FRAME*CalSetup.perPassLimit/100));
								failedThisTest |= TRUE;	
						}
...
//					if ((mode != MODE_11b) && (turbo != TURBO_ENABLE) && (turbo != HALF_SPEED_MODE))
					...
					if ((mode == MODE_11a) && (TX_PER_STRESS_MARGIN == 0) && !CalSetup.goldenPPMDisable)
					{
...
*/
				...
				if ((mode == MODE_11g) && (TX_PER_STRESS_MARGIN == 0) && !CalSetup.goldenPPMDisable ){ // do ppm test 
					iter = 0;
					failedThisTest = TRUE;
				...
				if ((mode != MODE_11b) && (TX_PER_STRESS_MARGIN == 0) && !CalSetup.goldenPPMDisable)
				{
...
//				if (1)
				...
				complete_timeout = 1500 + num_rx_desc;
				...
				while ( failedThisTest && (iter < CalSetup.maxRetestIters)) {
					...
					art_setAntenna(devNum, (USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B)); // Siavash added B
					art_rxDataSetup(devNum, num_rx_desc, rx_length, enable_ppm);
...
//					if (!testPERLocal && !pTestSet[mode]->pTestChannel[index].TEST_PER)
					...
					{
                        if(do_11g_CCK) {
							// dont seem to be right does it store rate  this is only pointer not value
							// Crapy programming ART style remark by Siavash
							#pragma warning(suppress: 4047) 
                            senRates[kk] = LO_SEN_TEST_RATE_11G_CCK;    
                        } else {
...
//		turbo = turbo_iter ;	
	...
	// reset params changed by SEN test for PER for each iteration
				...
				art_forceSinglePowerTxMax(devNum, (A_UINT16)(2*(20))); // send stats at 20dBm
																	   //-CalSetup.offsetCalPower
				...
				if ((mode == MODE_11g) && (TX_PER_STRESS_MARGIN == 0) && !CalSetup.goldenPPMDisable ){ // do ppm measurement
					enable_ppm = 1;
	...
	// turn around for rxsen test
				complete_timeout = (mode == MODE_11b) ? 1500 : 1000;
				...
				art_forceSinglePowerTxMax(devNum, (A_UINT16)(2*(goldenTXPowerLocal-CalSetup.offsetCalPower)));
				...
				while (repeatThisTest) {
					...
					//Siavash Test for power measure
	/*				for (i=0;i < 1000;i++){
						art_txDataSetup(devNum,rate_mask,txStation,num_desc_per_rate,tx_length,pattern,2,retry,antenna, broadcast);
						art_txDataBegin(devNum, complete_timeout, stats_mode);
					}*/
...
void test_for_temp_margin(A_UINT32 devNum, A_UINT32 mode)
{
	...
	for (ii=0; ii<pTestSet[mode]->numTestChannelsDefined; ii++)
	{
		...
		if (mode == MODE_11b)
		{
			//usleep(1000*150); // sleep 350 ms
			usleep(150000);
		} else
		{
			//(CalSetup.pmModel == PM_436A) ? usleep(1000*150) : usleep(1000*50);  //sleep 250 ms
			(CalSetup.pmModel == PM_436A) ? usleep(150000) : usleep(50000);
		}
...
A_BOOL prepare_for_next_card (A_UINT32 *p_devNum)
{
	...
	if(configSetup.enableLabelScheme) {
		uiPrintf("\n==============================================");
		uiPrintf("\nTest complete. Remove DUT (Device Under Test)");
		uiPrintf("\nPress any key to start or <ESC> to quit");
		uiPrintf("\n==============================================\n");
   		while (!kbhit());
		...
		{			
			...
			if(strncmp(strupr(yldStruct.cardLabel), "AP", 2)==0) {
			    ...
			    if(strncmp(strupr(yldStruct.cardLabel), "UB", 2)==0) {
				   configSetup.remote = 1;
				   configSetup.primaryAP = 1;
				}
				else {
		...
		uiPrintf("\n==============================================");
		uiPrintf("\nTest complete. Replace DUT (Device Under Test)");
		uiPrintf("\nPress any key to start or <ESC> to quit");
		uiPrintf("\n==============================================\n");
   		while (!kbhit())
		;
	...
	do_11g_CCK = FALSE;
	if ((exitLoop != TRUE) && (configSetup.remote)) 
	{
		...
		if  ((strnicmp((const char *)resp, "USB", 3) == 0)   || 
					(  (strnicmp((const char *)resp, "y", 1) == 0) && 
						(strnicmp((const char *)configSetup.machName, "USB", 3) == 0)
					)
			) {
...
// must've obtained card label etc.. by now
	...
	if(EEPROM_DATA_BUFFER != NULL){
		for (i = 0; i < 2; i++) {
			EEPROM_DATA_BUFFER[i]= (A_UINT32 *)malloc(sizeof(A_UINT32) * eepromSize);
				if(EEPROM_DATA_BUFFER[i] == NULL){
						printf(" Memory Not allocated in calibrationMenu() eep size =%x\n",eepromSize);
						exit(1);
				}
			}
		}
	else {
		printf(" Memory Not allocated in prepare for next card() \n");
		exit(1);
	}
...
A_UINT32 optimal_fixed_gain(A_UINT32 devNum, GAIN_OPTIMIZATION_LADDER *ladder, A_UINT32 mode) 
{
	...
	for(ii=0; ii<numCh; ii++) {
		...
		art_resetDevice(devNum, rxStation, bssID, channels[ii], 0);
...
void dutVerifyDataTXRX(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT32 frameLen)
{
	...
	if(!isGriffin(swDeviceID) && !isEagle(swDeviceID) && !isCobra(swDeviceID)) {
		art_writeField(devNum, "bb_max_rx_length", 0xFFF);
		art_writeField(devNum, "bb_en_err_length_illegal", 0);
	}
    ...
    // setup attenuator here
    if(CalSetup.useInstruments) {		
		if ( CalSetup.attModel != NO_ATT){//set attenuation manually
	...
	for (ii=0; ii<ChannelCount; ii++)
	{
		channelTimes10[ii] = testChannel*10;
	}
	...
	uiPrintf("Pattern :   inc    0xFF   0xAA   0x55   0x66   0x99  walk1  walk0\n");
	...
	for(index=0; index<ChannelCount; index++) {
		...
		if(testDataTXLocal) 
		{
			...
			rx_length = 100;
			start_timeout = 0;	// no wait time for transmit
			complete_timeout = ((mode == MODE_11b) ? 5000 : 500);
			stats_mode =  ENABLE_STATS_RECEIVE; // ENABLE_STATS_SEND |
...
// turn around for		
		...
		if (testDataRXLocal)
		{
			...
			for (ll = 0; ll < 1; ll++) {
				uiPrintf("\nTesting for Packet length %d (+ 6 for MDK header)\n",
					rx_length + ll);
				uiPrintf("             ");
				for (kk = 0; kk < NUM_VERIFY_DATA_PATTERNS; kk++) {
					...
					while ( failedThisTest && (iter < CalSetup.maxRetestIters)) {
						...
						art_setAntenna(devNum, (USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B)); //Siavash Added B
						art_rxDataSetup(devNum, num_rx_desc, rx_length+ll, enable_ppm);
//				        art_rxEnableDecompression(devNum, rxStation,63);					
						waitForAck(CalSetup.customerDebug);
						if (REWIND_TEST) {
							return;
						}
						...
						if(rStats.bitErrorCompares || rStats.decompressionErrors){
//							uiPrintf("SNOOP: errorCompares = %d, decompressionErrors = %d\n",
//								rStats.bitErrorCompares, rStats.decompressionErrors);
//							printf("snoop: crcErrors = %d, decryptErrors = %d\n",
//								rStats.crcPackets, rStats.decrypErrors);
						}
			...
			} //end ll loop over packet lengths
...
{
	A_UINT32 *channelTimes10;
	A_UINT16 index, ChannelCount, ii, kk, ll;
	...
	A_UINT32 retry = 0;		// broadcast mode, disable retry
    A_UINT32 antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B; //Siavash Added B
	A_UINT32 broadcast = 0;	// broadcast mode
	...
	for(index=0; index<ChannelCount; index++) {
		...
		if (testDataTXLocal)
		{
			...
			art_forceSinglePowerTxMax(devNum, (A_UINT16)(2*(20))); // send stats at 20dBm take note of offset introduced
																   //-CalSetup.offsetCalPower
		...
		if (testDataRXLocal)
		{
			...
			art_forceSinglePowerTxMax(devNum, (A_UINT16)(2*(goldenTXPowerLocal-CalSetup.offsetCalPower)));
//          configSetup.enableCompression = TRUE;
            configSetup.enableCompression = FALSE;
            art_configureLibParams(devNum);	
			...
			for (ll = 0; ll < 1; ll++) {
				for (kk=0; kk<NUM_VERIFY_DATA_PATTERNS; kk++) {
					...
					repeatThisTest = TRUE;
					while (repeatThisTest) {
						art_txDataSetup(devNum,rate_mask,txStation,num_desc_per_rate,tx_length+ll,verifyDataPattern[kk],LEN_VERIFY_DATA_PATTERNS,retry,antenna, broadcast);
						if (CalSetup.customerDebug > 0) {
...
void printTestEndSummary() {
	...
	if(TestFail) {
        ...
        if(toupper(getchar()) == 'Y') {
			...
			sprintf(filename, "%04X_%04X_%04X_epb",
				art_eepromRead(devNumArr[MODE_11a], 0x1f),
					art_eepromRead(devNumArr[MODE_11a], 0x1e), 
						art_eepromRead(devNumArr[MODE_11a], 0x1d));
			if(!backupEeprom(devNumArr[MODE_11a], filename)) {
					   ...
					   A_UINT32 numPackets)
{
	...
	A_UINT32 tx_length = frameLen;
	A_UINT32 retry = CalSetup.numRetriesThruput; //num retries configurable in calsetup
	A_UINT32 antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B; //Siavash Addded B
	A_UINT32 broadcast = 0;
    ...
    A_UCHAR *goldenStation = rxStation;
	...
	for(index=0; index<numChannels; index++) {
        if (index < NUM_THROUGHPUT_ADDRS) {
            goldenStation = throughputStationIDs[index];
        } else {
            goldenStation = throughputStationIDs[NUM_THROUGHPUT_ADDRS - 1];
        }
...
//			art_writeField(devNum, "mc_trig_level", 0x3f);
		...
		while ( failedThisTest && (iter < CalSetup.maxRetestIters)) {
			...
			art_txDataSetup(devNum, rate_mask | RATE_GROUP, goldenStation, 
						  ...
						  A_UINT32 numPackets)
{
	...
	A_UINT32 tx_length = frameLen;
	A_UINT32 retry = CalSetup.numRetriesThruput;		
    A_UINT32 antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B; //Siavash Added B
	A_UINT32 broadcast = 0;
	A_UCHAR  pattern[] = {0xaa, 0x55};
	...
	for(index=0; index<numChannels; index++) {
		...
		if(1)
		{
			art_resetDevice(devNum, goldenStation, bssID, *(channelTimes10+index), turbo);
			done_reset = TRUE;
		} else
		{
			art_changeChannel(devNum, *(channelTimes10+index));
...
void dutThroughputRxTest(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT16 frameLen, 
					   A_UINT32 numPackets)
{
	A_UINT32 *channelTimes10;
	A_UINT16 index, ii, numChannels;
	...
	// rxDataSetup
	A_UINT32 num_rx_desc = numPackets + 50;
	A_UINT32 rx_length = frameLen;
	A_UINT32 enable_ppm = 0;

	// txrxDataBegin
	A_UINT32 start_timeout = 7000;
	A_UINT32 complete_timeout = 500;
	A_UINT32 stats_mode =  ENABLE_STATS_RECEIVE;
	A_UINT32 compare = 0;
	A_UCHAR  pattern[] = {0xaa, 0x55};
	...
	if(do_11g_CCK) {
		uiPrintf("\nStart Throughput Rx Test in 11g CCK mode\n");
	} else {
    	uiPrintf("\nStart Throughput Rx Test in %s mode\n", modeName[mode]);
    }
	numChannels = pTestSet[mode]->numTestChannelsDefined;
	channelTimes10 = (A_UINT32 *)malloc(numChannels*sizeof(A_UINT32)) ;
	if(NULL == channelTimes10) {
	...
	uiPrintf("Channel Good  Throughput  Good  RSSI  CRCs  Missed  Result  Rate\n");
	uiPrintf("        (Rx)              (TX)  (RX)  (RX)\n");

	sprintf(testname_local, "thruput_rx%s", ((turbo==0) ? "" : ( (turbo==1) ? "_turbo" : ( (turbo==HALF_SPEED_MODE) ? "_halfspeed" : ""))));
	setYieldParamsForTest(devNum, modeName[mode], testname_local, "thruput", "mbps", "per", "percent"); 
	...
	for(index=0; index<numChannels; index++) {
		...
		if(turbo) {
			...
			testThisChannel = pTestSet[mode]->pTestChannel[index].TEST_THROUGHPUT_RX;
		}
		if (testThisChannel < 1)
		{
			continue; // skip Throughput test for this channel unless requested
		...
		while ( failedThisTest && (iter < CalSetup.maxRetestIters)) {
			...
			art_rxDataStart(devNum);
			sendAck(devNum, "ThroughputRxTest: Prepare to transmit", *(channelTimes10+index), 0, 0, CalSetup.customerDebug);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST) {
				return;
			}
		...
		//	art_txGetStats(devNum, 0, remoteStats, &tStats);
			...
			//#################Get the rate related stats and print also
			art_rxGetStats(devNum, rate, 0, &rRateStats);
			art_txGetStats(devNum, rate, 1, &tRateStats);
			missedPackets = numPackets - (rRateStats.goodPackets + rRateStats.crcPackets);
			if(turbo) {
//				passThreshold = pTestSet[mode]->pTestChannel[index].throughputTurboThreshold;
			} else {
				passThreshold = pTestSet[mode]->pTestChannel[index].throughputRxThreshold;
			}
			if ((float)rRateStats.newThroughput/1000 < passThreshold) {
				failedThisTest |= TRUE;
			}
			...
			channel_repeat = (failedThisTest && (iter < CalSetup.maxRetestIters)) ? 1 : 0 ;
			sendAck(devNum, "ThroughputRxTest: Repeat This Channel - see par2", *(channelTimes10+index), channel_repeat, 0, CalSetup.customerDebug);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST) {
				return;
			}
		...
		uiPrintf("% 4d     %5.1f   % 4d  % 4d  % 4d  % 4d",
			rRateStats.goodPackets, 
			(float)rRateStats.newThroughput/1000,
			tRateStats.goodPackets, 
			rRateStats.DataSigStrengthAvg,
			rRateStats.crcPackets,
			missedPackets);
		...
		if(failedThisTest) {
			...
			failTest[testnum] = 1;
			uiPrintf("       X:");
			logMeasToYield((double)(channelTimes10[index])/10, (double)(rate), (double)(frameLen), (double)(pTestSet[mode]->pTestChannel[index].throughputThreshold), ((double)(rRateStats.newThroughput)/1000.0), ((double)(rRateStats.goodPackets)*100.0/numPackets), "FAIL");
		}
		else {
			uiPrintf("         ");
			logMeasToYield((double)(channelTimes10[index])/10, (double)(rate), (double)(frameLen), (double)(pTestSet[mode]->pTestChannel[index].throughputThreshold), ((double)(rRateStats.newThroughput)/1000.0), ((double)(rRateStats.goodPackets)*100.0/numPackets), "PASS");
		}
		if(mode != MODE_11b) {
			uiPrintf("   %s", DataRateStr[dataRateIndex]);
			if(turbo == TURBO_ENABLE) {
...
void goldenThroughputRxTest(A_UINT32 devNum, A_UINT32 mode, A_UINT32 turbo, A_UINT16 rate, A_UINT16 frameLen,
						  A_UINT32 numPackets)
{
	A_UINT32 *channelTimes10;
	A_UINT16 index, numChannels, ii, kk;
	...
	uiPrintf("\n\nStart Throughput Rx test for %s mode \n", modeName[mode]);
...
//			testThisChannel = pTestSet[mode]->pTestChannel[index].TEST_TURBO_THROUGHPUT;
		...
		// reset params changed by testDataRXLocal test for testDataTXLocal for each iteration
		complete_timeout = (mode == MODE_11b) ? 5000 : 700;
		stats_mode = ENABLE_STATS_SEND ; //| ENABLE_STATS_RECEIVE;
		...
		art_forceSinglePowerTxMax(devNum, (A_UINT16)((2*(goldenTXPowerLocal-CalSetup.offsetCalPower)) <= 0? 0:(2*(goldenTXPowerLocal-CalSetup.offsetCalPower))));
		
		repeatThisTest = TRUE;
		while (repeatThisTest) {
			art_txDataSetup(devNum, rate_mask | RATE_GROUP, txStation, 
				num_tx_desc, tx_length, pattern, 2, retry, antenna, broadcast);
			...
			start_timeout = 5000;  // make it big as starting to rx before sending ack over ethernet.					
			sendAck(devNum, "ThroughputTest: Ready to transmit ", *(channelTimes10+index), 0, 0, CalSetup.customerDebug);
			usleep(1000*100);
			art_txDataBegin(devNum, start_timeout, stats_mode);
			
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST) {
				return;
			}
			if (!verifyAckStr("ThroughputRxTest: Repeat This Channel - see par2")) {
				uiPrintf("ERROR: miscommunication in sync. expected ack string: ThroughputTest: Repeat This Channel - see par2\n");
				exit(0);
			}
			repeatThisTest = (ackRecvPar2 > 0) ? TRUE : FALSE ;
			sendAck(devNum, "ThroughputRxTest: Repeat This Channel - saw par2", *(channelTimes10+index), ackRecvPar2, 0, CalSetup.customerDebug);
		}
...
{
	A_UINT16	i;
	A_BOOL temp = CalSetup.customerDebug;
  FILE *fStream;
	A_UINT32 length;
...
A_BOOL dutMeasCurrIdle(const int ud, const int model, A_UINT32 devNum)
{
	A_UINT32 sleep_state = 0;
	double current;
	A_BOOL failThisTest;
	A_BOOL failHi; //type of failure
	char *str1 = "";
	char *str2 = "";

	uiPrintf("\nTesting Idle Current Consumption...[mA]\n");
	...
	current = fabs(dmmMeasCurr(ud, model));
	if((current < CalSetup.CURR_CNSMPN_STTNGS.idleHi*.001) && (current > CalSetup.CURR_CNSMPN_STTNGS.idleLo*.001))
	{
		failThisTest = FALSE;
	}	
	else
	{
		...
		TestFail = TRUE;
		failTest[testnum] = 1;
		if (current > CalSetup.CURR_CNSMPN_STTNGS.idleHi*.001)
			failHi = TRUE;
		else
			failHi = FALSE;
	}
	...
	uiPrintf("Consumption: %.0f\tHigh Limit: %.0f%s   Low Limit: %.0f%s\n",
		current*1000, CalSetup.CURR_CNSMPN_STTNGS.idleHi, str1, 
			CalSetup.CURR_CNSMPN_STTNGS.idleLo, str2);

	return failThisTest; //returns true if failed
	

}

#define GPIO_CONFIGURE_REG 0x4014
#define GPIO_OUTPUT_REG    0x4018
#define GPIO_INPUT_REG     0x401c

static A_BOOL dutTestContinuity(A_UINT32 devNum)
{
	A_UINT32 gpioConfigure = 0;
	A_UINT32 gpioTest = 0;
	A_UINT32 gpioResult = 0;

	//testing LED(output) and wlan_disable(input), pins should be shorted 
	//together on extender card
	
	//configure the gpios as needed
	configureGPIOOutput(devNum, (A_UINT16)CalSetup.continuityOutputGpio);
	configureGPIOInput(devNum, (A_UINT16)CalSetup.continuityInputGpio);

	//write 1 to output GPIO ...
	driveGPIO(devNum, (A_UINT16)CalSetup.continuityOutputGpio, 1);
	
	//...and verify input GPIO sees it
	gpioResult = readGPIO(devNum, (A_UINT16)CalSetup.continuityInputGpio);

	if(gpioResult == 0) {
		uiPrintf("Continuity test failed, expected 1 on input gpio, got %d\n",
			gpioResult);
		return TRUE;
	}

	//write 0 to output GPIO ...
	driveGPIO(devNum, (A_UINT16)CalSetup.continuityOutputGpio, 0);

	//...and verify input GPIO sees it
	gpioResult = readGPIO(devNum, (A_UINT16)CalSetup.continuityInputGpio);

	if(gpioResult == 1) {
		uiPrintf("Continuity test failed, expected 0 on input gpio, got %d\n",
			gpioResult);
		return TRUE;
	}
	...
	if(gpioResult == 0) {
		uiPrintf("Continuity test failed, expected 1 on gpio0, got %d\n",
			gpioResult);
		return TRUE;
	}
...
void dutMeasCurrActive(A_UINT32 devNum, A_UINT32 mode)
{

	A_UINT32  numChannels;
	A_UINT32  *channelTimes10;
	A_UINT32  *channel_Times_10; //separate variable for the rx flow
	A_UINT32  i, ii, j, iter;
	A_UINT32  sleep_interval = 50;
	A_UINT32  antenna = DESC_ANT_A | USE_DESC_ANT | DESC_ANT_B; //Siavash Added B
	A_UINT32  start_timeout = 0;
	//A_UINT32  start_timeout = 0;//for art_rxDataComplete
	A_UINT32  complete_timeout = 500;
	// shouldn't stats_mode be DISABLE in our case?
	A_UINT32  stats_mode =  ENABLE_STATS_RECEIVE;
	A_UINT32  compare = 0;
	A_UINT16  channel_repeat;
	A_UINT32  attenVal;
	A_UINT32  atten_backoff = 10;
	double    current;  
	double    yldLogRate;
	A_BOOL	  failedThisTest = FALSE;
	A_BOOL    turbo = FALSE;
	A_BOOL	  printedHeaderTx = FALSE;
	A_BOOL	  printedHeaderRx = FALSE;
	RX_STATS_SNAPSHOT rxStatsSnapshot;
	A_UINT32  rate = 6;


	if (CalSetup.useInstruments < 1)
	{
		uiPrintf("useInstruments set to 0 in calsetup. Skipping Current Consumption test.\n");
		return;
	}

	if(do_11g_CCK) {
		rate = 0xb1;
		uiPrintf("\n\nStarting Current Consumption Test in mode 11g CCK  [mA]");
	} else {
    	uiPrintf("\n\nStarting Current Consumption Test in mode %s  [mA]", modeName[mode]);
    }
	yldLogRate = 6.0;
	numChannels = pTestSet[mode]->numTestChannelsDefined;
	channelTimes10 = (A_UINT32 *)malloc(numChannels*sizeof(A_UINT32)) ;
	if(NULL == channelTimes10) {
		uiPrintf("Error: unable to alloc mem for channel list in current consumption test Tx\n");
		return;
	}
	for (ii=0; ii<numChannels; ii++)
	{
	...
	if(CalSetup.CURR_CNSMPN_STTNGS.txTestEnable[mode] ) // tx
	{
		uiPrintf("\nTesting consumption in tx:\n");
		
		for (i = 0; i < numChannels; i++)
		{
			if (pTestSet[mode]->pTestChannel[i].TEST_CURR_CNSMPN_TX < 1)
			{
				continue; //skip current consumption test in tx at this channel
			}
			else
			{	
				art_resetDevice(devNum, rxStation, bssID, channelTimes10[i], turbo);
				art_txContBegin(devNum, CONT_FRAMED_DATA, REPEATING_5A , (A_UINT32)yldLogRate, antenna);
				usleep(1000*sleep_interval);
				failedThisTest = TRUE;
				iter = 0;
				while (failedThisTest && (iter < CalSetup.maxRetestIters))
				{
						iter++;
						current = fabs(dmmMeasCurr(devDMM, CalSetup.dmmModel)); //amps
						failedThisTest = FALSE;
						if(current*1000 < CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_TX][mode]
							||  current*1000 > CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_TX][mode])
						{
							failedThisTest = TRUE;
							usleep(1000*sleep_interval);
						}
				...
				if (!printedHeaderTx)
				{
					uiPrintf("Channel   Consumption\n");
					printedHeaderTx = TRUE;
				}
				uiPrintf("%4.0f\t%5.0f", (double)(channelTimes10[i])/10, current*1000);
				if(failedThisTest)
				{
					TestFail = TRUE;
					failTest[testnum] = 1;
				...
				uiPrintf("\n");
				if(current*1000 > CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_TX][mode])
					logMeasToYield( (double)(channelTimes10[i])/10, 1, (double)iter, current, 1, 
										CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_TX][mode], "FAIL");
				else if(current*1000 < CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_TX][mode])
					logMeasToYield( (double)(channelTimes10[i]/10), yldLogRate, (double)iter, current, 1,
										CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_TX][mode], "FAIL");
				else
					logMeasToYield( (double)(channelTimes10[i]/10), yldLogRate, (double)iter, current, 1,
										CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_TX][mode], "PASS");
			}
		} //for
		//uiPrintf("\n\n");
		if (channelTimes10)
	...
	if(CalSetup.CURR_CNSMPN_STTNGS.rxTestEnable[mode]) //rx		
	{
		uiPrintf("\nTesting consumption in rx:\n");
		channel_Times_10 = (A_UINT32 *)malloc(numChannels*sizeof(A_UINT32)) ;
		if(NULL == channel_Times_10)
		{
		uiPrintf("Error: unable to alloc mem for channel list in current consumption test Rx\n");
		return;
		}
		for (ii=0; ii<numChannels; ii++)
		{
			channel_Times_10[ii] = pTestSet[mode]->pTestChannel[ii].channelValueTimes10;
		}

		// set fixed attenuation, for consistent results
		if ( CalSetup.attModel != NO_ATT){
			attenVal = (A_INT32)(getTargetPower(mode, pTestSet[mode]->pTestChannel[ii].channelValueTimes10/10,6) 
							- CalSetup.attenDutGolden - pTestSet[mode]->pTestChannel[ii].targetSensitivity - atten_backoff) ;
			attenVal -= (turbo == 0) ? 0 : 4;
			attSet(devATT, attenVal);

		}else{
			//Set attenuation manually Siavash
		}
		for (j = 0; j < numChannels; j++)
		{
			if ( pTestSet[mode]->pTestChannel[j].TEST_CURR_CNSMPN_RX < 1 )
			{
				continue; //skip current consumption test in rx at this channel
			}
			else
			{
				failedThisTest = TRUE;
				memset(&rxStatsSnapshot, 0, sizeof(RX_STATS_SNAPSHOT));
				iter = 0;
				//cleanup descriptor queues, to free up mem
				art_cleanupTxRxMemory(devNum, TX_CLEAN | RX_CLEAN);
				art_resetDevice(devNum, rxStation, bssID, channel_Times_10[j], turbo);
				while (failedThisTest && (iter < CalSetup.maxRetestIters))
				{
					iter++;
					...
					//art_rxDataSetup(devNum, num_rx_desc, rx_length, enable_ppm);
					art_rxDataStart(devNum);
					sendAck(devNum, "Current Consumption Rx Test: Prepare to transmit", channel_Times_10[j], 0, 0, CalSetup.customerDebug);
					waitForAck(CalSetup.customerDebug);
					if (REWIND_TEST)
					{
						return;
					}
					if(!verifyAckStr("Current Consumption Rx Test: Ready to transmit"))
					{
						uiPrintf("ERROR: miscommunication in sync. expected ack string: Current Consumption Rx Test: Ready to transmit\n");
						exit(0);
					}
					usleep(1000*100);
					current = fabs(dmmMeasCurr(devDMM, CalSetup.dmmModel)); //amps
					failedThisTest = FALSE;
					if(current*1000 < CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_RX][mode]
						||  current*1000 > CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_RX][mode])
					{
						failedThisTest = TRUE;
						usleep(1000*sleep_interval);
					}
					if (art_mdkErrNo != 0) 
					{
						art_resetDevice(devNum, txStation, bssID, channel_Times_10[j], turbo);
					}
					channel_repeat = (failedThisTest && (iter < CalSetup.maxRetestIters)) ? 1 : 0 ;
					sendAck(devNum, "Current Consumption Rx Test: Repeat This Channel - see par2", channel_Times_10[j], channel_repeat, 0, CalSetup.customerDebug);
					waitForAck(CalSetup.customerDebug);
					if (REWIND_TEST)
					{
						return;
					}
					if(!verifyAckStr("Current Consumption Rx Test: Repeat This Channel - saw par2"))
					{
						uiPrintf("ERROR: miscommunication in sync. expected ack string: Current Consumption Rx Test: Repeat This Channel - saw par2\n");
						exit(0);
					}
					if (CalSetup.customerDebug) uiPrintf("SNOOP: Current Consumption Rx Test repeat iter = %d \n", iter);
				} // while failedThisTest
				...
				{
					uiPrintf("Channel   Consumption\n");
					printedHeaderRx = TRUE;
				}
				uiPrintf("%4.0f\t%5.0f", (double)(channel_Times_10[j])/10, current*1000);
				if(failedThisTest)
				{
					TestFail = TRUE;
					failTest[testnum] = 1;
				...
				uiPrintf("\n");
					if(current*1000 > CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_RX][mode])
					logMeasToYield( (double)(channel_Times_10[j])/10, 1, (double)iter, current, 1, 
										CalSetup.CURR_CNSMPN_STTNGS.hi[CURR_CNSMPN_RX][mode], "FAIL");
				else if(current*1000 < CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_RX][mode])
					logMeasToYield( (double)(channel_Times_10[j]/10), yldLogRate, (double)iter, current, 1,
										CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_RX][mode], "FAIL");
				else
					logMeasToYield( (double)(channel_Times_10[j]/10), yldLogRate, (double)iter, current, 1,
										CalSetup.CURR_CNSMPN_STTNGS.lo[CURR_CNSMPN_RX][mode], "PASS");
			} //else
		} //for
	uiPrintf("\n\n");
	}
	if(channel_Times_10)
	{
		free(channel_Times_10);
	}
	return;
}
	
void goldenMeasCurrActive(A_UINT32 devNum, A_UINT32 mode, A_UINT32 numPackets, A_UINT32 frameLen)
{
	A_UINT32  *channelTimes10;
	A_UINT16  i, numChannels, ii;
	A_UINT32  antenna = USE_DESC_ANT | DESC_ANT_A | DESC_ANT_B; // Siavash added B
	A_UINT32  broadcast = 0;
	A_UINT32  retry = CalSetup.maxRetestIters;
	A_UINT32  num_tx_desc = numPackets;
	A_UINT32  start_timeout = 5000;
	A_UINT32  tx_length = frameLen;
	A_UINT32  stats_mode =  ENABLE_STATS_RECEIVE;
	...
	if(do_11g_CCK) {
		uiPrintf("\n\nStart Current Consumption Rx test for 11g CCK mode \n");
		rate = 0xb1;
	} else {
    	uiPrintf("\n\nStart Current Consumption Rx test for %s mode \n", modeName[mode]);
    }
	numChannels = pTestSet[mode]->numTestChannelsDefined;
	channelTimes10 = (A_UINT32 *)malloc(numChannels*sizeof(A_UINT32)) ;
	if(NULL == channelTimes10) {
	...
	for (i=0; i < numChannels; i++)
	{
		if(pTestSet[mode]->pTestChannel[i].TEST_CURR_CNSMPN_RX < 1 )
		{
			continue; //skip current consumption test at this channel
		}
		//cleanup descriptor queues, to free up mem
		art_cleanupTxRxMemory(devNum, TX_CLEAN | RX_CLEAN);
		repeatThisTest = TRUE;
		...
		{
			art_resetDevice(devNum, rxStation, bssID, channelTimes10[i], turbo);
			art_txDataSetup(devNum, (A_UINT32)rate, txStation, num_tx_desc, tx_length, pattern, 2, retry, antenna, broadcast);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST)
			{
				return;
			...
			start_timeout = 5000;  // make it big as starting to rx before sending ack over ethernet.
			sendAck(devNum, "Current Consumption Rx Test: Ready to transmit", channelTimes10[i], 0, 0, CalSetup.customerDebug);
			art_txContBegin(devNum, CONT_FRAMED_DATA, REPEATING_5A, (A_UINT32)rate, antenna);
			waitForAck(CalSetup.customerDebug);
			if (REWIND_TEST)
			{
				return;
			}
			if (!verifyAckStr("Current Consumption Rx Test: Repeat This Channel - see par2"))
			{
				uiPrintf("ERROR: miscommunication in sync. expected ack string: Current Consumption Rx Test: Repeat This Channel - see par2\n");
				exit(0);
			}
			repeatThisTest = (ackRecvPar2 > 0) ? TRUE : FALSE ;
			sendAck(devNum, "Current Consumption Rx Test: Repeat This Channel - saw par2", channelTimes10[i], ackRecvPar2, 0, CalSetup.customerDebug);
...
void printCalFailed(A_UINT32 mode)
{
	printResultBanner(devNumArr[mode], TRUE);
	uiPrintf("\n*******************************************************\n");
	  uiPrintf("*****    DUT FAILED FOLLOWING TESTS              *****\n");
	  uiPrintf("*****    %s %s          *****\n", "Calibration in mode", modeName[mode]);
	  uiPrintf("*******************************************************\n");
}
void testTempMargin(){
...
void powerCal(int menuSelect){

    A_BOOL		exitLoop = FALSE;
	A_UINT32 modeSelect=0;
	//get CTL for later use
	if(!parseTargets()) 
	{
		uiPrintf("An error occured while parsing the file %s. Pl. check for format errors.\n", CalSetup.tgtPwrFilename);
	}
	...
	if (menuSelect != MH_POWER_PROFILE){
	uiPrintf("\n=======================================");
	uiPrintf("\nPlug in DUT (Device Under Test)        ");
	uiPrintf("\nPress any key to start or <ESC> to quit");
	uiPrintf("\n=======================================\n");
	...
	while (!kbhit())
		...
		uiPrintf("\n=======================================");
		uiPrintf("\nPower Profile loop ibn Progress");
		uiPrintf("\n=======================================\n");
	}
		globalteststarttime = milliTime();
		...
		//	uiPrintf("\nSetting up Spectrum Analyzer");
		//	devSA = spaInit(CalSetup.saGPIBaddr, CalSetup.saModel);
		//	//spaGetModelNo(devSA, 1)
		//	ResetSA(devSA, 1e9, 1e7, 10.0, 10, 1e6, 3e6);
		//	uiPrintf("\nNo Attenuator present\n");
		//	strcpy(testname[testnum],"instrument setup");
		//	testtime[testnum++] = milliTime() - timestart;
		if (CalSetup.pmModel != NO_PM){
			uiPrintf("\nSetting up Power Meter");
			...
			if ((CalSetup.pmModel == PM_E4416A) && !(SETUP_PM_MODEL2_ONCE))
			{
				...
				//exit(0);
				SETUP_PM_MODEL2_ONCE = TRUE;
			}else if ((CalSetup.pmModel == PM_436A) && !(SETUP_PM_MODEL2_ONCE)){
				gpibWrite(devPM, "PR");
				usleep(1000*2000);
				pmPreset(devPM, -20, -4e-6, 860e-6, 2.6e-3, 3.43e-3);
				usleep(1000*2000);
		...
		checkSetGain();
		testTempMargin();
		if ((menuSelect == MH_POWERCAL) || (menuSelect ==  MH_POWER_READ)){		
					// 11a calibration
					if((CalSetup.calPower && (CalSetup.Amode || CalSetup.enableJapanMode11aNew)) && (!REWIND_TEST)){
						timestart = milliTime();
						strcpy(testname[testnum],"11a cal");
						...
						dutCalibration(devNumArr[MODE_11a]);//, menuSelect); Siavash removed menu select
						initializeGainLadder(pCurrGainLadder);
						programNewGain(pCurrGainLadder, devNumArr[MODE_11a], 0);
						testtime[testnum++] = milliTime() - timestart;
					} 
					...
					if(CalSetup.calPower && CalSetup.Gmode && (!REWIND_TEST)){
						...
						dutCalibration_2p4(devNumArr[MODE_11g], MODE_11g);//, menuSelect); Siavash removed menu select
						initializeGainLadder(pCurrGainLadder);
						programNewGain(pCurrGainLadder, devNumArr[MODE_11g], 0);
 						testtime[testnum++] = milliTime() - timestart;
				   } 
					...
					if(CalSetup.calPower && CalSetup.Bmode && (!REWIND_TEST)){
						...
						dutCalibration_2p4(devNumArr[MODE_11b], MODE_11b);//,menuSelect ); Siavash Removed menuSelect
						initializeGainLadder(pCurrGainLadder);
						programNewGain(pCurrGainLadder, devNumArr[MODE_11b], 0);
						testtime[testnum++] = milliTime() - timestart;
					} 
		...
		}else if (menuSelect == MH_POWER_PROFILE){
			if(CalSetup.Gmode || CalSetup.Bmode){
				for (modeSelect = MODE_11g; modeSelect < MODE_11a; modeSelect++){
							configSetup.eepromLoad = 0;
							art_setResetParams(devNumArr[modeSelect], configSetup.pCfgFile, (A_BOOL)configSetup.eepromLoad,
									(A_BOOL)configSetup.eepromHeaderLoad, (A_UCHAR)modeSelect, configSetup.use_init);		
							timestart = milliTime();
							...
							if (!setup_datasets_for_cal_gen5(devNumArr[modeSelect], modeSelect)) { 
								uiPrintf("Could not setup gen5 raw datasets for mode %d. Exiting...\n", modeSelect);
								closeEnvironment();
								exit(0);
							}
							
							MH_measure_all_channels(devNumArr[modeSelect], CalSetup.customerDebug, modeSelect) ;
...
void IQCal(int menuSelect){
	...
	eepromType = (A_UINT16) CalSetup.dutPromSize ;
	uiPrintf("\n============================================");
	uiPrintf("\n Make Sure the Golden Unit PC is running ART");
	uiPrintf("\nPlug in DUT (Device Under Test)				");
	uiPrintf("\nPress any key to start or <ESC> to quit		");
	uiPrintf("\n==========================================\n");
	while (!kbhit())
	...
	// Put the IQ test here
		...
		if (CalSetup.calPower && CalSetup.do_iq_cal && (((swDeviceID & 0xFF) == 0x14)||((swDeviceID & 0xFF) >= 0x16))) 
		{
			...
			if ((CalSetup.Amode|| CalSetup.enableJapanMode11aNew) && (CalSetup.calPower) && (!REWIND_TEST)) 
			{
				timestart = milliTime();
 				strcpy(testname[testnum],"IQ_cal for 11a");
				configSetup.eepromLoad = 0;
			...
			sendAck(devNumArr[MODE_11g], "Done with iq_cal", 0, 0, 0, CalSetup.customerDebug);
