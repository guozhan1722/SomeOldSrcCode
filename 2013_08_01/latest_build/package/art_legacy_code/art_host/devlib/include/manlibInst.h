...
//daniel #include <rsspecan.h>

#define USENIMAX						0x01
...
MANLIB_API int spaInit(const int adr, const int model); //,  ViSession *globalSession
MANLIB_API int spaClose();
...
MANLIB_API double saChanPwr(const int ud, const double center, const double span, const double ref_level, 
...
MANLIB_API char *spaMeasTxSpurs(const int ud, const double freq_start, const double freq_stop, const double spur_thresh, const int reset);
...
//daniel MANLIB_API ViStatus openVisaSession();
//daniel MANLIB_API ViStatus closeVisaSession();
//daniel MANLIB_API  int isRMsessionOpen();
//MANLIB_API  int instantiatePm();
//MANLIB_API  int unInstantiatePm();
#endif // #ifndef SWIG
