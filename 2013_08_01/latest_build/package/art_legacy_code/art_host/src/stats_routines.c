...
//printf("SNOOP: bR = %d, Time = %5.5lf tput = %5.1f\n", bitsReceived,
//	   newTxTime, (float)txStats[descRate].newThroughput/1000); 
	}
}
...
)
{
	...
	A_UINT32 i, statsLoop;//, tempRate; removed Siavash
	A_UINT32 bitsReceived;
...
#else
	A_MEM_SET(txStats, 0, sizeof(TX_STATS_STRUCT) * STATS_BINS);
	// traverse the descriptor list and get the stats from each one
	A_MEM_SET(&statsInfo, 0, sizeof(TX_STATS_TEMP_INFO));
#endif
...
//				txStats[statsInfo.descRate].endTime = descTempTime;
				txStats[statsInfo.descRate].endTime = descMidTime; //FJC 10/17/05 think should be mid time not temp time
...
//					txStats[statsInfo.descRate].startTime = descTempTime;
					txStats[statsInfo.descRate].startTime = descMidTime; //FJC 10/17/05 think should be mid time not temp time
