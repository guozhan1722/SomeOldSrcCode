...
static const char mcastcfg[] = "/etc/config/mcast";
...
static void UpdateRoute(void)
{
    ...
    if(!did_final_init) return;

    if(!once) {
        memset(tunlnc, 0, sizeof(tunlnc));
        while(tmcastctl)
        {
            current = tmcastctl;
            ...
            if (current->channel[0]){
                    ...
                    if ((!(v->uv_flags & VIFF_DISABLED)) && (!(v->uv_flags & VIFF_DOWN))) {                    
                        if (v->uv_flags & VIFF_TUNNEL) {   
                            if(!strcmp(v->uv_name, current->channel)) {
                                strcat(tunlnc, current->channel);
                                strcat(tunlnc, " ");                                   
                                current->Niftunnel = vifi;
                            }
                        }
                    } 
                }
                ...
                for (vifi = 0, v = uvifs; vifi < numvifs; ++vifi, ++v) {
                    if ((!(v->uv_flags & VIFF_DISABLED)) && (!(v->uv_flags & VIFF_DOWN))) {                    
                        if (v->uv_flags & VIFF_TUNNEL) {
                            if (v->uv_rmt_addr == current->remoteip32)
                            {
                                current->Niftunnel = vifi;
                                sprintf(tmp, "%d", vifi);
                                strcat(tunlnc, "dvmrp");
        ...
        tmcastctl = mcastctl;
        if(strlen(tunlnc) > 4) {
            fwt = fopen("/var/run/tunlfl", "wt");
            if(fwt == NULL) {
                logit(LOG_ERR, 0, "Couldn't write %s.", "/var/run/tunlfl");
                return; 
            } else {
                fwrite(tunlnc, strlen(tunlnc), 1, fwt);
                fclose(fwt);
            }
        }
        Dbgmcastctl();
    ...
    while(tmcastctl)
    {
        ...
        if( ((current->srcip32) & (current->srcmask32)) == ((current->localip32) & (current->srcmask32)) ) {
            if(!current->once)
            {
                current->once = 1;
            }
            if(current->Niftunnel >= 0) update_lclgrp(current->Niftunnel, current->grpip32);
        } else {    // client
            if(!current->once) {
                current->once = 1;
                if(current->Niftunnel >= 0)
                {
                    update_route( ((current->srcip32) & (current->srcmask32)), current->srcmask32, 0, 0, current->Niftunnel, 0);
                    add_table_entry(current->srcip32, current->grpip32); 
                }               
            }            
        }
    }
...
void k_add_rg_src(u_int32 origin, u_int32 mcastgrp);
static void final_init(void *i)
{
    ...
    did_final_init = 1;
    //k_add_rg_src(0xc0a801f0, 0xefffffc8);
}
