 ...
 */
int grplst_mem(vifi_t vifi, u_int32 mcastgrp)
{
    struct listaddr *g;
    struct uvif *v;
 ...
 * head of the list.
 */
int find_src_grp(u_int32 src, u_int32 mask, u_int32 grp)
{
    struct gtable *gt;
...
static void send_prune(struct gtable *gt)
{
	...
	IF_DEBUG(DEBUG_PRUNE) {
	    logit(LOG_DEBUG, 0, "Not rexmitting prune for (%s %s)/%d on vif %d to %s",
		  RT_FMT(gt->gt_route, s1), inet_fmt(gt->gt_mcastgrp, s2, sizeof(s2)),
		  gt->gt_prsent_timer, gt->gt_route->rt_parent,
		  inet_fmt(gt->gt_route->rt_gateway, s3, sizeof(s3)));
	}
	...
	IF_DEBUG(DEBUG_PRUNE) {
	    logit(LOG_DEBUG, 0, "Not bothering to send prune for (%s,%s)/%d on vif %d to %s because it's too short",
		  RT_FMT(gt->gt_route, s1), inet_fmt(gt->gt_mcastgrp, s2, sizeof(s2)),
		  gt->gt_prsent_timer, gt->gt_route->rt_parent,
		  inet_fmt(gt->gt_route->rt_gateway, s3, sizeof(s3)));
	}
 ...
 */
struct gtable *find_grp(u_int32 grp)
{
   struct gtable *gt;
 ...
 * entry
 */
struct stable *find_grp_src(struct gtable *gt, u_int32 src)
{
   struct stable *st;
 ...
 * @mask: 
 */
int next_grp_src_mask(struct gtable **gtpp, struct stable **stpp, u_int32 grp, u_int32 src, u_int32 mask)
{
 ...
 */
void refresh_sg(struct sioc_sg_req *sg, struct gtable *gt, struct stable *st)
{
   static   int lastq = -1;
 ...
 */
int next_child(struct gtable **gtpp, struct stable **stpp, u_int32 grp, u_int32 src, u_int32 mask, vifi_t vifi)
{
    /* Get (G,S,M) entry */
    if (mask!=0xFFFFFFFF
        || !((*gtpp) = find_grp(grp))
...
void add_table_entry(u_int32 origin, u_int32 mcastgrp)
{
	...
	*stnp = st;
    ...
    kroutes++;
    logit(LOG_DEBUG, 0, "add_table_entry: origin %s", inet_fmt(origin, s1, sizeof(s1)));
    k_add_rg(origin, gt);
 ...
 */
void reset_neighbor_state(vifi_t vifi, u_int32 addr)
{
    struct rtentry *r;
    struct gtable *g;
 ...
 */
void del_table_entry(struct rtentry *r, u_int32 mcastgrp, u_int del_flag)
{
 ...
 */
void update_table_entry(struct rtentry *r, u_int32 old_parent_gw)
{
    struct gtable *g;
    struct ptable *pt, **ptnp;
	...
	while ((pt = *ptnp)) {
		...
		IF_DEBUG(DEBUG_PRUNE) {
		    logit(LOG_DEBUG, 0, "update_table_entry() deleting prune for (%s %s) from %s on vif %d -%s%s",
			  RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2, sizeof(s2)),
		...
		if (!NBRM_ISSET(pt->pt_index, g->gt_prunes)) {
		    logit(LOG_WARNING, 0, "gt_prunes lost track of (%s %s) from %s on vif %d",
			  RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2, sizeof(s2)),
			  inet_fmt(pt->pt_router, s3, sizeof(s3)), pt->pt_vifi);
		}
 ...
 */
void update_lclgrp(vifi_t vifi, u_int32 mcastgrp)
{
    struct rtentry *r;
    struct gtable *g;
 ...
 */
void delete_lclgrp(vifi_t vifi, u_int32 mcastgrp)
{
    struct gtable *g;
    
    logit(LOG_DEBUG, 0, "vif%d leave group 0x%x", vifi, mcastgrp); 
    IF_DEBUG(DEBUG_MEMBER) {
...
void accept_prune(u_int32 src, u_int32 dst, char *p, size_t datalen)
{
    ...
    IF_DEBUG(DEBUG_PRUNE) {
	logit(LOG_DEBUG, 0, "%s on vif %d prunes (%s %s)/%d", inet_fmt(src, s1, sizeof(s1)),
	...
	IF_DEBUG(DEBUG_PRUNE) {
	    logit(LOG_DEBUG, 0, "Found grp state, (%s %s), metric is %d, children are %x, subords are %08x%08x",
 ...
 */
void chkgrp_graft(vifi_t vifi, u_int32 mcastgrp)
{
    struct rtentry *r;
    struct gtable *g;
...
void accept_graft(u_int32 src, u_int32 dst, char *p, size_t datalen)
{
    ...
    IF_DEBUG(DEBUG_PRUNE) {
	logit(LOG_DEBUG, 0, "%s on vif %d grafts (%s %s)", inet_fmt(src, s1, sizeof(s1)), vifi, 
	      inet_fmt(graft_src, s2, sizeof(s2)), inet_fmt(graft_grp, s3, sizeof(s3)));
    }
...
void age_table_entry(void)
{
	...
	for (i = 0; i < numvifs; i++) {
		...
		logit(LOG_WARNING, 0, "(%s %s) is blackholing on vif %d",
	...
	while ((pt = *ptnp) != NULL) {
		...
		if (gt->gt_prsent_timer > 0) {
		    logit(LOG_WARNING, 0, "Prune (%s %s) from %s on vif %d expires with %d left on prsent timer",
			  RT_FMT(r, s1), inet_fmt(gt->gt_mcastgrp, s2, sizeof(s2)),
...
void dump_cache(FILE *fp2)
{
	...
	for (st = gt->gt_srctbl; st; st = st->st_next) {
	    fprintf(fp2, ">%-18s %-8s %6ld", inet_fmt(st->st_origin, s1, sizeof(s1)),
