...
static void create_route(u_int32 origin, u_int32 mask)
{
    ...
    logit(LOG_DEBUG, 0, "create_route: origin %s(0x%x), mask %s(0x%x)", 
          inet_fmt(origin, s1, sizeof(s1)),  origin, inet_fmt(mask, s2, sizeof(s2)), mask);
...
//    logit(LOG_DEBUG, 0, "update_route: origin %s(0x%x), src %s(0x%x), metric=%d, vifi=0x%x", 
//          inet_fmt(origin, s1, sizeof(s1)), origin, inet_fmt(src, s2, sizeof(s2)), src, metric, vifi);
    /*
...
struct rtentry *determine_route(u_int32 src)
{
    ...
    for (rt = routing_table; rt != NULL; rt = rt->rt_next) {
         if (rt->rt_origin == (src & rt->rt_originmask) && rt->rt_metric != UNREACHABLE) break;
    }
