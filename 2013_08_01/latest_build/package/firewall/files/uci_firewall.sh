...
get_dmz_multiport()
{
    ...
    while true ; do
        havedot=$(echo $mports |grep ',')
        if [ -n "$havedot" ] ;then
            port_first=$(echo $mports|awk -F ',' '{print $1}')
            append get_ports "$port_first" "$N" 
            port_rest=${mports#*,}
            mports=$port_rest
        else
            break
...
fw_dmz() {
	...
	#src="wan"
	config_get src $1 dmz_src wan2
	config_get disabled_dmz $1 disabled_dmz
	proto="TCP"
        config_get src_dport $1 dmz_ex_port 0
        config_get dest_ip $1 dmz_ip
         

	add_rule_ex_port() {
		$IPTABLES -I zone_${src}_prerouting -t nat \
			${proto:+-p $proto} \
			${sport:+--dport $sport} \
			-j ACCEPT

		$IPTABLES -I zone_${src}\
			${proto:+-p $proto} \
			${sport:+--dport $sport} \
			-j ACCEPT
	}
	...
	add_rule_dest_ip() {
		$IPTABLES -A zone_${src}_prerouting -t nat \
      		...
      		$IPTABLES -I zone_${src}_forward  \
			-d $dest_ip \
			-j ACCEPT
	}

        [ "$disabled_dmz" = "1" ] && return
        [ "$src_dport" != "0" ] && {
            get_dmz_multiport "$src_dport"
            for sport in $get_ports ; do
                if [ "$sport" != "0" ]; then
                    src_dport_first=${sport%-*}
                    src_dport_last=${sport#*-}
                    [ "$src_dport_first" -ne "$src_dport_last" ] && { \
                            sport="$src_dport_first:$src_dport_last"; }
                    add_rule_ex_port
                fi
            done
        }
...
fw_init() {
	...
	config_foreach fw_include include
        for file in /var/run/ipsecfirewall/firewall.*;do sh $file 1>/dev/null 2>/dev/null;done
}
...
fw_stop() {
	...
	uci_revert_state firewall
        for sfile in /etc/firewall_stop/*;do sh $sfile 1>/dev/null 2>/dev/null;done
}
