 ...
 * it.
 */
int mesh_rmc_check(u8 *sa, void *skb_data,
		   struct ieee80211_rx_data *rx)
{
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb_data;
	unsigned int hdrlen = ieee80211_hdrlen(hdr->frame_control);
	struct ieee80211s_hdr *mesh_hdr = (struct ieee80211s_hdr *)((u8 *)hdr+hdrlen);
	...
	{
		rx->key = rcu_dereference(sdata->default_multicast_key);
		...
		case WLAN_CIPHER_SUITE_WEP104:
			mesh_hdr = (struct ieee80211s_hdr *)((u8 *)hdr+hdrlen+WEP_IV_LEN);
			break;
		case WLAN_CIPHER_SUITE_TKIP:
			mesh_hdr = (struct ieee80211s_hdr *)((u8 *)hdr+hdrlen+TKIP_IV_LEN);
			break;
		case WLAN_CIPHER_SUITE_CCMP:
			mesh_hdr = (struct ieee80211s_hdr *)((u8 *)hdr+hdrlen+CCMP_HDR_LEN);
			break;
		default:
			/*
			 * other encryption we do not support
			 */
			return 0;
		}
