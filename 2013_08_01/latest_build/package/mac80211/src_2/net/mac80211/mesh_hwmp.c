...
#define mhwmp_dbg(fmt, args...)   printk(KERN_INFO "Mesh HWMP: " fmt, ##args)
#else
			    ...
			    u8 *hwmp_ie, enum mpath_frame_type action)
{
	...
	if (memcmp(orig_addr, sdata->vif.addr, ETH_ALEN) == 0) {
		...
		if (fresh_info) {
//            mhwmp_dbg("hwmp_route_info_get1: orig_addr=%pM\n", orig_addr);
	...
	if (memcmp(orig_addr, ta, ETH_ALEN) == 0)
		...
		if (fresh_info) {
//            mhwmp_dbg("hwmp_route_info_get2: orig_addr=%pM\n", orig_addr);
			mesh_path_assign_nexthop(mpath, sta);
			mpath->flags &= ~MESH_PATH_SN_VALID;
				    ...
				    u8 *preq_elem, u32 metric)
{
	...
	//mhwmp_dbg("target_addr= %pM, received PREQ from orig_addr=%pM.\n", target_addr, orig_addr);
...
//		mhwmp_dbg("forwarding the PREQ from %pM to target_addr=%pM\n", orig_addr, target_addr);
		--ttl;
...
static void mesh_queue_preq(struct mesh_path *mpath, u8 flags)
{
    ...
    if(!LookupMeshMacBlack(mpath->dst)) return;
    ...
    mhwmp_dbg("mesh_queue_preq: preq_node->dst=%pM, \n", preq_node->dst);
...
void mesh_path_start_discovery(struct ieee80211_sub_if_data *sdata)
{
	...
	rcu_read_lock();
//    mhwmp_dbg("mesh_path_start_discovery: preq_node->dst=%pM, \n", preq_node->dst);
			...
			struct ieee80211_sub_if_data *sdata)
{
	...
	if(is_multicast_ether_addr(target_addr)) return 0;
#endif
//    mhwmp_dbg("mesh_nexthop_lookup: target_addr=%pM, \n", target_addr);
    if(!LookupMeshMacBlack(target_addr)) return -1;
