...
static void ieee80211_teardown_sdata(struct net_device *dev)
{
	...
	if (ieee80211_vif_is_mesh(&sdata->vif))
	{
		...
		if(mesh_iface_count < 0) mesh_iface_count = 0;

		if (mesh_allocated && (!mesh_iface_count)) {
			ieee80211s_stop();
			mesh_allocated = 0;
		}
	}
...
/* This is an optimization, just ignore for older kernels */
    ...
    AddMeshMacBlack(sdata->vif.addr);
	...
	for (i = 0; i < IEEE80211_NUM_BANDS; i++) {
        ...
        //	printk(KERN_DEBUG "ieee80211_if_add bitrate_mask 0x%02x\n", sdata->rc_rateidx_mcs_mask[1][i]);
	//}
