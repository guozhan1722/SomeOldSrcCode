...
static struct mesh_table *mesh_paths = NULL;
static struct mesh_table *mpp_paths = NULL; /* Store paths for MPP&MAP */
...
int mesh_path_add(u8 *dst, struct ieee80211_sub_if_data *sdata)
{
    ...
    if(!LookupMeshMacBlack(dst)) return -ENOTSUPP;
...
#ifdef CONFIG_MICROHARD
	if( mesh_paths || mpp_paths ) return;
#endif
...
#ifdef CONFIG_MICROHARD
	rcu_assign_pointer(mesh_paths, NULL);
	rcu_assign_pointer(mpp_paths, NULL);
#endif
}
