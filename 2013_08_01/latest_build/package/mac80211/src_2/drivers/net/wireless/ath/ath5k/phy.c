		...
		struct ieee80211_channel *channel)
{
    ...
    case 2: // 5.8G	
    //case 3: // 2.4G
    case 4: // 4.9G microhard standard
	  tmp = tmp*1000 - acting->lfreq;
      if (tmp == 3000) d = 10*c - 5; 
      else if (tmp%2500) d = 10*c + 5;     
	  break;  
	default:
	  break;
	}
	...
	if (!d) {
        printk(KERN_INFO "setup center_freq to %dMhz\n", c);
		if (c < 4800) {
			if (!((c - 2224) % 5)) {
        ...
        printk(KERN_INFO "setup center_freq to %dKhz\n", d*100);
		data0 = ath5k_hw_bitswap((d - 48000) / 25, 8);
		data2 = ath5k_hw_bitswap(0, 2);	
	}
...
#ifdef CONFIG_TARGET_ar71xx_IPnDDL
    ah->ah_txpower.txp_min_pwr = 0;
    ah->ah_txpower.txp_max_pwr = 120;
#else
...
#ifdef CONFIG_TARGET_ar71xx_IPnDDL
        printk(KERN_INFO "Power rate[%d]=%d\n", i, rates[i]/2);
#endif
		if (rates[i] > 63)
			rates[i] = 63;
	}
