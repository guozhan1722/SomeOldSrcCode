...
void serialDrive(u32 data) {
   if (adfParams.ah) {
        spiOutWord(adfParams.cs, adfParams.ah, data);
   }
}
 ...
 * Returns:     
 ******************************************************************************/
void ADF_4350_Init(struct ath_common *common)
{
    memset(&adfParams, 0, sizeof(adfParams));
    printk(KERN_INFO "ADF_4350_Init: ah=0x%x\n", (u32)common->ah);   
    if (common) {
        adfParams.ah = common->ah;
    } else {
        adfParams.ah = NULL;
    }

    spiInit(adfParams.ah);
...
int ADF4350_SetParams(u64 freq)
{
    ...
    printk(KERN_INFO "\nADF4350_SetParams: freq=%lld=0x%llx.\n", freq, freq);
...
int ADF4350_ComputeParams(u64 freq)
{
	...
	if ((ADF4350_LOWEST_FREQ  > freq) || (ADF4350_HIGHEST_FREQ  < freq)) {
		return -EINVAL ;
	}
