...
#ifdef CONFIG_SUPPORT_XR

#define FRAME_DATA	2       /* Data frame */
#define SUBT_DATA_CFPOLL    2       /* Data + CF-Poll */
#define SUBT_NODATA_CFPOLL	6       /* No Data + CF-Poll */

static void ath5k_set_xr_mode(struct ath5k_hw *ah)
{
	u32 xrmode_reg, xrto_reg, xrdel_reg;

	printk(KERN_DEBUG"BS DEBUG: setup XR register on STA mode\n");
	...
	xrmode_reg = ath5k_hw_reg_read(ah, AR5K_XRMODE);
	xrmode_reg &= ~ (AR5K_XRMODE_POLL_TYPE|AR5K_XRMODE_POLL_SUBTYPE);

	xrmode_reg |= AR5K_REG_SM(FRAME_DATA, AR5K_XRMODE_POLL_TYPE);
	xrmode_reg |= AR5K_REG_SM(SUBT_NODATA_CFPOLL, AR5K_XRMODE_POLL_SUBTYPE);
	ath5k_hw_reg_write(ah, xrmode_reg, AR5K_XRMODE);
	...
	//if (1) {
		...
		xrto_reg = ath5k_hw_reg_read(ah, AR5K_XRTIMEOUT);
		xrto_reg &= ~(AR5K_XRTIMEOUT_CHIRP|AR5K_XRTIMEOUT_POLL);
		xrto_reg |= AR5K_REG_SM(ath5k_hw_htoclock(ah, 2 * XR_SLOT_DELAY + XR_DATA_DETECT_DELAY + 80), AR5K_XRTIMEOUT_CHIRP);
		xrto_reg |= AR5K_REG_SM(ath5k_hw_htoclock(ah, (INIT_AIFS_XR + INIT_CWMAX_XR) * XR_SLOT_DELAY + XR_CHIRP_DUR + 16), AR5K_XRTIMEOUT_POLL);
		ath5k_hw_reg_write(ah, xrto_reg, AR5K_XRTIMEOUT);
		...
		xrdel_reg = ath5k_hw_reg_read(ah, AR5K_XRDELAY);
		xrdel_reg &= ~(AR5K_XRDELAY_SLOT_DELAY);
		xrdel_reg |= AR5K_REG_SM(ath5k_hw_htoclock(ah, XR_SLOT_DELAY), AR5K_XRDELAY_SLOT_DELAY);
		ath5k_hw_reg_write(ah, xrdel_reg, AR5K_XRDELAY);
		#endif
	...
	//} else {
		...
		AR5K_REG_WRITE_BITS(ah, AR5K_XRDELAY, AR5K_XRDELAY_CHIRP_DELAY,
							ath5k_hw_htoclock(ah, 2 * XR_SLOT_DELAY - (XR_CHIRP_DUR + 1)));


                //OS_REG_WRITE(ah, AR_D_GBL_IFS_SLOT, MAC_CLKS(XR_SLOT_DELAY));
		ath5k_hw_reg_write(ah, ath5k_hw_htoclock(ah, XR_SLOT_DELAY), AR5K_DCU_GBL_IFS_SLOT);

                //OS_REG_WRITE(ah, AR_XRMODE,
                //        OS_REG_READ(ah, AR_XRMODE) | AR_XRMODE_XR_WAIT_FOR_POLL);
		xrmode_reg = ath5k_hw_reg_read(ah, AR5K_XRMODE);
		xrmode_reg |= AR5K_XRMODE_WAIT_FOR_POLL;
		ath5k_hw_reg_write(ah, xrmode_reg, AR5K_XRMODE);
		//}
...
#undef DATA_FRAME
#undef SUBT_DATA_CFPOLL
#undef SUBT_NODATA_CFPOLL

#endif /* CONFIG_SUPPORT_XR */
...
int ath5k_hw_nic_wakeup(struct ath5k_hw *ah, int flags, bool initial)
{
	...
	if (ah->ah_version != AR5K_AR5210) {
...
#ifdef CONFIG_SUPPORT_XR
		if (ah->ah_xrmode) {
			printk(KERN_DEBUG"BS DEBUG: setup PHY MODE XR at line %d\n", __LINE__);
			mode |= AR5K_PHY_MODE_XR;
		}
#endif
		...
		if (ah->ah_bwmode == AR5K_BWMODE_40MHZ) {
			...
			/* the original code breaks the turbo feature
			 * probably caused by compiler bug */
			turbo = AR5K_PHY_TURBO_MODE;
			turbo |= (ah->ah_radio == AR5K_RF2425) ? 0 : AR5K_PHY_TURBO_SHORT;
#else
			turbo = AR5K_PHY_TURBO_MODE |
				(ah->ah_radio == AR5K_RF2425) ? 0 :
				AR5K_PHY_TURBO_SHORT;
#endif /* CONFIG_MICROHARD */
...
int mdf_ath5k_setup_bands(struct ieee80211_hw *hw);
int ath5k_hw_set_capabilities(struct ath5k_hw *ah);

static char mynoname[256];
static struct ieee80211_regdomain *myregdomain = 0;

static void fill_myregdomain(struct wifpara *wifa)
{
  memset(&mynoname, 0, sizeof(mynoname));
  myregdomain = (struct ieee80211_regdomain*)&mynoname;
   
  myregdomain->n_reg_rules = 1;
  myregdomain->alpha2[0] = 'D';
  myregdomain->alpha2[1] = 'B';
    
  myregdomain->reg_rules[0].freq_range.start_freq_khz = wifa->lfreq;
  myregdomain->reg_rules[0].freq_range.end_freq_khz = wifa->rfreq;
  myregdomain->reg_rules[0].freq_range.max_bandwidth_khz = wifa->bandwidth;
  myregdomain->reg_rules[0].power_rule.max_antenna_gain = DBI_TO_MBI(wifa->antgai);
  myregdomain->reg_rules[0].power_rule.max_eirp = DBM_TO_MBM(wifa->maxeirp);
  myregdomain->reg_rules[0].flags = 0;
}
#endif // CONFIG_TARGET_ixp4xx_generic_VIP421EXP49G
		...
		struct ieee80211_channel *channel, bool fast, bool skip_pcu)
{
	...
	struct wifpara *wifa =ieee80211_mdf_read();
	struct wifpara *acting = wifa + 1;
	if(wifa->pending)
	{
	  while(!mdf_mutex_lock());	
	  if (acting->curntmd != wifa->mdfmode)
	  {
          switch(wifa->mdfmode) {
          case 0:
              if(sc->chbkdone)
              {
                  printk(KERN_INFO "mdf restore to default.\n");
                  acting->pending = 1;	//setting
                  memcpy(acting, wifa, sizeof(struct wifpara));
                  acting->curntmd = acting->mdfmode;
                  memcpy(sc->channels, sc->chanbk, sizeof(sc->chanbk));
                  memcpy(sc->sbands, sc->sbandsbk, sizeof(sc->sbandsbk));
                  ath5k_hw_set_capabilities(ah);	 
                  acting->pending = 0;	//setup done
              }
              break;
          case 1:
          case 2:
          ...
          case 5:
          case 6:
              printk(KERN_INFO "mdf setup to mode(%d).\n", wifa->mdfmode);
              acting->pending = 1;	//setting
              wifa->boot = 0;
              fill_myregdomain(wifa);	
              ath5k_hw_set_capabilities(ah);
              mdf_ath5k_setup_bands(hw);
              if(wiphy)wiphy_apply_custom_regulatory(wiphy, myregdomain);
              if(acting->cardmod) wifa->cardmod = acting->cardmod;
              memcpy(acting, wifa, sizeof(struct wifpara));
              acting->curntmd = acting->mdfmode;
              acting->pending = 0;	//setup done
              break;
