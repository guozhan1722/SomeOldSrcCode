...
struct ieee80211_if_ap {
	...
	int dtim_count;
#ifdef MICROHARD
	bool mesh_flag; /* ture if ap prticipates in mesh network */
#endif
};
...
struct ieee80211_if_mesh {
	...
	bool root_node;
	u8 root[ETH_ALEN];
	u8 root_nexthop[ETH_ALEN];


	struct timer_list pro_rreq_timer;
	bool pro_rreq;
	u32 pro_rreq_dsn;
	u8 depend_mp[1550];
	...
	/* Poratl */
	bool portal_flag;
	bool portal_timer_flag;
	struct timer_list portal_timer;
	u8 pann_seq_num;

	spinlock_t portal_list_lock;
	struct mesh_portal_list portal_list;
	int portal_list_len;
#endif
};
