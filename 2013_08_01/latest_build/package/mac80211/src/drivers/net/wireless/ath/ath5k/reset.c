...
int ath5k_hw_nic_wakeup(struct ath5k_hw *ah, int flags, bool initial)
{
	...
	if (ah->ah_version != AR5K_AR5210) {
		...
		else if (ah->ah_bwmode == AR5K_BWMODE_5MHZ)
			mode |= AR5K_PHY_MODE_QUARTER_RATE;
#endif
		...
		if (ah->ah_bwmode == AR5K_BWMODE_10MHZ)
			...
			clock |= AR5K_PHY_PLL_QUARTER_RATE;
#endif
 ...
 */
static void ath5k_hw_init_core_clock(struct ath5k_hw *ah, struct ieee80211_channel *channel)
{
        u32 usec_reg, txlat, rxlat, usec, clock, sclock, txf2txs = AR5K_INIT_TXF2TXD_START_DEFAULT;
	...
	struct ieee80211_channel *channel, bool change_channel)
{
	...
	switch(ah->ah_bwmode) {
	case AR5K_BWMODE_5MHZ:
		channel->hw_value &= ~(CHANNEL_HALF | CHANNEL_TURBO);
		channel->hw_value |= CHANNEL_QUARTER;
		break;
	case AR5K_BWMODE_10MHZ:
		channel->hw_value &= ~(CHANNEL_QUARTER | CHANNEL_TURBO);
		channel->hw_value |= CHANNEL_HALF;
		break;
	case AR5K_BWMODE_40MHZ:
		channel->hw_value &= ~(CHANNEL_QUARTER | CHANNEL_HALF);
		channel->hw_value |= CHANNEL_TURBO;
		break;
	default:
		channel->hw_value &= ~(CHANNEL_QUARTER | CHANNEL_HALF | CHANNEL_TURBO);
		break;
	}
#endif
	...
	ath5k_hw_init_core_clock(ah, channel);
#endif
	...
	if (ret)
		return ret;
#else
// original code. will be removed lately
...
#ifdef MICROHARD
	        /* this workaround supposed to be at the Rx side. Changing on Tx side has negative impact on the throughput */
                AR5K_REG_WRITE_BITS(ah, AR5K_TXCFG,
                        AR5K_TXCFG_SDMAMR, AR5K_DMASIZE_128B);
#else
		AR5K_REG_WRITE_BITS(ah, AR5K_TXCFG,
