...
static int ath9k_key_config(struct ath_common *common,
...
static void ath9k_key_delete(struct ath_common *common, struct ieee80211_key_conf *key)
{
	struct ath_hw *ah = common->ah;
			 ...
			 struct ieee80211_key_conf *key)
{
	...
	switch (cmd) {
	case SET_KEY:
		ret = ath9k_key_config(common, vif, sta, key);
		if (ret >= 0) {
			key->hw_key_idx = ret;
