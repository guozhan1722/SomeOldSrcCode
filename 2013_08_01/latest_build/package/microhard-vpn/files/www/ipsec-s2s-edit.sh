    ...
    equal "$?" 0 && {
	    add_s2s_cfg="$FORM_s2s_tunnel_name"
 	    uci_set "ipsec" "$add_s2s_cfg" "interface"       "$FORM_inf"
            ...
            [ -f "/etc/debugipsec" ] && logger -t "ipsec-s2s-edit.sh" "uci set $add_s2s_cfg"
            /usr/sbin/ipsec auto --delete $add_s2s_cfg >/dev/null 2>&1
            echo "action=edit" > /var/run/mipsec/"$add_s2s_cfg".status
            echo "status=waiting for connection" >> /var/run/mipsec/"$add_s2s_cfg".status
            echo "constat=Connect" >> /var/run/mipsec/"$add_s2s_cfg".status
    }
fi
