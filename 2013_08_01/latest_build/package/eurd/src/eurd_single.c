...
static int check_carrierIP_changed()
{
    if(last_carrierIP_str[0]==0)
    {
        strcpy(last_carrierIP_str,"N/A");
        return 0;
    }

    char ip_carrier[32]="N/A";
    if (false==fetch_Local_IP_ADDR(ifname_carrier,ip_carrier))fetch_Local_IP_ADDR(ifname_eth2,ip_carrier);
    if(strlen(ip_carrier)<3)strcpy(ip_carrier,"N/A");
    if(strcmp(ip_carrier,last_carrierIP_str)!=0)
    {
            strcpy(last_carrierIP_str,ip_carrier);
            return 2;
    }
    ...
    {
        tmp_buff[0]=0;
        if(fgets(tmp_buff, 64, f))
        {
            if(strlen(tmp_buff)>10)
            {
                p=NULL;
                p = strstr(tmp_buff, "on");
                if(p!=NULL)
                {
                    *p=0;
                    if(*(p-1)==' ')
                    ...
                    strcpy(ip_carrier,tmp_buff);
                }
            }
        }
        fclose(f);
    }
    if(strlen(ip_carrier)>4)
    {
        if(strcmp(ip_carrier,last_carrierIP_str)!=0)
        {
            strcpy(last_carrierIP_str,ip_carrier);
...
static void updateIPsocket()
{
    ...
    char tmpbuff[256];
    sprintf(tmpbuff,"IPlookup %s %d",DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+AERC_REPORT_NUM],AERC_REPORT_NUM);
    if(SubProgram_Start(eurdscriptFile,tmpbuff)==false)return;
    FILE *f;
    char *p;
    char *p1;
    sprintf(tmpbuff,"/tmp/run/u_s2ip%d",AERC_REPORT_NUM);
    f=fopen(tmpbuff,"r");
    if(f==NULL)return;
    int i=0;
    while(fgets(tmpbuff,255,f))
    {
        if(i==1)
        {
            i=0;
            p=NULL;
            p=strstr(tmpbuff,"Address");
            if(p!=NULL)
            {
                p=NULL;
                p=strchr(tmpbuff,':');
                if(p!=NULL)
                {
                    ...
                    p1=p;
                    while(*p!=' ' && *p!='\n' && *p!=0)p++;
                    if(p-p1>31||p-p1<4)continue;
                    strncpy(serverip,p1,p-p1);
                    serverip[p-p1]=0;
                    break;
                }
            }
        ...
        p=NULL;
        p=strstr(tmpbuff,"Name:");
        if(p!=NULL)
        {
            p=NULL;
            p=strstr(tmpbuff,DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+AERC_REPORT_NUM]);
            if(p!=NULL)i=1;
        }
    }
    fclose(f);

    if(serverip[0]!=0)inet_pton(AF_INET, serverip, &(eur_buff.addr_remote.sin_addr)); // Net Address
    else
    {
        syslog(LOGOPTS,"Can't find EURD Server's IP, program will try to update next send time.\n");
    }
}
...
static void init_socket()
{
    ...
    hp=gethostbyname(DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+AERC_REPORT_NUM]);
    if(hp==NULL)
    {
        syslog(LOGOPTS,"EURD Server's IP not found, program will try to update next send time.\n");
    }else
    {
        p = hp->h_addr_list;
        struct in_addr in;
        memcpy(&in.s_addr, *p, sizeof(in.s_addr));
        sprintf(serverip,"%s",inet_ntoa(in));
    }
...
static int check_updateconf(void)
{
    FILE * pFile;
    long lSize;
    char * buffer;
    ...
    DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RR][AERC_REPORT_NUM]=0;
    DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+AERC_REPORT_NUM][0]=0;
    DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_SEC0+AERC_REPORT_NUM][0]=0;
    DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RPORT0+AERC_REPORT_NUM][0]=0;
    DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+AERC_REPORT_NUM][0]=0;


    pFile = fopen ( "/etc/config/udprpt.conf" , "rb" );
    if (pFile==NULL) {
        return 1;
    }
    ...
    if (buffer == NULL) {
        syslog(LOGOPTS,"EURD: read /etc/config/udprpt.conf Memory error\n");
        return 1;
    }
    ...
    if (result != lSize) {
        syslog(LOGOPTS,"EURD: read /etc/config/udprpt.conf fread error\n");
        return 1;
    }
    ...
    // catch configuration from the memory buffer
    startp = strstr(buffer,"\"webUIRepId\":");
    if (startp) {
        endp = strchr(startp,',');
        if (endp) {
            startp += strlen("\"webUIRepId\":");
            strncpy(tmpb,startp,endp-startp);
            tmpb[endp-startp]='\0';
            syslog(LOGOPTS,"EURD report%d: webUIRepId=%s\n",AERC_REPORT_NUM,tmpb);
            i = atoi(tmpb);
            if (i != AERC_REPORT_NUM) {
                syslog(LOGOPTS,"EURD: webUIRepId=%d is not equal report_num=%d\n",i,AERC_REPORT_NUM);
                return 1;
            }
            if (i<0||i>3) {
                syslog(LOGOPTS,"EURD: webUIRepId=%d error\n",i);
                return 1;
            }
            DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RR][AERC_REPORT_NUM]='D';
        } else
            syslog(LOGOPTS,"EURD report%d: webUIRepId endp is NULL\n",AERC_REPORT_NUM);
    } else
        syslog(LOGOPTS,"EURD report%d: webUIRepId startp is NULL\n",AERC_REPORT_NUM);

    startp = strstr(buffer,"\"repServer\":\"");
    if (startp) {
        startp += strlen("\"repServer\":\"");
        endp = strchr(startp,'\"');
        if (endp) {

            strncpy(tmpb,startp,endp-startp);
            tmpb[endp-startp]='\0';
            strcpy(DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+i],tmpb);
            syslog(LOGOPTS,"EURD report%d: repServer=%s\n",AERC_REPORT_NUM,DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+i]);
        } else
            syslog(LOGOPTS,"EURD report%d: repServer endp is NULL\n",AERC_REPORT_NUM);
    } else
        syslog(LOGOPTS,"EURD report%d: repServer startp is NULL\n",AERC_REPORT_NUM);

    startp = strstr(buffer,"\"repPort\":");
    if (startp) {
        endp = strchr(startp,',');
        if (endp) {
            startp += strlen("\"repPort\":");
            strncpy(tmpb,startp,endp-startp);
            tmpb[endp-startp]='\0';
            strcpy(DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RPORT0+i],tmpb);
            syslog(LOGOPTS,"EURD report%d: repPort=%s\n",AERC_REPORT_NUM,DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RPORT0+i]);
        } else
            syslog(LOGOPTS,"EURD report%d: repPort endp is NULL\n",AERC_REPORT_NUM);
    } else
        syslog(LOGOPTS,"EURD report%d: repPort startp is NULL\n",AERC_REPORT_NUM);

    startp = strstr(buffer,"\"repInterval\":");
    if (startp) {
        endp = strchr(startp,',');
        if (endp) {
            startp += strlen("\"repInterval\":");
            strncpy(tmpb,startp,endp-startp);
            tmpb[endp-startp]='\0';
            strcpy(DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_SEC0+i],tmpb);
            syslog(LOGOPTS,"EURD report%d: repInterval=%s\n",AERC_REPORT_NUM,DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_SEC0+i]);
        } else
            syslog(LOGOPTS,"EURD report%d: repInterval endp is NULL\n",AERC_REPORT_NUM);
    } else
        syslog(LOGOPTS,"EURD report%d: repInterval startp is NULL\n",AERC_REPORT_NUM);

    startp = strstr(buffer,"\"desiredModules\":");
    if (startp) {
        endp = strstr(startp,"],");
        if (endp) {
            startp += strlen("\"desiredModules\":");
            strncpy(tmpb,startp,endp-startp);
            tmpb[endp-startp]='\0';
            if (strstr(tmpb,"Ethernet")==NULL) DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][0]='A';
            else DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][0]='B';
            if (strstr(tmpb,"Carrier")==NULL) DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][1]='A';
            else DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][1]='B';
            if (strstr(tmpb,"Radio")==NULL) DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][2]='A';
            else DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][2]='B';
            if (strstr(tmpb,"Com")==NULL) DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][3]='A';
            else DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][3]='B';
            if (strstr(tmpb,"DIDO")==NULL) DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][4]='A';
            else DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+i][4]='B';

            if (strstr(tmpb,"Vpn")==NULL) {
                ;//do nothing
            }

            if(DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RR][AERC_REPORT_NUM]=='D' && DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_REIP0+AERC_REPORT_NUM][0]!=0 && DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_RPORT0+AERC_REPORT_NUM][0]!=0 && DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_SEC0+AERC_REPORT_NUM][0]!=0 && DataBase1_Buff[ADVANCED_EUR_CONF][AERC_ITEM_MAS0+AERC_REPORT_NUM][0]!=0) 
            {
                if (write_Database(ADVANCED_EUR_CONF)==false) {
                    syslog(LOGOPTS,"EURD exit: update uci config error\n");
                    free (buffer);
                    exit(1);
                } else unlink("/etc/config/udprpt.conf");
                syslog(LOGOPTS,"EURD update udp report config from web service\n");
            }
            syslog(LOGOPTS,"EURD report%d: desiredModules=%s\n",AERC_REPORT_NUM,tmpb);
        } else
            syslog(LOGOPTS,"EURD report%d: desiredModules endp is NULL\n",AERC_REPORT_NUM);
    } else
        syslog(LOGOPTS,"EURD report%d: desiredModules startp is NULL\n",AERC_REPORT_NUM);
...
send_rightnow:
        ...
        } else if (eur_buff.mess_type_mask >= JSONINFO) 
        {
            ...
            if (eur_buff.product_flage == EURD_MAS_CONF_CHANGED || IP_changed_sign>0) {
...
#if defined DEBUG_PRINT 
printf("\n Now finished send %d Bytes.",num);
printf("\n==================%d - %s Now checking here at: %s\n",eur_buff.min_interval,last_carrierIP_str,ctime(&start));
#endif



            time(&end);
            diffsec=((end-start)>0?(end-start):0);
            //syslog(LOGOPTS,"EURD: diffsec=%d\n",diffsec);

            if (eur_buff.min_interval > diffsec) 
            {
                eur_buff.min_interval = eur_buff.min_interval - diffsec;
            } else
                eur_buff.min_interval = 0;

            //sleep_us(eur_buff.min_interval,0);
            //syslog(LOGOPTS,"EURD: min_inter=%d\n",eur_buff.min_interval);
            if(IP_changed_sign>0)IP_changed_sign--;
            while(eur_buff.min_interval>2)
            {
                eur_buff.min_interval--;
                eur_buff.min_interval--;
                sleep(2);
                if(check_carrierIP_changed()>0)
                {
                    IP_changed_sign=2;
                    goto send_rightnow;
                }
            }
            if(eur_buff.min_interval>0)sleep(eur_buff.min_interval);

            eur_buff.min_interval = eur_buff.interval_tm;
            // syslog(LOGOPTS,"EURDreport%d: end after sleep min_interval=%d\n",AERC_REPORT_NUM,eur_buff.min_interval);
    }//end for(;;)
