...
char r_rate_cmd[] = {0x04, 0x00, 0x00, 0x14, 0x02};
char w_rate_cmd[] = {0x05, 0x00, 0x00, 0x46, 0x02, 0x01};
char r_format_cmd[] = {0x04, 0x00, 0x00, 0x14, 0x13};
char w_format_cmd[] = {0x05, 0x00, 0x00, 0x46, 0x13, 0x01};

char reset_module_cmd[] = {0x03, 0x00, 0x00, 0xff};
...
int para_check()
{
    ...
    //send cmd to read baud rate 
    if (write(rfd, r_rate_cmd, 5) < 0) {
        fprintf(stderr, "ERROR: write (rfd=%d) baud rate failed, "
                "errno=%d\n", rfd, errno);
        return -1;
    }
    n=0;
    ...
    while (n<6) {
        ...
        tval1.tv_usec=0;
        tval1.tv_sec=1;
        if ((ret = select(rfd+1, &dfds, NULL, NULL, &tval1)) < 0) {
        ...
        if (FD_ISSET(rfd, &dfds)) {
            //read baud rate 
            if ((rn = read(rfd, res_buf+n, sizeof(res_buf)-n)) < 0) {
                fprintf(stderr, "ERROR: read(fd=%d) failed, "
                        "errno=%d\n", rfd, errno);
                return -1;
            }
            n+=rn;
            //printf("LOOP: first Baud Rate Response: %s\n", res_buf);
        }
    }
    // force baud rate to 115200 if it is not
    if (res_buf[5] != 0x01) {
        if (write(rfd, w_rate_cmd, 6) < 0) {
            fprintf(stderr, "ERROR: write (rfd=%d) baud rate failed, "
                    "errno=%d\n", rfd, errno);
            return -1;
        }
    ...
    usleep(10*1000);
    //send cmd to read data format 
    if (write(rfd, r_format_cmd, 5) < 0) {
        fprintf(stderr, "ERROR: write (rfd=%d) baud rate failed, "
                "errno=%d\n", rfd, errno);
        return -1;
    }
    ...
    while (n<6) {
        ...
        if (FD_ISSET(rfd, &dfds)) {
            ...
            n+=rn;
            //printf("LOOP: second format Rate Response: %s\n", res_buf);
        }
    }
    // force data formate to 8N1 if it is not
    if (res_buf[5] != 0x01) {
        if (write(rfd, w_format_cmd, 6) < 0) {
            fprintf(stderr, "ERROR: write (rfd=%d) data format failed, "
                    "errno=%d\n", rfd, errno);
            return -1;
        }
        usleep(100*1000);
        if (write(rfd, reset_module_cmd, 4) < 0) {
            fprintf(stderr, "ERROR: write (rfd=%d) module reset cmd failed, "
                    "errno=%d\n", rfd, errno);
            return -1;
        }
        usleep(100*1000);
...
int loopit()
{
    ...
    for (;;) {
        ...
        tval.tv_usec=0;
        tval.tv_sec=1;
        if (select(maxfd, &infds, NULL, NULL, &tval) < 0) {
            fprintf(stderr, "ERROR: select() failed, errno=%d\n",
                    errno);
            return -1;
        }
        ...
        if (current -last > 1) {
            ...
            // force data format to 8N1 if it is not
            if (res_buf[5] != 0x01) {
                if (write(rfd, w_format_cmd, 6) < 0) {
                    fprintf(stderr, "ERROR: write (rfd=%d) data format failed, "
                            "errno=%d\n", rfd, errno);
...
int main(int argc, char *argv[])
{
    ...
    // check and set para if it should be
    para_check();
    ...
    if (tcpport) {
        ...
        for (;;) {
            ...
            while ((ifd = accept(fd, &p, &plen)) < 0) {
                ...
                current = time(NULL);
                if (current - last > 1) {
                    fprintf(stderr, "Call Para_check().\n ");
                    para_check();
                    last = current;
                }
                //sleep(1);             
