...
prepare_interface() {
	...
	case "$iftype" in
		bridge)
			[ -x /usr/sbin/brctl ] && {
				if [ ! -z "$(ifconfig br-$config 2>/dev/null)" ] 
				then 
					...
					if [ "$bonding" -gt 0 ]; then 
					    ifconfig bond0 up 2>/dev/null >/dev/null
					    ifenslave bond0 "$iface" 2>/dev/null >/dev/null
					    $DEBUG brctl addif "br-$config" bond0 2>/dev/null >/dev/null
					else
...
setup_interface() {
	...
	prepare_interface "$iface_main" "$config" "$vifmac" "$bonding"|| return 0
	...
	case "$proto" in
		...
		;;
		dhcp)
                        vip4g_status=$(uci get lte.connect.disabled)
                        #if [ "$config" = "wan2" -a "$vip4g_status" = "0" ]; then
                        if [ "$config" = "wan2" ]; then
                                logger -t "dhcp client" "4g enabled. using dhcp_client to setup dhcp"
                        else 
        			...
        			if [ -d "/proc/$pid" ] && grep udhcpc "/proc/${pid}/cmdline" >/dev/null 2>/dev/null; then
        				...
        				$DEBUG eval udhcpc -A 18 -t 0 -i "$iface_main" ${ipaddr:+-r $ipaddr} ${hostname:+-H $hostname} ${clientid:+-c $clientid} -b -p "$pidfile" ${dhcpopts:- -O rootpath -R &}
        				lock -u "/var/lock/dhcp-$iface_main"
        			fi
                        fi
		;;
