...
static bool read_configdata()
{
    int j;
    //gpsr config
    for(j=0;j<ADVANCED_GPS_CONF_REPORT_NUM;j++)get_option_by_section_name(ctx, "gpsr","gpsr_conf",Item_Keywords[ADVANCED_GPS_CONF_REPORT][j], DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][j]);

    //smtp config
    for(j=0;j<GPS_SUB_MENU_SMTP_NUM;j++)get_option_by_section_name(ctx, "gpsr","gpsr_conf",Item_Keywords[GPS_SUB_MENU_SMTP][j], DataBase1_Buff[GPS_SUB_MENU_SMTP][j]);
...
static bool check_sockbuf(int index)
{
    if(DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_Local_Stream_Set0+index][0]!='B')return true;

    //ippass_check---script: config and ippth_4g.---->gpslocalip
    //here read file: /tmp/run/gpslocalip   --------if it is OK, change

    //--------------if it is OK, change to DataBase1_Buff
    //0.read file: /tmp/run/gpsarpingend and unlink for start and end.
    //
    //1.arping_check last_DataBase1_Buff_ip;--->/tmp/run/gpsarpingresult
    //
    //2.arping_more: get arping list and check one by one.  --->/tmp/run/gpsarpingresult
    //
    //3.arping_scan: scan background and return false.  check /tmp/run/gpsarpingstart  gpsarpingend
    ...
    SubProgram_Start(gpsrscriptFile,"ippass_check");
    f=NULL;
    if (f = fopen("/tmp/run/gpslocalip", "r"))
    {
        tmp_buff[0]=0;
        if(fgets(tmp_buff, 255, f))
        {
            if(strlen(tmp_buff)>7)
            {
                strcpy(DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index],tmp_buff);
                fclose(f);
                goto set_new_ip;
            }
        }
    ...
    sprintf(tmp_buff,"arping_check %s",DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index]);
    SubProgram_Start(gpsrscriptFile,tmp_buff);
    f=NULL;
    if (f = fopen("/tmp/run/gpsarpingresult", "r"))
    {
        tmp_buff[0]=0;
        if(fgets(tmp_buff, 255, f))
        {
    ...
    SubProgram_Start(gpsrscriptFile,"arping_more");
    f=NULL;
    if (f = fopen("/tmp/run/gpsarpingresult", "r"))
    {
        tmp_buff[0]=0;
    ...
    //printf("*******scanning now*****\n");
    SubProgram_Start(gpsrscriptFile,"arping_scan");
    f=NULL;
    if (f = fopen("/tmp/run/gpsarpingresult", "r"))
    {
        tmp_buff[0]=0;
...
set_new_ip:
    ...
    while(i<20)
    {
        c_one=DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index][i];
        if(c_one=='\r' || c_one=='\n' || c_one==' ' || c_one==0 )
        {
            DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index][i]=0;
            break;
        }
        i++;
    }
    inet_pton(AF_INET, DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index], &(sockbuf[index].addr_remote.sin_addr)); // Net Address
    bzero(&(sockbuf[index].addr_remote.sin_zero), 8);                  // Flush the rest of struct
    //printf("******send to IP:%s\n",DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_REIP0+index]);
    return true;
}
        ...
        if (fetch_one_pattern(buf,pbuf)!=0) {
            ...
            for (i=0;i<count;i++) {
                ...
                if (DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_RR][enabled_report[i]]=='B') {
                    ...
                    if(check_sockbuf(enabled_report[i])==true)sockbuf[enabled_report[i]].num = sendto(sockbuf[enabled_report[i]].sockfd, sbuf, strlen(sbuf), 0, (struct sockaddr *)&(sockbuf[enabled_report[i]].addr_remote), sizeof(struct sockaddr_in));
                    else sockbuf[enabled_report[i]].num=0;
...
//printf("##Update pos1: %d##\n",i);
        ...
        bzero(buf,sizeof(buf));
        if (start1.tv_sec>2) {
            sleep_us(start1.tv_sec-2,start1.tv_usec);
...
//printf("**dis %d trigger%d %s*%f,%f--%f,%f\n",int_dis,item,ctime(&test_time),tmp_1,tmp_2,base_gps_pos.lat,base_gps_pos.lng);
                ...
                if (DataBase1_Buff[ADVANCED_GPS_CONF_REPORT][AGCR_ITEM_RR][item]=='B') {
                    ...
                    if(check_sockbuf(item)==true)sockbuf[item].num = sendto(sockbuf[item].sockfd, sbuf, strlen(sbuf), 0, (struct sockaddr *)&(sockbuf[item].addr_remote), sizeof(struct sockaddr_in));
                    else sockbuf[item].num =0;
//time(&test_time);
