...
for gre in $gre_cfgs; do
  ...
  if ! empty "$FORM_connect" ; then
       [ -f "/etc/debugipsec" ] && logger -t "gre-summary.sh" "$FORM_connect is not empty and action"
...
form="  string|<div class=\"settings\">
	string|<table style=\"width: 90%; margin-left: 2.5em; text-align: left; font-size: 0.8em;\" border=\"0\" cellpadding=\"3\" cellspacing=\"2\" summary=\"@TR<<GRE Configuration Summary>>\">
	string|<th>@TR<<No.>></th>
...
for gre in $gre_cfgs; do
    ...
    config_get FORM_mroutename              $gre mroutename
   ...
   if [ "$FORM_connect_status" = "Waiting..." -o "$FORM_connect_status" = "N/A" ]; then
      ...
      form="string|<td>
            ...
            string|<td>
            string|<a href=\"/cgi-bin/webif/network-gre-edit.sh?editname=$gre&mname=$FORM_mroutename&eipsec=$FORM_enableipsec&oinf=$FORM_interface\">@TR<<Edit>></a>
            string|</td>
