...
/*
 *	ublox.c -- simple ublox protocol simulation program based on tip/cu program.
 *
 *	(C) Copyright 2010, Microhard Systems Inc(www.microhardcorp.com)
 *
...
/*
 *     Translate the user entered packet into a ublox packet
 *     the buffer has to be pre-allocated
 *
 */
int translate_to_ublox( unsigned char *ublox_buf, char *user_str )
{
	int ret = 1;
	int payload_len = 0;
	char * pch;
	...
	/* ublox sync bytes */
	ublox_buf[offset++] = 0xb5;
	ublox_buf[offset++] = 0x62;

	//printf("BS DEBUG: input_buf=%s\n", input_buf);

	pch = strtok(user_str, " ,.\n");
	while (pch != NULL)
	{
		sscanf(pch, "%x", &this_byte);
		ublox_buf[offset++] = this_byte;
		pch = strtok (NULL, " ,.\n");
	}

	/* validate the payload length */
	payload_len = ublox_buf[5]; /* LSB */
	payload_len *= 256;
	payload_len += ublox_buf[4];

	//printf("BS DEBUG: ublox_buf=%02x %02x %02x %02x %02x %02x %02x\n", ublox_buf[0], ublox_buf[1], 
	//		ublox_buf[2], ublox_buf[3], ublox_buf[4], ublox_buf[5], ublox_buf[6]);

	//printf("BS DEBUG: offset=%d payload_len=%d\n", offset, payload_len);

	if(payload_len + 6 != offset) ret =0;
...
/*
 *     Padding the checksum to a null-terminated string
 *     the buffer has to be pre-allocated
 *
 */
int padding_crc( unsigned char *buf )
{
	int ret = 0;
	unsigned char *p = buf;
	unsigned char ck_a = 0;
	unsigned char ck_b = 0;
	int len = 0; /* note: this is the payload length */
	int i;

	if( *p++ != 0xb5 ) return ret;

	if( *p++ != 0x62 ) return ret;

	/* this is a valid ublox packet 
	   let's find out the packet len to 
...
int loopit()
{
	...
	int	maxfd, n;
	int	partialescape = 0;
	unsigned char    ublox_buf[1000];
	int ublox_remaining = 0;
	int ublox_offset = 0;
	
	memset(input_buf, 0, sizeof(input_buf));
	memset(ublox_buf, 0, sizeof(ublox_buf));
	...
	for (;;) {
		...
		if (FD_ISSET(rfd, &infds)) {
			bp = ibuf;
			n = sizeof(ibuf);
			if(ublox_remaining > 0) n = ublox_remaining;
			if ((n = read(rfd, &(ibuf[ublox_offset]), n)) < 0) {
			...
			if ( (unsigned char)bp[0] == 0xb5 && (unsigned char)bp[1] == 0x62 )
			{
                                int i;
				char temp[10];
				...
				if( ublox_offset < 6 ) /* payload length bytes have not shown up yet */
				{
					/* read 2 more bytes */
					ublox_remaining = 2;
				}
				...
				{
					ublox_remaining = bp[5];
					ublox_remaining *= 256;
					ublox_remaining += bp[4]; /* now it is the payload length */
					ublox_remaining += 8; /* now it is the total packet len including the checksum */
					ublox_remaining -= ublox_offset;
					if(ublox_remaining <= 0)
					{
						ublox_remaining = 0;
					}
				}

				/* the packet is assembled and we are ready to output it */
				/* since it is not readable we are going to convert it*/
				if(ublox_remaining <= 0)
				{
	                                sprintf(output_buf, "\nReceived UBlox packet:");
	                                for(i=0; i<ublox_offset; i++)
					{
        	                            sprintf(temp, "%02x ", bp[i]);
					    strcat(output_buf, temp);
					}

					/* since it is not readable */	
					if (write(ofd, output_buf, strlen(output_buf)) < 0) {
						fprintf(stderr, "ERROR: write(fd=%d) failed, "
							"errno=%d\n", 1, errno);
						exit(1);
					}
					if (cfd > 0)
						write(cfd, output_buf, strlen(output_buf));
		...
		if (FD_ISSET(ifd, &infds)) {
			...
			if (n == 0) {
				fprintf(stderr, "read(ifd=%d) zero.\n", rfd);
				break;
		        }
#ifdef ALTERNATE
			if ((n == 1) && (*bp == 26))
			...
			if ((n == 1) && (*bp == 0x1))
				break;
#endif
			if ((n == 1) && (*bp == 0x1b)) //ESC exit from loop
				break;
			...
			/* looking for the '\n' as the end symbol */
			if(*(bp+n-1) == 0x0d || *(bp+n-1) == 0x0a)
			{
				if( !translate_to_ublox(ublox_buf, input_buf) )
				{
					fprintf(stderr, "ERROR: invalid ublox packet is entered\n");
					memset(input_buf, 0, sizeof(input_buf));
					memset(ublox_buf, 0, sizeof(ublox_buf));
				...
				/* reset the buffer for next packet */
				memset(input_buf, 0, sizeof(input_buf));
				memset(ublox_buf, 0, sizeof(ublox_buf));
			}
		}
