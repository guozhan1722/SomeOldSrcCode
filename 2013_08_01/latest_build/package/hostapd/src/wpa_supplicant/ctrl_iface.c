...
static int wpa_supplicant_ctrl_iface_ap_freq(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int ap_freq = atoi(cmd);
    wpa_s->conf->ap_freq = ap_freq;
    return 0;
}
#endif
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_supplicant_ctrl_iface_ap_scan(wpa_s, buf + 8))
			reply_len = -1;
#ifdef MICROHARD_AP_FREQ
   } else if (os_strncmp(buf, "AP_FREQ ", 8) == 0) {
        if (wpa_supplicant_ctrl_iface_ap_freq(wpa_s, buf + 8))
            reply_len = -1;
#endif
