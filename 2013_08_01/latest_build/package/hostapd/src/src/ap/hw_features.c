...
#else
    int basic_rates_g[] = { 60, 120, 240, -1 };
#endif
	int *basic_rates;
...
{
#ifndef CONFIG_MICROHARD_IEEE80211N_NO_SWITCH_PRI_SEC
	if (iface->conf->secondary_channel > 0) {
...
#else
	wpa_printf(MSG_INFO, "*** INGORE SWITCH CHANNEL, FORCE TO CONTINUE (%d, %c) ***", 
		iface->conf->channel, iface->conf->secondary_channel>1?'+':'-');
#endif
}
...
#ifndef CONFIG_MICROHARD_IEEE80211N_FORCE_40MHZ
		iface->conf->secondary_channel = 0;
		iface->conf->ht_capab &= ~HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET;
#else
		wpa_printf(MSG_INFO, "*** IGNORE ABOVE MESSAGE, FORCE TO 20/40 MHz OPERATION ***");
#endif
	}
