			...
			int val)
{
	...
	os_snprintf(name, sizeof(name), "%s.sta%d", hapd->conf->iface, aid);
#endif
...
#ifdef MICROHARD
/* this is a bug fixing to fix the wds bridge wep encryption issue */
	ret = hapd->driver->set_wds_sta(hapd->drv_priv, addr, aid, val, bridge);

	if(val)
          for (i = 0; i < 4; i++) {
                if (hapd->conf->ssid.wep.key[i] &&
                    hostapd_drv_set_key(name, hapd, WPA_ALG_WEP, NULL, i,
