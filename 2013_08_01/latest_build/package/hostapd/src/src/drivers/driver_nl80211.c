...
struct wifpara {
  ...
  unsigned char curntmd; unsigned char cardmod;
};
	...
	nla_for_each_nested(nl_band, tb_msg[NL80211_ATTR_WIPHY_BANDS], rem_band) {
		...
		*(phy_info->num_modes) += 1;
		...
		nla_for_each_nested(nl_freq, tb_band[NL80211_BAND_ATTR_FREQS], rem_freq) {
			...
			{
			case 1:
			   tmp = mode->channels[idx].freq*1000 - wifa.lfreq;
			   if((!(wifa.lfreq%1000))&&(tmp/2500 == 1)) mode->channels[idx].chan =  1;
		           else if(tmp%2500) mode->channels[idx].chan = (tmp+500)/2500;
			   else mode->channels[idx].chan = tmp/2500; break;
			case 2:
		  	case 3:
			case 5:
			  mode->channels[idx].chan = (mode->channels[idx].freq*1000 - wifa.lfreq)/wifa.bandwidth;
			  break;
            case 4:
                tmp = mode->channels[idx].freq*1000 - wifa.lfreq;
                if((!(wifa.lfreq%1000))&&(tmp/2500 == 1))
                    mode->channels[idx].chan =  5;
                else if(tmp%2500) mode->channels[idx].chan = 5*(tmp+500)/2500;
                else mode->channels[idx].chan = 5*tmp/2500;			  
                break;
			default:
			  /* crude heuristic */
			  if (mode->channels[idx].freq < 4000)
