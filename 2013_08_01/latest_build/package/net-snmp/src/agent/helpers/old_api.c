                         ...
                         const char *context, int timeout, int flags)
{
    ...
    for (i = 0; i < numvars; i++) {
		...
		//if(strcmp(moduleName,"VIP_MIB")==0){
		//	syslog(LOGOPTS, "[netsnmp_register_old_api ]The var [i = %d ] [ magic =%d]\n",i,vp->magic);
		//}
        /*
        ...
        if ( retval!= MIB_REGISTERED_OK) {
			syslog(LOGOPTS, "[netsnmp_register_old_api ] ################ magic %d can not regist ret =%d\n",vp->magic,retval);
