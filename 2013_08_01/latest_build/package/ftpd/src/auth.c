...
#include	<radiusclient_pathnames.h>
#include	<radlogin.h>
...
int
auth_pass ( char *passwd, struct credentials *pcred)
	{
	switch (pcred->auth_type)
		{
#ifdef WITH_PAM
		...
		default:
			{
				...
				int     maxtries = 0;
				char        *ttyn  = NULL;
				char            *path_radiusclient_conf = "/etc/radiusclient-ng/radiusclient.conf";//RC_CONFIG_FILE;
				...
				if (rc_conf_int(rh, "auth_order") & AUTH_LOCAL_FST)
					{
					login_func = auth_local(user, passwd);         
					if (!login_func)
						if (rc_conf_int(rh, "auth_order") & AUTH_RADIUS_SND)
							login_func = auth_radius(rh, client_port, user, passwd);
					}
				else
					{
					login_func = auth_radius(rh, client_port, user, passwd);
...
sgetcred (const char *name, struct credentials *pcred)
	...
	pcred->uid = p->pw_uid;
	pcred->gid = p->pw_gid;
	pcred->name =strdup(name);// sgetsave (p->pw_name);
   	pcred->passwd = sgetsave (p->pw_passwd);
	pcred->rootdir = sgetsave (p->pw_dir);

	pcred->homedir = sgetsave ("/upgrade/upgrade");
