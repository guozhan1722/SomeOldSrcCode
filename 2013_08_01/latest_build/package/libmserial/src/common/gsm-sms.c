/*
  ...
  $Id: gsm-sms.c,v 1.171 2007/11/07 18:28:18 pkot Exp $
                                ...
                                unsigned int size, unsigned int udhlen, gn_sms_dcs dcs)
{
    ...
    dprintf("%s\n", output);
    dprintf("sms_data_decode return\n");
    return GN_ERR_NONE;
}
...
GNOKII_API gn_error gn_sms_delete_option(gn_data *data, struct gn_statemachine *state)
{
    gn_sms_raw rawsms;
    ...
    data->raw_sms = &rawsms;
    return gn_sm_functions(GN_OP_DeleteSMSOption, data, state);
}

GNOKII_API gn_error gn_smslist_get(gn_data *data, struct gn_statemachine *state)
{
    gn_sms_raw rawsms;
    ...
    data->raw_sms = &rawsms;
    if (!data->raw_sms) return GN_ERR_INTERNALERROR;
    return gn_sm_functions(GN_OP_GetSMSList, data, state);
}
/**
 ...
 * in the gn_data structure must be initialized.
 */
GNOKII_API gn_error gn_sms_get_pdu(gn_data *data, struct gn_statemachine *state)
{
    gn_error error;
    gn_sms_raw rawsms;
...
GNOKII_API char *gn_sms2mbox(gn_sms *sms, char *from)
{
    ...
    if (!tmp)
        goto error;
    strftime(tmp, MAX_DATE_LENGTH - 1, "Date/time: %d/%m/%Y %H:%M:%S %z (%Z)\n", loctime);
#ifdef ENABLE_NLS
    setlocale(LC_ALL, loc);
#endif
    APPEND(buf, tmp, size);
    CONCAT(buf, tmp, size, "From: %s\n", sms->remote.number);
#if 0
...
GNOKII_API char *gn_sms2cgi(gn_sms *sms, char *dt, char *subject)
{
    struct tm t, *loctime;
    time_t caltime;
    int size = 0;
    ...
    //memset(subject, 0, 25*sizeof(char));
    //memset(dt, 0, 32*sizeof(char));
    if (!subject) goto error;
    if (!dt) goto error;
    dprintf("goto gn_sms2cgi\n");
    t.tm_sec = sms->smsc_time.second;
    t.tm_min = sms->smsc_time.minute;
    t.tm_hour = sms->smsc_time.hour;
    t.tm_mday = sms->smsc_time.day;
...
dprintf("getlocaltime\n");
    ...
    dprintf("calloc tmp\n");
    sprintf(tmp,"<div id='sms%d' name='smscontent' style='display:none;'><pre><hr style='width:800;' align='left'>\n",sms->number);
    APPEND(buf, tmp, size);
    CONCAT(buf, tmp, size, "<input type='submit' name='subdel%d' value='Delete'><input type='submit' name='reply%d' value='Reply'>\n", sms->number, sms->number);
    strftime(dt, 31, "%d/%m/%Y %H:%M:%S %z (%Z)\n", loctime);
#ifdef ENABLE_NLS
    setlocale(LC_ALL, loc);
#endif

    CONCAT(buf, tmp, size, "Date/time: %s\n", dt);
    CONCAT(buf, tmp, size, "From: %s\n", sms->remote.number);
#if 0
    CONCAT(buf, tmp, size, "X-GSM-SMSC: %s\n", sms->smsc.number);
    CONCAT(buf, tmp, size, "X-GSM-Status: %s\n", status2str(sms->status));
    ...
    if (strlen(sms->user_data[0].u.text) < MAX_SUBJECT_LENGTH) {
        CONCAT(buf, tmp, size, "Subject: %s\n\n", sms->user_data[0].u.text);
        sprintf(subject, "%s", sms->user_data[0].u.text);
    } else {
        aux = calloc(MAX_TEXT_SUMMARY + 1, sizeof(char));
        if (!aux)
            goto error;
        ...
        CONCAT(buf, tmp, size, "Subject: %s...\n\n", aux);
        sprintf(subject, "%s...", aux);
        free(aux);
    }

    CONCAT(buf, tmp, size, "%s\n</pre><hr style='width:800;' align='left'></div>\n", sms->user_data[0].u.text);
    
    //snprintf(subject,MAX_TEXT_SUMMARY, "%s", sms->user_data[0].u.text);
    #if 0
    fprintf(cgiOut, "<tr name='row' onclick=\"showsms(this,'#C0C0C0','sms%d');\"><td align='center'><input type='checkbox' name='check%d'></td><td>%s</td><td>%s</td><td>%s</td></tr>\n",
            sms->number, sms->number, sms->remote.number, 
            subject,
            dt);
#endif
    dprintf("return buf\r\n%s\r\n",buf);
