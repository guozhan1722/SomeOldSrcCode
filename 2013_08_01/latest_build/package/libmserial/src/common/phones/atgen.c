/*
  ...
  $Id: atgen.c,v 1.163 2007/11/07 18:28:19 pkot Exp $
...
static gn_error ReplyGetSMSList(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state);
...
static gn_error AT_SetTextMode(gn_data *data, struct gn_statemachine *state);
static gn_error AT_AT(gn_data *data, struct gn_statemachine *state);
...
static gn_error AT_GetSMSList(gn_data *data, struct gn_statemachine *state);
...
static gn_error AT_SetSMSCenter(gn_data *data, struct gn_statemachine *state);
...
static gn_error AT_DeleteSMSOption(gn_data *data, struct gn_statemachine *state);
...
static at_function_init_type at_function_init[] = {
    ...
    { GN_OP_AT_SetTextMode,        AT_SetTextMode,           Reply},
    { GN_OP_AT_AT,                 AT_AT,                    Reply},
    ...
    { GN_OP_SetSMSCenter,          AT_SetSMSCenter,          Reply},
#if 0
#ifdef SECURITY
    ...
    { GN_OP_DeleteSMSOption,       AT_DeleteSMSOption,       Reply},
    { GN_OP_GetSMSList,            AT_GetSMSList,            ReplyGetSMSList},
};
...
static gn_error SetTextMode(gn_data *data, struct gn_statemachine *state)
{
    if (sm_message_send(10, GN_OP_Init, "AT+CMGF=1\r", state)) return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_Init, data, state);
}

static gn_error SetPDUMode(gn_data *data, struct gn_statemachine *state)
{
    if (sm_message_send(10, GN_OP_Init, "AT+CMGF=0\r", state)) return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_Init, data, state);
}
...
static gn_error AT_SetTextMode(gn_data *data, struct gn_statemachine *state)
{
    if (sm_message_send(10, GN_OP_AT_SetTextMode, "AT+CMGF=1\r", state))
        return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_AT_SetTextMode, data, state);
}

static gn_error AT_AT(gn_data *data, struct gn_statemachine *state)
{
    if (sm_message_send(3, GN_OP_AT_AT, "AT\r", state))
        return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_AT_AT, data, state);
}
...
static gn_error AT_GetSMS(gn_data *data, struct gn_statemachine *state)
{
    ...
    dprintf("PDU mode set\n");
    if (state->config.connect_script[0]==0)
        snprintf(req, sizeof(req), "AT+CMGR=%d\r", data->raw_sms->number);
    else
        snprintf(req, sizeof(req), "AT+MMGR=%d\r", data->raw_sms->number);
...
static gn_error AT_GetSMSList(gn_data *data, struct gn_statemachine *state)
{
    ...
    if (state->config.connect_script[0]==0) snprintf(cmd, sizeof(cmd), "AT+CMGL");
    else snprintf(cmd, sizeof(cmd), "AT+MMGL");

    state->config.connect_script[0] = 0;
    switch (data->raw_sms->number) {
    case 0:
        snprintf(req, sizeof(req), "%s=0\r",cmd);
        break;
    case 1:
        snprintf(req, sizeof(req), "%s=1\r",cmd);
        break;
    case 4:
        snprintf(req, sizeof(req), "%s=4\r",cmd);
        break;
    default:
        snprintf(req, sizeof(req), "%s=0\r",cmd);
        break;
    ...
    if (sm_message_send(strlen(req), GN_OP_GetSMSList, req, state))
        return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_GetSMSList, data, state);
}
...
static gn_error AT_DeleteSMSOption(gn_data *data, struct gn_statemachine *state)
{
    ...
    if (sm_message_send(strlen(req), GN_OP_DeleteSMSOption, req, state))
        return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_DeleteSMSOption, data, state);
}
/* 
...
static gn_error AT_SetSMSCenter(gn_data *data, struct gn_statemachine *state)
{
    ...
    snprintf(req, sizeof(req), "AT+CSCA=\"%s\",%d\r", data->sms->smsc.number, data->sms->smsc.type);
    if (sm_message_send(strlen(req), GN_OP_SetSMSCenter, req, state))
        return GN_ERR_NOTREADY;
    return sm_block_no_retry(GN_OP_SetSMSCenter, data, state);
}
#if 0
    #ifdef SECURITY
static gn_error AT_GetSecurityCodeStatus(gn_data *data, struct gn_statemachine *state)
...
static gn_error AT_GetSMSMemorySize(gn_data *data, struct gn_statemachine *state)
{
    if (sm_message_send(18, GN_OP_AT_GetSMSMemorySize, "AT+CPMS=\"SM\",\"ME\"\r", state))
        return GN_ERR_NOTREADY;
...
static gn_error ReplyGetSMSStatus(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    dprintf("buf.line2: %s\n",buf.line2);
    if (sscanf(buf.line2, "+CPMS: \"%*c%*c\",%d,%d,%*s", &data->sms_status->unread, &data->sms_status->number) != 2)
        return GN_ERR_FAILED;
    dprintf("sms_status->number=%d, sms_status->unread=%d\n",data->sms_status->number, data->sms_status->unread);
...
static gn_error ReplySendSMS(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    dprintf("buf.line1: %s\n",buf.line1);
    /* SendSMS or SaveSMS */
    if (!strncmp("+CMGW:", buf.line1, 6) ||
        !strncmp("+CMGS:", buf.line1, 6))
        data->raw_sms->reference = atoi(buf.line1 + 6);
    else
...
static gn_error ReplyGetSMS(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    if (state->config.disconnect_script[0]==1) {
        unsigned char *pos;
        pos = findcrlf(buffer+1, 0, length);
        if (pos) {
            *pos = 0;
            tmp = skipcrlf(++pos);
        } else {
        ...
        pos = strstr(tmp,"OK");
        if (pos) sms_len = pos-tmp;
        else sms_len = length;

        data->raw_sms->user_data_length = (sms_len > GN_SMS_LONG_MAX_LENGTH) ? GN_SMS_LONG_MAX_LENGTH : sms_len;

        memcpy(data->raw_sms->user_data, tmp, data->raw_sms->user_data_length);
        dprintf("rawdata: %s\n",data->raw_sms->user_data);

        return GN_ERR_NONE;
    }
    state->config.disconnect_script[0] = 0;
...
static gn_error ReplyGetSMSList(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    pos = strstr(tmp,"OK");
    if (pos) data->raw_data->length = pos-tmp;
    else data->raw_data->length = length;

    data->raw_data->data = calloc(data->raw_data->length,sizeof(char));
    if (data->raw_data->data) {
        dprintf("empty user_data: %s",data->raw_data->data);
        memcpy(data->raw_data->data,tmp,data->raw_data->length);
        data->raw_data->full = true;
        dprintf("user_data: %s",data->raw_data->data);
    } else {
        ret = GN_ERR_MEMORYFULL;
    }
...
static gn_error ReplyGetNetworkInfo(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    if (!strncmp(buf.line1, "AT+CREG?", 8)) {
        data->raw_data->length = strlen(buf.line2);

        data->raw_data->data = calloc(data->raw_data->length,sizeof(char));
        if (data->raw_data->data) {
            memcpy(data->raw_data->data,buf.line2,data->raw_data->length);
            data->raw_data->full = true;
            dprintf("user_data: %s",data->raw_data->data);
            return GN_ERR_NONE;
        } else {
...
static gn_error ReplyGetDateTime(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    strcpy(data->raw_sms->user_data, buf.line2);
...
static gn_error ReplyGetSMSMemorySize(int messagetype, unsigned char *buffer, int length, gn_data *data, struct gn_statemachine *state)
{
    ...
    if (sscanf(buf.line2, "+CPMS: %*d,%d,%*d,%d", &drvinst->smmemorysize, &drvinst->mememorysize) != 2)
        return GN_ERR_FAILED;
    drvinst->smsmemorytype = GN_MT_SM;
...
static gn_error Initialise(gn_data *setupdata, struct gn_statemachine *state)
{
    ...
    if (state->config.disconnect_script[0]==1)
        SetPDUMode(&data, state);
    else
        SetTextMode(&data, state);
    state->config.disconnect_script[0]=0;
    /*
