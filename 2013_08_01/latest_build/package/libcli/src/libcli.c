...
int cli_show_top_help(struct cli_def *cli, struct cli_command *c)
{
    struct cli_command *p;
    ...
    for (p = c; p; p = p->next)
    {
        ...
        if (p->children)
            cli_show_top_help(cli, p->children);
    }
...
{
    cli_error(cli, "\nPress Tab to fill in the rest of the Commands\n");
    cli_error(cli, "\nCommands available:\n");
    cli_show_top_help(cli, cli->commands);
...
static int cli_at_parse_line(char *line, char *words[], int max_words)
{
    char *p = line;
    char *word_start = 0;
    char *eqp = 0;
    ...
    eqp=strchr(word_start,'=');
    /*
     * AT<command_name>?
     * AT<command_name>
     */
    if (eqp == NULL) {
        mp=strchr(word_start,'?');
        if (word_start) {
            int len = 0;
            if (mp == NULL) {
                len = strlen(word_start);
                memcpy(words[nwords] = malloc(len + 1), word_start, len);
                words[nwords++][len] = 0;
            } else {
                len = mp - word_start;
                memcpy(words[nwords] = malloc(len + 1), word_start, len);
                words[nwords++][len] = 0;
                memcpy(words[nwords] = malloc(2),"?",1);
                words[nwords++][1] = 0;
            }
     ...
     * find first '=' to store AT<command_name>= to words[0]
     * parameter'i' be stored into words[i]
     */
    else {
        int len = eqp - word_start;
        ...
        word_start = eqp+1;
        while (nwords < max_words - 1) {
            if (!*p || *p == inquote || (word_start && !inquote && *p == ',')) {
                if (word_start) {
                    int len = p - word_start;
...
static int cli_find_at_command(struct cli_def *cli, struct cli_command *commands, int num_words, char *words[], int start_word)
{
    struct cli_command *c;
    int c_words = num_words;
    int find = 0;
    ...
    for (c = commands; c; c = c->next) {
        ...
        int rc = CLI_OK;
        int len = strlen(words[start_word]);
        // Found a word!
        if (!c->callback) {
            cli_error(cli, "No callback for \"%s\"", cli_command_name(cli, c));
            return CLI_ERROR;
        }
        if (words[start_word][len-1]=='=' && c_words==1) {
            cli_error(cli, "Incomplete command, need at least one parameter");
            return CLI_ERROR;
        }
    ...
    if (!find)
        cli_error(cli, "Invalid command \"%s\"",words[start_word]);

    return CLI_ERROR;
}


int cli_run_at_command(struct cli_def *cli, char *command)
{
    int r;
    unsigned int num_words, i, f;
    char *words[CLI_MAX_LINE_WORDS] = {0};
    ...
    num_words = cli_at_parse_line(command, words, CLI_MAX_LINE_WORDS);

    filters[f] = 0;

    if (num_words)
        r = cli_find_at_command(cli, cli->commands, num_words, words, 0);
    else
        r = CLI_ERROR;
...
int cli_loop(struct cli_def *cli, int sockfd)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            //add with AT command type, find first "=" replace with " " as the first parameters.
                if(strncmp(cmd,"AT+",3)==0)
                {
                  if (cli_run_at_command(cli, cmd) == CLI_QUIT)
                  break;
                }else
                {
                ...
                if(at_flag==3)
                {
                    cli->idle_timeout=at_parameter_timeout;
                    at_flag=0;
                }
