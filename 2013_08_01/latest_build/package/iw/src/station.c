...
static int print_sta_handler(struct nl_msg *msg, void *arg)
{
...
#ifdef MICROHARD
        if (sinfo[NL80211_STA_INFO_RX_DROPPED])
                printf("\n\trx dropped:\t%u",
                        nla_get_u32(sinfo[NL80211_STA_INFO_RX_DROPPED]));
#endif
	...
	if (sinfo[NL80211_STA_INFO_SIGNAL_AVG])
        ...
        if (sinfo[NL80211_STA_INFO_RSSI])
                printf("\n\trssi:\t\t%d",
                        (int8_t)nla_get_u8(sinfo[NL80211_STA_INFO_RSSI]));
#endif
