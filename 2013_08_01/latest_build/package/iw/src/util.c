...
int ieee80211_channel_to_frequency(int chan)
{
  ...
  memset(&wifa, 0 , sizeof(wifa));
  pdev= open(MDFDEV,O_RDWR);
  if (pdev)
  {    
      read(pdev, &wifa, sizeof(wifa));  
      close(pdev); 
      mdfmode = wifa.curntmd;  
  }
...
int ieee80211_frequency_to_channel(int freq)
{
  ...
  {
    case 1: // 4940-4990, 2.5M per channel
        tmp = freq*1000 - wifa.lfreq;
        if((!(wifa.lfreq%1000))&&(tmp == 3000))return 1;
        if(tmp%2500) return (tmp+500)/2500; 
        else return tmp/2500; 
    case 2: // 5.8G
    case 3: // 2.4G
        return (freq - wifa.lfreq/1000)/5;
    case 4: // 4940-4990, 2.5M per channel, Microhard standard
        tmp = freq*1000 - wifa.lfreq;
        if((!(wifa.lfreq%1000))&&(tmp == 3000))return 5;
