...
static int handle_retry_limit(struct nl80211_state *state,
			struct nl_cb *cb,
			struct nl_msg *msg,
			int argc, char **argv)
{
	char *end;
	unsigned int retry_short;
	unsigned int retry_long;
...
COMMAND(set, retry_limit, "<short packet retry limit> <long packet retry limit>",
	NL80211_CMD_SET_WIPHY, 0, CIB_PHY, handle_retry_limit,
	"Set retransmission limit.\n"
	"Valid values: 0 - 255. default value is 7");
