...
if empty "$FORM_submit"; then
	...
	SAVED=1
validate <<EOF
int|FORM_distance_${device}|@TR<<Distance>>|min=1|$FORM_distance
EOF
        if [ "$?" = 0 ]; then
...
for vcfg in $vface; do
    ...
    if [ "$FORM_device" = "$device" ]; then
	    ...
	    eval FORM_mesh_id="\$FORM_mesh_id_$vcfgcnt"
            ...
            {
                case "$FORM_encryption" in
		    psk|psk2|psk+psk2|*mixed*) append validate_forms "wpapsk|FORM_wpa_psk_$vcfgcnt|@TR<<Encryption Key (min needs 8 characters) >>|min=8 max=63 required|$FORM_key" "$N";;
...
#	append forms "$network" "$N"
	...
	[ "$FORM_wep_passphrase" = "" ] && FORM_wep_passphrase="$(dd if=/dev/urandom count=200 bs=1 2>/dev/null|tr "\n" " "|sed 's/[^a-zA-Z0-9]//g')"
		...
		if empty "$FORM_wep_passphrase"; then
            ...
            encryption_forms="field|@TR<<Encryption>>
