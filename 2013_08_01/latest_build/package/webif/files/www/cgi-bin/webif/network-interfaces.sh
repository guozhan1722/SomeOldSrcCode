	...
	if [ "$switching_iface" != "1" ]; then
		...
		if [ "$FORM_network_iface" = "" ]; then
			for network in $networks; do
				...
				if [ "$type" = "bridge" ]; then
					#Use device to get the ifname of a bridged connection
					#config_get network_ifname $network ifname   
				        network_ifname="`uci get network.$network.ifname`"
				else
					eval network_ifname="\$ifnames_$network"
				fi
		...
		if [ "$FORM_submit" != "" ]; then
			for network in $networks; do
				...
				if [ "$type" = "bridge" ]; then
					#Use device to get the ifname of a bridged connection
					#config_get network_ifname $network ifname
				        network_ifnames="`uci get network.$network.ifname`"
				        network_ifname="`uci get network.$network.ifname`"
				else
					eval network_ifname="\$ifnames_$network"
				fi
				echo $network_ifname |grep -q "$iface"
				#if [ "$network_ifname" = "$iface" ]; then
				if [ "$?" = "0" ]; then
					if [ "$FORM_network_iface" != "$network" ]; then
						for interface in $network_ifname; do
					...
					if [ "$FORM_network_iface" = "$network" ]; then
						#prevent lots of spaces from being added to the config file
						if [ "$network_ifnames" != "" ] && [ "$network_ifnames" != "$iface" ]; then
