...
for interface in $network; do
	...
	if [ "$interface" != "loopback" -a "$interface" != "wan2" ]; then
...
netmask|FORM_${interface}_netmask|$interface @TR<<Netmask>>||$FORM_netmask
...
EOF
		[ "$?" = "0" -a "$interface" != "$FORM_add_network" ] && {
			...
			#   uci_set "network" "$interface" "ipaddr" ""
         		#   uci_set "network" "$interface" "ip6addr" ""
			#   uci_set "network" "$interface" "netmask" ""
			#   uci_set "network" "$interface" "gateway" ""
			#   uci_set "network" "$interface" "ip6gw" ""
			#   uci_set "network" "$interface" "dns" ""
                        logger "network set to dhcp"
			else
...
#	start_form|@TR<<IP Settings>>|${interface}_ip_settings|hidden
...
#	append forms "$remove_network_form" "$N"
	...
	javascript_forms="
		...
		v = (isset('${interface}_proto', 'static') || isset('${interface}_proto', 'pptp'));
