...
struct cpx_board boards[] = {
    ...
    BOARD(VENID_MICROHARD, DEVID_MICROHARD_VIPEXP, 16,
            ...
            "VIP421EXP", "Microhard ixp4xx export 1-miniPCI board",
            "Microhard ixp4xx export 1-miniPCI board wifi product",
            0x0, 0x0),
...
"  -V <fw version high>:<fw version low>\n"
"                  add firmware version high:low\n"
"  -h              show this screen\n"
	);
...
int
parse_opt_version(char ch, char *arg)
{
	...
	/* processing version high number*/
	p = argv[0];
	if (is_empty_arg(p)) {
		errmsg(0,"fw version high is missing from -%c %s",ch, arg);
		goto err_out;
	} else if (str2u32(p, &fw_header.fwhi) != 0) {
		errmsg(0,"invalid fw version high: %s", p);
		goto err_out;
	}

	dbgmsg(1, "fw version high is set to %0d", fw_header.fwhi);

	/* processing version low number*/
	p = argv[1];
	if (is_empty_arg(p)) {
		errmsg(0,"fw version low number is missing from -%c %s",ch, arg);
		goto err_out;
	} else if (str2u32(p, &fw_header.fwlo) != 0) {
		errmsg(0,"invalid fw version low: %s", p);
		goto err_out;
	}

	dbgmsg(1, "fw version low is set to %0d", fw_header.fwlo);
...
main(int argc, char *argv[])
{
	...
	fw_header.fwhi = 0;
	fw_header.fwlo = 0; /* default version number: 0.00 */
	fw_header.flags = 0;

	opterr = 0;  /* could not print standard getopt error messages */
	while ((c = getopt(argc, argv, "b:B:f:hi:p:r:s:V:v")) != -1) {
		optinvalid = 0;
		switch (c) {
		case 'b':
		...
		case 's':
			...
			break;
		case 'V':
			optinvalid = parse_opt_version(c,optarg);
			break;
		case 'v':
			verblevel++;
			break;
