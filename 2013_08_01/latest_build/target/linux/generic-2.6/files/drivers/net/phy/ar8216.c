	...
	for (i = 0; i < val->len; i++) {
		...
		if (p->flags & (1 << SWITCH_PORT_FLAG_TAGGED))
			...
			for (j = 0; j < AR8X16_MAX_VLANS; j++) {
				if ((j == val->port_vlan) || (p->id ==AR8216_PORT_CPU))//MHX: cpu port can be in multiple VLANs in untagged mode.
					continue;
...
ar8216_hw_apply(struct switch_dev *dev)
{
	...
	for (i = 0; i < AR8216_NUM_PORTS; i++) {
        ...
        //Read back and show
        val = priv->read(priv, AR8216_REG_PORT_STATUS(i));
        printk(KERN_INFO "\n* Swtich Port%x Status = 0x%08x\n",i,val );
        val = priv->read(priv, AR8216_REG_PORT_CTRL(i));
        printk(KERN_INFO "* Swtich Port%x Control = 0x%08x\n",i,val );
        val = priv->read(priv, AR8216_REG_PORT_VLAN(i));
        printk(KERN_INFO "* Swtich Port%x Port-based VLAN Register = 0x%08x\n",i,val );
	}
/***************************************
    val = priv->read(priv, 0x8);
	printk(KERN_INFO "	******MH AR8316 Reg8 (PowerOnStrapping) = 0x%x\n", val );
	val = priv->read(priv, 0x24);
	printk(KERN_INFO "	******MH AR8316 Reg24(MAC0.1.2.3) = %x\n",val );
    val = priv->read(priv, 0x78);
	printk(KERN_INFO "	******MH AR8316 Reg78(CPU Port) = %x\n",val );
****************************************/
...
ar8316_hw_init(struct ar8216_priv *priv) { 
        ...
        orig_val = val = priv->read(priv, 0x8); 
		...
		val = priv->read(priv, 0x78);
        val |= 0x100;   //cpu is connected to port0;
        priv->write(priv, 0x78, val); 
...
ar8216_reset_switch(struct switch_dev *dev)
{
	...
	for (i = 0; i < AR8216_NUM_PORTS; i++) {
		...
		if (i == AR8216_PORT_CPU) {
			priv->write(priv, AR8216_REG_PORT_STATUS(i),
                ...
                priv->write(priv, AR8216_REG_PORT_STATUS(i), AR8216_PORT_STATUS_LINK_AUTO);
		}
	}
	...
	priv->write(priv, 0x38, 0xc000050e);
        ...
        if (priv->chip == AR8316) { 
                /* enable Port0 to receive multicast and broadcast frames. And enable CPU port (BROAD_TO_CPU_EN=1) */ 
                priv->write(priv, AR8216_REG_FLOOD_MASK, 0x043f003f); 
        } 
        mutex_unlock(&priv->reg_mutex); 
...
ar8216_config_init(struct phy_device *pdev)
{
    ...
    printk(KERN_INFO "*MHX ar8216_config_init() : %s: AR%d PHY driver attached.\n", pdev->attached_dev->name, priv->chip); 
 
    if (pdev->addr != SWITCH_PHYADDR) { //addr0 is not connected (floating?). External PHY addresses start from 1.
            if (priv->chip == AR8316) { 
	...
	if ((ret = register_switch(&priv->dev, pdev->attached_dev)) < 0) {
        printk(KERN_INFO "ar8216_config_init: failed to register_switch(). \n"); 
		kfree(priv);
		goto done;
	}
	...
	dev->phy_ptr = priv;
        ...
        if (pdev->addr == SWITCH_PHYADDR && priv->chip == AR8216) { 
                pdev->pkt_align = 2; 
...
ar8216_probe(struct phy_device *pdev)
{
	...
	priv.phy = pdev;
/*************************************************************************
	printk(KERN_INFO "---MH ar8216_probe() Atheros device probed [ver=%d, rev=%d, phy_ctrl=%x, phy_status=%x, phy_id=%04x%04x]\n", 
              (int)(val >> AR8216_CTRL_VERSION_S), 
              (int)(val & AR8216_CTRL_REVISION), 
              priv.phy->bus->read(priv.phy->bus, priv.phy->addr, 0), 
              priv.phy->bus->read(priv.phy->bus, priv.phy->addr, 1), 
              priv.phy->bus->read(priv.phy->bus, priv.phy->addr, 2), 
              priv.phy->bus->read(priv.phy->bus, priv.phy->addr, 3));
*************************************************************************/
